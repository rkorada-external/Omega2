...
#
# Parametres entree :
# 	logFolderToRead(/..)		Repertoire ou lire les logs
#	fileNamePattern(*.log)		Masque permettant de filter les fichiers a lire
#	closingDate(YYYYMMDD)		La date du closing concernee
#	environment(DEV UAT PRD)	L environnement concerne
#	resultFolderToWrite(/..)	Repertoire ou les metriques sont ecrites
# Resultat en sortie :
#	2 fichiers dans resultFolderToWrite
# Exemple d appel:
#	reportLogs.py /scordata_dcvuatobbatch/ubas/log *20200317*.log 20200317 UAT /scor/data/logstash
...
#folderToWrite = "/scor/data/logstash"
#print ("Travail depuis: "+os.getcwd())
if len(sys.argv) < 5:
	print("Vous devez indiquer 4 arguments: logFolderToRead(/..) fileNamePattern(*.log) closingDate(YYYYMMDD) environment(DEV UAT PRD) resultFolderToWrite(/..)")
	sys.exit(-1)
# La date doit etre un numerique
try:
        int(sys.argv[3])
except ValueError:
	print("Le troisieme argument doit etre la date du closing (au format numerique YYYYMMDD)")
	sys.exit(-1)
...
print("Rep. ecriture result.:"+sys.argv[5])
	
folderToWrite = sys.argv[5] + "/"	+ sys.argv[4]
# main
print("Extraction...")
getLogsOfFolder(sys.argv[1],sys.argv[2],sys.argv[3], folderToWrite)		
