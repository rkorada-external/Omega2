...
def deliverComponent(cmp, spira, revision_dev = '0' ):
			...
			if ret == 0 :
				...
				if  len(a) > 1 :  
					...
					if read :
						...
						if ret == 0 :
							ret=SVN('svn export -r '+ revision_dev + '  ' +  url + ' ' + root_delivery + component + " --force")
						if ret == 0 :
							f=open(fName,'r') 
							source=re.search(r'^A    (.*)', f.read(), re.MULTILINE).group(1)
							f.close()
							ret=SVN( 'svn add '+ source )
							ret=SVN( 'svn commit '+ source +'   -m "' + label +'"')
							if os.path.getsize(fName) == 0 : 
								print "# Info: file " + cmp + " not changed since last commit"
								ret=SVN( 'svn info '+ source )
								text = open(fName,mode='r').read()
								result = re.search('Last Changed Rev: (.*)',text,re.MULTILINE)
								if  result :	
									revision_delivery=result.group(1)
									displayLogSvn()
									rec=InitEmptyRec()
								...
								f=open(fName,'r') 
								read=re.search(r'^Committed revision ([0-9]*).', f.read(), re.MULTILINE)
								f.close()
								if read :
	...
	print "info ---------------" , nb_component_not_commited , nbCompenentsToDelivered
	return ret
...
try:
	...
	for line in open(fName).readlines():
		matchLock=re.match("Lock Owner: (.*)",line.strip())
		if matchLock :
			userLock=matchLock.group(1)
	if userLock =="" :
		...
		print "\n-------------------------------------------\n"
		print "\t"+listComponentsFile + " is locked by " + userLock
		print "\n-------------------------------------------\n"
