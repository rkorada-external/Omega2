...
def AnalyseCmd ( env,chaines,job,step):
	
	if env.lower() == "dev":
		SRC_DIRECTORY_DCMD="/scor/scoromega/runnable/cmd/".format(site=site.lower())
	else:
		SRC_DIRECTORY_DCMD="/scoromega_runnable_aen{env}o2batch/cmd/".format(env=env.lower())
...
#  dicoPerms = extractPermFilesFromlog.getDicoPerms( chaines,'/scordata_dcvprdobbatch/ubeu','/scor/home/frdev11/mehdi/data','P','T')
	...
	for chain in dirs :
		...
		f= open(src_chain, "r", encoding='UTF-8')
		a = f.readlines()
		findCHAININT=0
		findJOBINIT=0
		...
		for line in a:
		   ...
		   if findCHAININT == 1 and  (re.search(r'\.cmd.*2>&1 | ${TEE}',line)) :
			   #print(( line)
			   AnalyseJobLine(env,chain.replace('.cmd',''),line.strip())
		f.close()
		#t=genxWiki(job,step)
		#open('out/'+ chaines+ '.xWiki', "w").write(t)
		getPermFile (xWikiFD,chain,job,step)
	
def getPermFile (xWikiFD,chain,job0,step0):
	pi=[]
	po=[]
	for chain in xWikiFD :
		for job in xWikiFD[chain] :
			...
			for step in xWikiFD[chain][job] :
				...
				for file in  xWikiFD[chain][job][step]['input'] : 
					if  re.search(r'{EST_.*',file[1]) or re.search(r'{EPO_.*',file[1]) or re.search(r'{EPL_.*',file[1]) : 
						ff= file[1].split('{')[1].split('}')[0]
						if ff not in pi :  pi.append(job+'\t'+ ff)
			for step in xWikiFD[chain][job] :
				if step0 != ''  and  step != job + '_' + step0 : continue
				for file in  xWikiFD[chain][job][step]['output'] : 
					if  re.search(r'{EST_.*',file[1]) or re.search(r'{EPO_.*',file[1]) or re.search(r'{EPL_.*',file[1]): 
						ff= file[1].split('{')[1].split('}')[0]
						if ff not in po :  po.append(job+'\t'+ ff)
	print(  '--- input -----')
	#pi.sort()
	print( chain+'\tinput\t', ('\n' +chain+'\tinput\t').join(pi)						)
	print(  '--- output -----')
	#po.sort()
	print(   chain+'\toutput\t', ('\n' +chain+'\toutput\t').join(po)						)
...
def AnalyseJobLine ( env,chain,line):
	if env.lower() == "dev":
		SRC_DIRECTORY_DCMD="/scor/scoromega/runnable/cmd/".format(site=site.lower())
	else:
		SRC_DIRECTORY_DCMD="/scoromega_runnable_aen{env}o2batch/cmd/".format(env=env.lower())
...
def AnalyseJobFile ( SRC_DIRECTORY , chain,job):
	...
	print (src_file)
	f=open(src_file, "r", encoding='UTF-8', errors='ignore')
	a = f.readlines()
	findSTEPINT=0
	NSTEP ="" ;
	linesOfStep=[]
	...
	for line in a:
		if line.strip().startswith("NSTEP=") :#or line.startswith("JOBEND"): 
			if NSTEP != "": 
...
def isListContainRegex(list,reg):
	for line in list:
		if re.search(reg,line.strip()): return 1
	return 0
...
def AnalyseBCP ( chain,job,linesOfStep ,STEP):
	for line in  linesOfStep: 
		if not  line.strip().startswith('#') :
			...
			if  re.search(r'BCP_I\=$',line.strip()) :  
				xWikiFD[chain][job][STEP]['input'].append(line.strip().split('='))
				print( "\t\t\t" +  line.strip())
			if  re.search(r'BCP_O=\$',line.strip()) :  
				xWikiFD[chain][job][STEP]['output'].append(line.strip().split('='))
				print( "\t\t\t" +  line.strip())
...
if __name__ == '__main__':
	if len(sys.argv) >= 4:  job =  sys.argv[3] 
	else: job=''

	if len(sys.argv) >= 5:  step =  sys.argv[4] 
	else: step = ''
						
	AnalyseCmd ( sys.argv[1] ,sys.argv[2],job,step)
