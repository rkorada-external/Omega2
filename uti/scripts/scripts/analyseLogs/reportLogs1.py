...
def getSteps(flog,dt):
	...
	for line in a:
		...
		IDF_CT_group=re.match( r'#----> IDF_CT .........................: (.*)', line)
		if IDF_CT_group :
			IDF_CT = IDF_CT_group.group(1)
		job_group=re.match( r'# Begin of job : (.*) Date :', line)
		if job_group :
			job = job_group.group(1)
		...
		if start :
			...
			step = start.group(1)
			num_step=step.replace(job+'_','')
			startSteps[start.group(1)]=(job,num_step,start.group(2))
			isInStep = True
	...
	for step in startSteps:
		deb=datetime.datetime.strptime(startSteps[step][2], frmt)
		s_end=''
		s_elapse=''
		read=''
		output1=''
		...
		if ( s_end != '') :
			IDF_CT=IDF_CT if IDF_CT != "" else chain
			stepsInfo.append(chain+";"+IDF_CT+";"+startSteps[step][0]+";"+startSteps[step][1]+";"+steps[step][8]+";"+ steps[step][0] +";"+ steps[step][1]+";"+steps[step][2]+";"+steps[step][3]+";"+steps[step][4]+";"+steps[step][5]+";"+steps[step][6]+";"+dt)

	return (chain,IDF_CT,elapse.strip(),stepsInfo,dt)
...
def getStepsOld(flog,dt):
	...
	for step in startSteps:
		...
		if ( s_end != '') :
			IDF_CT=IDF_CT if IDF_CT != "" else chain
			stepsInfo.append(chain+";"+IDF_CT+";"+job+";"+num_step+";"+steps[step][8]+";"+ steps[step][0] +";"+ steps[step][1]+";"+steps[step][2]+";"+steps[step][3]+";"+steps[step][4]+";"+steps[step][5]+";"+steps[step][6]+";"+dt)
...
def getLogsOfFolder ( folder, patern,dt ):
	#print(folder, patern,dt)
	DFILT=os.getenv("DFILT")
	fSteps = open(DFILT+"/"+"ElapsesStep.txt", "a")
	...
	for log in logs :
		#info=getChainInfo(log)
		ret=getSteps(log,dt)
		IDF_CT=ret[1] if ret[1] != "" else ret[2]
		fChains.write(dt+";"+ret[0]+";"+ret[1]+";"+ret[2]+"\n" )
		for val in ret[3] :
			print (val)
			fSteps.write(val+"\n")
	fSteps.close()
