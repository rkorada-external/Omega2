...
print pathname +  "/" + 'scrut'+delivryEnv +'.properties' 
config.read(pathname +  "/" + 'scrut'+delivryEnv +'.properties')
...
def getCmpAfterRevision(root, revision ):
	...
	for line in a :
		#print "line:",line
		tab=line.split()
		if ( tab[1] >= revision and tab[0] != "Status" and tab[3].find(".") != -1) :
			listComponentsAfterRevision[tab[3]]= tab
	return listComponentsAfterRevision


	
def createCsvFile(root,revision,status,fo):
	global ECHO
	cmpsList=getCmpAfterRevision(root,revision )
	ECHO="off"
	print "-------------------------"
...
fo= open(csvFile, 'w')
fo.write(header)
createCsvFile("/scor/scoromega/delivery/3I_DEV/oest",revision,status,fo)
createCsvFile("/scor/scoromega/delivery/3I_DEV/otec",revision,status,fo)
createCsvFile("/scor/scoromega/delivery/3I_DEV/oret",revision,status,fo)
createCsvFile("/scor/scoromega/delivery/3I_DEV/osta",revision,status,fo)
fo.close