...
def getLogsOfFolder (patern):
	DFILT=os.getenv("DFILT")
	fSteps = open(DFILT+"/"+"ElapsesStep.txt","w")
	fChains = open(DFILT+"/"+"ElapsesChain.txt","w")
	logs = glob.glob(patern)
	logs.sort(key=os.path.getmtime)
	for log in logs :
		print(log)
		info=getChainInfo(log)
...
#etLogsOfFolder(sys.argv[1],sys.argv[2],sys.argv[3])		
getLogsOfFolder(sys.argv[1])		
