...
def Init():
	...
	#pprint (dicoChainsFctPerms)
	
def updateChain (chain)	:
	
	print ("-------------------------------"     )
	print ("--mapping of ", chain                    )
	print ("\n	----------   Clean tables   ---------------------\n")

	print ( "	delete BEST..TI17PERMFIL where IDF_CT in  ( select IDF_CT from BEST..TI17FNC where   CHAIN_CT='{chain}')".format(chain=chain))
	print ( "	delete BEST..TI17REQFNC where     IDF_CT  in ( select IDF_CT from BEST..TI17FNC where   CHAIN_CT='{chain}')".format(chain=chain))
	print ( "	delete BEST..TI17FNC where CHAIN_CT='{chain}'".format(chain=chain))
	print ( "	delete BEST..TI17CHN  where CHAIN_CT='{chain}'".format(chain=chain))
	
	
	print ( "\n	insert into BEST..TI17CHN values ('{chain}',  '{label}')".format(chain=chain,label=dicoChains[chain][1]) )
	...
	for fct in dicoChainsFctPerms[chain]:
		...
		print ("\n	----------IDF_CT:  " , fct  , "------------------\n")
		print ( """		insert into BEST..TI17FNC values ('{IDF_CT}','{IDF_LL}','{CHAIN_CT}',{SEVERITY_CT})
		...
		for perm in dicoChainsFctPerms[chain][fct]:
				print ( "			insert into BEST..TI17PERMFIL values ('%s',  '%s','%s','%s','%s')" % (fct,perm[1],perm[2],perm[3],perm[4]))
		print ("\n		----------   Reqs    ---------------------\n")
...
def cleanAll():
	...
	print ( "	delete BEST..TI17PERMFIL" )
	print ( "	delete BEST..TI17REQFNC"   )
	print ( "	delete BEST..TI17CHN"   )
	print ( "	delete BEST..TI17FNC"   )
	print ( "	delete BEST..TI17REQ"   )
...
def isertTI17REQ () :
	for req in dicoReqs:
			print ( """	insert into BEST..TI17REQ values ('{reqcod_ct}', '{label}','{SAPOSTING_CT}','{REC_CF}')
			 """.format( reqcod_ct=dicoReqs[req][0],
...
if __name__ == '__main__':
	...
	print("root={root} mode={mode}  ".format(mode=mode,root=root) )
	if mode == "INIT":
		print("INIT root={root} mode={mode}  ".format(mode=mode,root=root) )
		sys.stdout = open(root+"/ALL_MAPPING.sql", "w")
		chains= getChains("%")
		Init()
		cleanAll()
		isertTI17REQ ()
		...
		if mode == "ALL":
			print("ALL CHAINS: root={root} mode={mode}  ".format(mode=mode,root=root) )
			chains= getChains("%")
			#pprint(chains)
			Init()
			for chain in chains:
				sys.stdout = open(root+"/MAPPING_{chain}.sql".format(chain=chain[0]), "w")
				updateChain (chain[0])

		else:
			print("ONE CHAI; root={root} mode={mode}  ".format(mode=mode,root=root) )
			chains= getChains(mode)
			#pprint(chains)
			Init()
			for chain in chains:
				updateChain (chain[0])
