...
keys_parameters = ['Keys', 'JOINKEYS' ,'BCP_WAY', 'BCP_OUT' ,'BCP_VER' ,'BCP_QRY', 'ISQL_BASE', 'ISQL_QRY', 'ZIP_MODE', 'ZIP_ODIR']
				...
				if  xWikiFD[chain][job][step]['libel'] != '' :
					...
					if len( xWikiFD[chain][job][step]['input']) == 0 :
						...
						for io in xWikiFD[chain][job][step]['input']	:
							...
							# if io[0] == 'Keys' or io[0] == 'JOINKEYS' or io[0] == 'BCP_WAY' or  io[0] == 'BCP_OUT' or io[0] == 'BCP_VER' or io[0] == 'BCP_QRY':
							if io[0] in keys_parameters:
...
def AnalyseStep ( chain,job,linesOfStep , STEP):
	...
	if isListContainRegex(linesOfStep,r'^\s*ISQL.*=*') == 1 : AnalyseISQL(chain, job, linesOfStep, STEP) 

	if isListContainRegex(linesOfStep,r'^\s*ZIP.*=*') == 1 : AnalyseZIP(chain, job, linesOfStep, STEP) 
...
def AnalyseSort ( chain,job,linesOfStep ,STEP):
	...
	for line in  linesOfStep: 
		...
		# parse JoinKeys
		if  joinKey != '' and endjoinKeys == 0 and re.search(r'^/',line.strip()):  
			endjoinKeys = 1
		if  re.search(r'^/JOINKEYS\s*',line.strip().upper()):  
			joinKey = line.strip().upper().replace("#.*",'')
			properties[countjoinKey] = joinKey
		if  joinKey != '' and endjoinKeys == 0 and not re.search(r'^/JOINKEYS\s',line.upper()):  
			joinKey += line.strip().upper()
			properties[countjoinKey] = joinKey.replace('/JOINKEYS','')


		if  re.search(r'^/INFILE\s', line.strip()):
			properties['INFILE'] = line.strip().split(' ')[1]
			joinKey=''
			countjoinKey='JOINKEYS_2'
			endjoinKeys=0
	...
	if properties['JOINKEYS_1']:
		xWikiFD[chain][job][STEP]['input'].append(['JOINKEYS', properties['JOINKEYS_1']])
	if properties['INFILE']:
		xWikiFD[chain][job][STEP]['input'].append(['INFILE', properties['INFILE']])
	if properties['JOINKEYS_2']:
		xWikiFD[chain][job][STEP]['input'].append(['JOINKEYS', properties['JOINKEYS_2']])
...
def AnalyseZIP(chain, job, linesOfStep, STEP):
	libel='WARNING missing description !!!!!!'

	for line in linesOfStep:
		line = line.strip()

		if re.search(r'LIBEL=',line):
			libel=line.replace('LIBEL=','').replace('"','')
			xWikiFD[chain][job][STEP]['libel']=libel
			xWikiFD[chain][job][STEP]['services'].append(('ZIP',libel))

		if re.search(r'.*ZIP_MODE=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))

		if re.search(r'.*ZIP_ODIR=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))

		if re.search(r'.*ZIP_I=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))
			FindPermAndInterFile(line)

		if re.search(r'.*ZIP_O=.*$',line):
			# # print "\t\t\t" +  line.strip()
			xWikiFD[chain][job][STEP]['output'].append(line.split('='))
			FindPermAndInterFile(line)

def AnalyseISQL(chain, job, linesOfStep, STEP):
	libel='WARNING missing description !!!!!!'
	...
	for line in linesOfStep:
		...
		if re.search(r'LIBEL=',line):
			libel=line.replace('LIBEL=','').replace('"','')
			xWikiFD[chain][job][STEP]['libel']=libel
			xWikiFD[chain][job][STEP]['services'].append(('ISQL',libel))

		if re.search(r'.*ISQL_BASE=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))

		if re.search(r'.*ISQL_QRY=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))

		if re.search(r'.*ISQL_I.*\=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))
			FindPermAndInterFile(line)

		if re.search(r'.*ISQL_O.*\=.*$',line):
			# # print "\t\t\t" +  line.strip()
			xWikiFD[chain][job][STEP]['output'].append(line.split('='))
			FindPermAndInterFile(line)
...
def AnalyseBCP ( chain,job,linesOfStep ,STEP):
	libel='WARNING missing description !!!!!!'
	if chain == 'ESFD3870' and job == 'ESFD3871' and STEP == 'ESFD3871_80':
		print(linesOfStep)
	for line in linesOfStep:
		line = line.strip()
		if chain == 'ESFD3870' and job == 'ESFD3871' and STEP == 'ESFD3871_80':
		...
		if re.search(r'LIBEL=',line):
			libel=line.replace('LIBEL=','').replace('"','')
			xWikiFD[chain][job][STEP]['libel']=libel
			xWikiFD[chain][job][STEP]['services'].append(('BCP',libel))

		if re.search(r'.*BCP_WAY=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))

		if re.search(r'.*BCP_VER=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))

		if re.search(r'.*BCP_QRY=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))

		if re.search(r'.*BCP_I.*\=.*$',line):
			xWikiFD[chain][job][STEP]['input'].append(line.split('='))
			FindPermAndInterFile(line)

		if re.search(r'.*BCP_O.*\=.*$',line):
			# # print "\t\t\t" +  line.strip()
			xWikiFD[chain][job][STEP]['output'].append(line.split('='))
			FindPermAndInterFile(line)

	if chain == 'ESFD3870' and job == 'ESFD3871' and STEP == 'ESFD3871_80':
		print(xWikiFD[chain][job][STEP])
