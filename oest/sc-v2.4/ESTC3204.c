...
void main(int argc  , char *argv[])
{
	...
	if ( n_OpenFileAppl ( "ESTC3204_O1","wt",&Kp_OutputFilMvt ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
	...
	if ( n_CloseFileAppl( "ESTC3204_I1", &( bd_RuptMvt.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTC3204_I2", &( bd_RuptExc.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTC3204_O1", &Kp_OutputFilMvt ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
...
retour :
	0K
==============================================================================*/
n_InitExc(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC3204_I2", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		return ERR ;
...
n_InitMvt(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC3204_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR ) 
		return ERR ;
	...
	char **pbd_InRecChild  ) /* adresse de la ligne de l'esclave Mouvement comptable */
{
	...
	if ( ( ret = strcmp( pbd_InRecOwner[EXC_CUR_CF], pbd_InRecChild[MVT_CUR_CF] ) ) != 0 ) return ret ;
...
{			
	...
	ptb_InRecChild[MVT_CUR_R] = ptb_InRecOwner[EXC_EXC_R] ;
...

{
	...
	sprintf( MsgAno, "La monnaie origine acceptation %s du mouvement comptable (Filiale %s - Annee bilan %s - Contrat %s - Avenant %s - Section %s - Exercice %s - Numero ex %s) n'existe pas dans la table des cours de change de devise BREF..TCURQUOT\n",
		ptb_InRecChild[MVT_CUR_CF], ptb_InRecChild[MVT_SSD_CF],  ptb_InRecChild[MVT_BALSHEY_NF],  
