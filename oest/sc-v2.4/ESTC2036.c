...
int n_ActionFirstRuptGT ( char **ptb_InRec_Cur)
{
	...
	if (ptb_InRec_Cur[GT_ESTCRB_CT][0]==' ') Kb_AR=0; else Kb_AR=1;
...
int n_ActionLigneGT(char **ptb_InRec_Cur)
{
	...
	if (Kb_AR==1)
	{
	  if ((Kc_crible=='O') || (Kc_crible=='N'))
	  {
	    ...
	    if ((Kb_SyncCmpl==1) && (atoi(ptb_InRec_Cur[GT_ACY_NF])<=Kn_annee))
	      Kb_COMACC_B=1;  /* Modif par Mehdi */
	    else
	      Kb_COMACC_B = 0 ; /* Ajout par Mehdi */
	  }
	  /* Pour les autres cribles, on prend l'AS du GT */
	  else Kb_COMACC_B=atoi(ptb_InRec_Cur[GT_COMACC_B]);
	}
	...
	switch (Kc_crible)
	{
	 case 'N' : n_WriteCols(Kp_GT_N_Fil,ptb_InRec_Cur,SEPARATEUR,0); break;
	 case 'R' : n_WriteCols(Kp_GT_R_Fil,ptb_InRec_Cur,SEPARATEUR,0); break;
	 default  : n_WriteCols(Kp_GT_OS_Fil,ptb_InRec_Cur,SEPARATEUR,0); break;
	}
...
)
{
	...
	/* derniere annee de compte statistiquee		 */
	...
	)
{
	...
	if ((ret=strcmp(pbd_InRecOwner[GT_CTR_NF],
			pbd_InRecChild[MTH_RETCTR_NF]))!=0)
	RETURN_VAL (ret);
...
int n_ActionLastRuptMth(char **ptb_InRecOwner,char **ptb_InRecChild)
{
	...
	strncpy(Ksz_Mois,ptb_InRecChild[MTH_LSTENDMTH_NF],2);
	Ksz_Mois[2]=0;
