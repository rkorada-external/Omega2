...
/*
 * DROP PROC dbo.PiCTRANO_20
 */
IF OBJECT_ID('dbo.PiCTRANO_20') IS NOT NULL
BEGIN
    DROP PROC dbo.PiCTRANO_20
    PRINT '<<< DROPPED PROC dbo.PiCTRANO_20 >>>'
END
go
...
Programme: PiCTRANO_20

Fichier script associé : ESICTR20.PRC
...
IF @retour = 1
 BEGIN
	...
	/*-----------------------------------------------------------------------------------
		Lancement procedure batch asynchrone qui Imprime "La liste des anomalies contrat"
...
exec sp_SCOR_INSPRC 'ESICTR20', 'PiCTRANO_20', 'BEST', 'ME34'
go
IF OBJECT_ID('dbo.PiCTRANO_20') IS NOT NULL
    PRINT '<<< CREATED PROC dbo.PiCTRANO_20 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROC dbo.PiCTRANO_20 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PiCTRANO_20
 */
GRANT EXECUTE ON dbo.PiCTRANO_20 TO GOMEGA
go
