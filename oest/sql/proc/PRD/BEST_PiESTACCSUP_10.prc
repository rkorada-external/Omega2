USE BEST
go
IF OBJECT_ID('PiESTACCSUP_10') IS NOT NULL
BEGIN
  DROP PROC PiESTACCSUP_10
  PRINT '<<< DROPPED PROC PiESTACCSUP_10 >>>'
END
go
create procedure PiESTACCSUP_10(
  @p_balshtyea_nf int,
...
print 'PiESTACCSUP_10 ==> @isPostingSas_d = %1! ',  @isPostingSas_d
...
execute @result04=BEST..PiACCSUP_04 @p_ssd_cf,@p_usr_cf,'batch', @datej, @isPostingSas_d
...
EXEC sp_procxmode 'dbo.PiESTACCSUP_10', 'unchained'
go
IF OBJECT_ID('dbo.PiESTACCSUP_10') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PiESTACCSUP_10 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PiESTACCSUP_10 >>>'
go
GRANT EXECUTE ON dbo.PiESTACCSUP_10 TO GOMEGA
go
GRANT EXECUTE ON dbo.PiESTACCSUP_10 TO GDBBATCH
go
