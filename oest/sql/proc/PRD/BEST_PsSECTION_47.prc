use BEST
go
if object_id('PsSECTION_47') is not null
begin
  drop PROC PsSECTION_47
  print '<<< DROPPED PROC PsSECTION_47 >>>'
end
go
create procedure PsSECTION_47
(
@p_segtyp_ct char(1)
)
...
Date de creation:
Description du programme: Descente du périmètre rétrocession vie au niveau CASEX des affaires non comptabilisables. ( on prend le complementaire du perimètre descendu par la proc PsSECTION_21 )
...
select
  ...
  null,   -- Champ non utilisé, modif du 15/09/98
  null,   -- Champ non utilisé, modif du 15/09/98
  null,   -- Champ non utilisé (segmentation client), modif 002
  null,   -- Champ non utilisé (segmentation client), modif 002
  null,   -- Champ non utilisé (segmentation client), modif 002
  null,   -- Champ non utilisé (segmentation client), modif 002
  null,   -- Champ non utilisé (segmentation client), modif 002
 ...
 from BRET..TRETSEC RETSEC, BRET..TRETCTR RETCTR, BTRAV..TESTSSD ESTSSD,  BRET..TRACCCOND RACCCOND, BRET..TRFAMPRE FAMPRE
  where RETSEC.SSD_CF=ESTSSD.SSD_CF
    and RETCTRSTS_CT not in ( 3, 19 )  -- on descend les affaires non comptabilisees
...
go
if object_id('PsSECTION_47') is not null
  print '<<< CREATED PROC PsSECTION_47 >>>'
else
  print '<<< FAILED CREATING PROC PsSECTION_47 >>>'
go
grant execute on PsSECTION_47 TO GOMEGA, GDBBATCH
go
