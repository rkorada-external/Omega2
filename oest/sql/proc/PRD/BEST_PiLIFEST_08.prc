...
IF OBJECT_ID ('dbo.PiLIFEST_08') IS NOT NULL
   BEGIN
      DROP PROCEDURE dbo.PiLIFEST_08

      IF OBJECT_ID ('dbo.PiLIFEST_08') IS NOT NULL
         PRINT '<<< FAILED DROPPING PROCEDURE dbo.PiLIFEST_08 >>>'
      ELSE
         PRINT '<<< DROPPED PROCEDURE dbo.PiLIFEST_08 >>>'
   END
go

/***** create procedure dbo.PiLIFEST_08 *****/
...
Description    	 : This procedure cleans the Estimates grid when user brings back a treaty from Automatic to other Estimates Types. To do so, S/P and R/P ratios must be reset on all sections.
				   Created for EST41 (linked to defect #42716)
...
INSERT INTO #TLIFEST
  SELECT CTR_NF,
   ...
   WHERE CTR_NF = @p_ctr_nf
	 AND DETTRNCOD_CF IN ('59000', '59001') -- Selecting all S/P and R/P values
...
-- Ratios are reset to 0 for all sections, all cells of the grid
...
RETURN @v_erreur
go
EXEC sp_procxmode 'dbo.PiLIFEST_08', 'unchained'
go

IF OBJECT_ID ('dbo.PiLIFEST_08') IS NOT NULL
   PRINT '<<< CREATED PROCEDURE dbo.PiLIFEST_08 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROCEDURE dbo.PiLIFEST_08 >>>'
go

GRANT EXECUTE ON dbo.PiLIFEST_08 TO GOMEGA
go
GRANT EXECUTE ON dbo.PiLIFEST_08 TO GDBBATCH
go
