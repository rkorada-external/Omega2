...
/*
 * DROP PROC dbo.PiSEGMENT_10
 */
IF OBJECT_ID('dbo.PiSEGMENT_10') IS NOT NULL
BEGIN
    DROP PROC dbo.PiSEGMENT_10
    PRINT '<<< DROPPED PROC dbo.PiSEGMENT_10 >>>'
END
go
...
Programme: PiSEGMENT_10

Fichier script associé : ESISEG10.PRC
...
IF @retour = 1
 BEGIN
	...
	/*-----------------------------------------------------------------------------
		Lancement procedure batch asynchrone qui Imprime "La liste des segments"
...
exec sp_SCOR_INSPRC 'ESISEG10', 'PiSEGMENT_10', 'BEST', 'ME34'
go
IF OBJECT_ID('dbo.PiSEGMENT_10') IS NOT NULL
    PRINT '<<< CREATED PROC dbo.PiSEGMENT_10 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROC dbo.PiSEGMENT_10 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PiSEGMENT_10
 */
GRANT EXECUTE ON dbo.PiSEGMENT_10 TO GOMEGA
go
