...
IF OBJECT_ID('dbo.PuACCEXCPRO_LSTUPDUSR') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.PuACCEXCPRO_LSTUPDUSR
    IF OBJECT_ID('dbo.PuACCEXCPRO_LSTUPDUSR') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.PuACCEXCPRO_LSTUPDUSR >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.PuACCEXCPRO_LSTUPDUSR >>>'
END
go
CREATE PROCEDURE dbo.PuACCEXCPRO_LSTUPDUSR
AS
/***************************************************
...
Ajout du LSTUPDUSR dans la table ACCEXCPRO
...
UPDATE  BEST..TACCEXCPRO
SET     e.LSTUPDUSR_CF  = t.LSTUPDUSR_CF
FROM    BEST..TACCEXCPRO e,
        BCTA..TACCTRN t
...
go
IF OBJECT_ID('dbo.PuACCEXCPRO_LSTUPDUSR') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PuACCEXCPRO_LSTUPDUSR >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PuACCEXCPRO_LSTUPDUSR >>>'
go
GRANT EXECUTE ON dbo.PuACCEXCPRO_LSTUPDUSR TO GOMEGA
go
GRANT EXECUTE ON dbo.PuACCEXCPRO_LSTUPDUSR TO GDBBATCH
go
