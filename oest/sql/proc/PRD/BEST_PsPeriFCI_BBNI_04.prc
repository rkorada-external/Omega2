...
go
/*
 * DROP PROC dbo.PsPeriFCI_BBNI_04
 */
IF OBJECT_ID('dbo.PsPeriFCI_BBNI_04') IS NOT NULL
BEGIN
    DROP PROC dbo.PsPeriFCI_BBNI_04
    PRINT '<<< DROPPED PROC dbo.PsPeriFCI_BBNI_04 >>>'
END
go
...
create procedure PsPeriFCI_BBNI_04
     (
       @p_segtyp_ct           char(1),
       @p_ssd_cf              USSD_CF,
       @p_seg_d               char(8),
...
go
IF OBJECT_ID('dbo.PsPeriFCI_BBNI_04') IS NOT NULL
    PRINT '<<< CREATED PROC dbo.PsPeriFCI_BBNI_04 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROC dbo.PsPeriFCI_BBNI_04 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PsPeriFCI_BBNI_04
 */
GRANT EXECUTE ON dbo.PsPeriFCI_BBNI_04 TO GOMEGA
go
GRANT EXECUTE ON dbo.PsPeriFCI_BBNI_04 TO GDBBATCH
go
