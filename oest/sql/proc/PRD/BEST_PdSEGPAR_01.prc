...
DROP PROC dbo.PdSEGPAR_01
go

IF OBJECT_ID('dbo.PdSEGPAR_01') IS NOT NULL
   BEGIN
   DROP PROC dbo.PdSEGPAR_01
   PRINT '<<< DROPPED PROC dbo.PdSEGPAR_01 >>>'
END
go
...
Programme: PdSEGPAR_01

Fichier script associé : ESDSEG01.PRC
...
Description du programme: 
      ...
      suppression d'enregistrement dans TSEGPAR
...
exec sp_SCOR_INSPRC 'ESDSEG01', 'PdSEGPAR_01', 'BEST', 'ANB'
go

IF OBJECT_ID('dbo.PdSEGPAR_01') IS NOT NULL
   PRINT '<<< CREATED PROC dbo.PdSEGPAR_01 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROC dbo.PdSEGPAR_01 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PdSEGPAR_01
 */
GRANT EXECUTE ON dbo.PdSEGPAR_01 TO GOMEGA
go
