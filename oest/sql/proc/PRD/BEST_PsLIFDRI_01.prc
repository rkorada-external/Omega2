 ...
 *
 * DROP PROC dbo.PsLIFDRI_01
*/

IF OBJECT_ID('dbo.PsLIFDRI_01') IS NOT NULL
BEGIN
   DROP PROC dbo.PsLIFDRI_01
   PRINT '<<< DROPPED PROC dbo.PsLIFDRI_01 >>>'
END
go
...
Create Procedure PsLIFDRI_01 (@p_end_nt        UEND_NT,
                              @p_sec_nf        USEC_NF,
                              @p_uw_nt         UUW_NT,
                              @p_uwy_nf        UUWY_NF,
	                          @p_visu_mois     tinyint,
...
Fichier script associé      : BEST_PsLIFDRI_01.prc
...
Description du programme    : Sélection d'enregistrement dans TLIFDRI :  ESTIMATIONS VIE, 
                              cas où type comptable = 1 ou 4 (année de compte = exercice)
...
-- ainsi que l'indicateur de rétro interne
...
/*--------------------------------------------------*/
/* Selection dans TLIFDRI : on ne prend par année   */ 
...
exec sp_SCOR_INSPRC 'ESSDRI01', 'PsLIFDRI_01', 'BEST', 'ME01'
go

IF OBJECT_ID('dbo.PsLIFDRI_01') IS NOT NULL
   PRINT '<<< CREATED PROC dbo.PsLIFDRI_01 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROC dbo.PsLIFDRI_01 >>>'
go

/*
 * Granting/Revoking Permissions on dbo.PsLIFDRI_01
 */

GRANT EXECUTE ON dbo.PsLIFDRI_01 TO GOMEGA
go
