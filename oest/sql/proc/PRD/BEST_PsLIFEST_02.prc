 ...
 *
 * DROP PROC dbo.PsLIFEST_02
*/

IF OBJECT_ID('dbo.PsLIFEST_02') IS NOT NULL
   BEGIN
   DROP PROC dbo.PsLIFEST_02
   PRINT '<<< DROPPED PROC dbo.PsLIFEST_02 >>>'
END
go
...
Programme: PsLIFEST_02

Fichier script associé : ESSLIF02.PRC
...
Date de creation: 7 Avril 1997

Description du programme:

 	ESTIMATIONS VIE Acceptation et Rétro :
	Sélection dans TLIFEST des estimations d'un contrat de type comptable 2, 3 ou 5,
	ceci tous exercices confondus.
...
/*--------------------------------------------------*/
/* Maj montants cumulés par années de compte,       */
/* code traitement (prs_cf) et poste cumul          */
...
Select acmtrs_nt,
	   ...
	   sum(estmnt_m)
from #montants_w
group by acy_nf, prs_cf, acmtrs_nt
order by acy_nf, prs_cf, acmtrs_nt
...
exec sp_SCOR_INSPRC 'ESSLIF02', 'PsLIFEST_02', 'BEST', 'ME01'
go

IF OBJECT_ID('dbo.PsLIFEST_02') IS NOT NULL
   PRINT '<<< CREATED PROC dbo.PsLIFEST_02 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROC dbo.PsLIFEST_02 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PsLIFEST_02
 */
GRANT EXECUTE ON dbo.PsLIFEST_02 TO GOMEGA
go
