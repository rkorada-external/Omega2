USE BEST
go
IF OBJECT_ID('dbo.PsLORETFACTOR_SERQ_02') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.PsLORETFACTOR_SERQ_02
    IF OBJECT_ID('dbo.PsLORETFACTOR_SERQ_02') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.PsLORETFACTOR_SERQ_02 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.PsLORETFACTOR_SERQ_02 >>>'
END
go
 ...
 * creation des  fonctions 
    ...
    BEST..PsLORETFACTOR_SERQ_02  '20250630', 'INV'
 ...
 */
CREATE PROCEDURE dbo.PsLORETFACTOR_SERQ_02
(
...
Programme:              PsLORETFACTOR_SERQ_02
Fichier script associé: PsLORETFACTOR_SERQ_02.PRC
...
[013] 30/06/2025  M.NAJI SPIRA 111672  Evolution SERQ
[014] 25/11/2025  M.NAJI US7359 SERQS - fix doublons
*****************************************************/
...
SELECT distinct
        ...
        FROM  					bret..tcession a
			...
			left outer join  	bret..tretctr b  on   a.retctr_nf=b.retctr_nf        and a.rty_nf=b.rty_nf   and   b.ctrexp_d IS NOT NULL   
			join  				btrt..tcontr c   on    a.UWY_NF = c.UWY_NF     and a.UW_NT = c.UW_NT   and a.CTR_NF = c.CTR_NF
			join  				btrt..tsection e on a.UWY_NF = e.UWY_NF       and a.UW_NT = e.UW_NT   and a.CTR_NF = e.CTR_NF    
			left outer join   	BREF..TBATCHSSD acc on   a.ACCSSD_CF = acc.SSD_CF
			left outer join   	BREF..TBATCHSSD ret on    a.RETSSD_CF = ret.SSD_CF      
		where 	( (a.cesupdtyp_cf='' AND a.cessts_cf='01') OR                   (a.cesupdtyp_cf='S' AND a.cessts_cf='03')                )
			...
			and   b.ctrexp_d IS NOT NULL 
			and  CTRSTS_CT IN (14, 16, 17, 19) --[010]
union        
SELECT distinct
            a.CTR_NF,
            c.END_NT, --0 END_NT,
        ...
        FROM					bret..tcession a
				...
				join  			bfac..tsection e on a.UWY_NF = e.UWY_NF       and a.UW_NT = e.UW_NT   and a.CTR_NF = e.CTR_NF   
				left outer join BREF..TBATCHSSD acc on   a.ACCSSD_CF = acc.SSD_CF
				left outer join BREF..TBATCHSSD ret on    a.RETSSD_CF = ret.SSD_CF      
		where 	(   (a.cesupdtyp_cf='' AND a.cessts_cf='01') OR                   (a.cesupdtyp_cf='S' AND a.cessts_cf='03')                )
		and 	a.CESSIONCAT_CF= '1'
        ...
        RETURN 0
go
EXEC sp_procxmode 'dbo.PsLORETFACTOR_SERQ_02', 'unchained'
go
IF OBJECT_ID('dbo.PsLORETFACTOR_SERQ_02') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PsLORETFACTOR_SERQ_02 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PsLORETFACTOR_SERQ_02 >>>'
go
GRANT EXECUTE ON dbo.PsLORETFACTOR_SERQ_02 TO GOMEGA
go
GRANT EXECUTE ON dbo.PsLORETFACTOR_SERQ_02 TO GDBBATCH
go
