...
/*
 * DROP PROC dbo.PsUNDSTA_10 */
IF OBJECT_ID('dbo.PsUNDSTA_10') IS NOT NULL
BEGIN
    DROP PROC dbo.PsUNDSTA_10
    PRINT '<<< DROPPED PROC dbo.PsUNDSTA_10 >>>'
END
go

/*
 * creation de la procedure */
create procedure PsUNDSTA_10 (
    @p_ctr_nf   UCTR_NF,
    @p_end_nt   UEND_NT,
    @p_sec_nf   USEC_NF,
    @p_uwy_nf   UUWY_NF,
...
exec sp_SCOR_INSPRC 'ESSUND10', 'PsUNDSTA_10', 'BEST', 'ME34'
go
IF OBJECT_ID('dbo.PsUNDSTA_10') IS NOT NULL
    PRINT '<<< CREATED PROC dbo.PsUNDSTA_10 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROC dbo.PsUNDSTA_10 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PsUNDSTA_10
 */
GRANT EXECUTE ON dbo.PsUNDSTA_10 TO GOMEGA
go
