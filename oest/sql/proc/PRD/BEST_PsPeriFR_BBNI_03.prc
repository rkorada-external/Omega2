 ...
 */
 ...
 /* DROP PROC dbo.PsPeriFR_BBNI_03
*/
IF OBJECT_ID('dbo.PsPeriFR_BBNI_03') IS NOT NULL
   BEGIN
   DROP PROC dbo.PsPeriFR_BBNI_03
   PRINT '<<< DROPPED PROC dbo.PsPeriFR_BBNI_03 >>>'
END
go
...
create procedure PsPeriFR_BBNI_03
     (
       @p_segtyp_ct           char(1),
       @p_ssd_cf              USSD_CF,
       @p_seg_d               char(8),
...
IF OBJECT_ID('dbo.PsPeriFR_BBNI_03') IS NOT NULL
   PRINT '<<< CREATED PROC dbo.PsPeriFR_BBNI_03 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROC dbo.PsPeriFR_BBNI_03 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PsPeriFR_BBNI_03
 */
GRANT EXECUTE ON dbo.PsPeriFR_BBNI_03 TO GOMEGA
go
GRANT EXECUTE ON dbo.PsPeriFR_BBNI_03 TO GDBBATCH
go
