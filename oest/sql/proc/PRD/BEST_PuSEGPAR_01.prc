...
DROP PROC dbo.PuSEGPAR_01
go

IF OBJECT_ID('dbo.PuSEGPAR_01') IS NOT NULL
   BEGIN
   DROP PROC dbo.PuSEGPAR_01
   PRINT '<<< DROPPED PROC dbo.PuSEGPAR_01 >>>'
END
go
...
Programme: PuSEGPAR_01

Fichier script associé : ESUSEG01.PRC
...
Description du programme: 
      ...
      Modification d'enregistrement dansTSEGPAR 
...
exec sp_SCOR_INSPRC 'ESUSEG01', 'PuSEGPAR_01', 'BEST', 'ANB'
go

IF OBJECT_ID('dbo.PuSEGPAR_01') IS NOT NULL
   PRINT '<<< CREATED PROC dbo.PuSEGPAR_01 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROC dbo.PuSEGPAR_01 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PuSEGPAR_01
 */
GRANT EXECUTE ON dbo.PuSEGPAR_01 TO GOMEGA
go
