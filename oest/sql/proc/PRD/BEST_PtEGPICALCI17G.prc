use BEST
go
if object_id('PtEGPICALCI17G') is not null
begin
  drop PROC PtEGPICALCI17G
  print '<<< DROPPED PROC PtEGPICALCI17G >>>'
end
go

create procedure PtEGPICALCI17G
(
	@p_Norme     char(5),
	@p_sgttyp_nt int,
	@p_segtyp_ct char(1),
...
MOD04:01/12/2020:KBagwe:91591: Main lob batch update
MOD05:08/03/2021:K Bhimasen:92910:IFRS 17 - Errors in the determination of I17 segment
MOD06:08/10/2021:K Bhimasen: 99582: UAT contrats avec main lob vide
MOD07:22/12/2021:K Bhimasen: 101019: Main LOB - EGPI nil
*****************************************************/
...
SELECT @labl = "Multi LoB " + upper(substring(suser_name(), 3, 2))
...
--BEGIN TRAN
	...
	AND C.SGTTYP_NT = @p_sgttyp_nt AND c.CTRTYP_CT = 'T'
	AND B.CRUW_NT = C.CRUW_NT AND B.CRUWY_NF= C.CRUWY_NF
	...
	WHERE ROWSEL_B=1   AND B.CR_NF = C.CR_NF  AND B.CRUWY_NF = C.CRUWY_NF AND B.CRUW_NT = C.CRUW_NT 
	...
	WHERE b.CTR_NF = a.CTR_NF AND b.UW_NT = a.UW_NT  
		...
		AND B.SGTTYP_NT = @p_sgttyp_nt AND B.CTRTYP_CT = 'T'
		AND A.CRUW_NT = B.CRUW_NT AND A.CRUWY_NF= B.CRUWY_NF
		AND A.CRUW_NT = C.CRUW_NT AND A.CRUWY_NF= C.CRUWY_NF
	...
	WHERE b.CTR_NF = a.CTR_NF AND b.UWY_NF = a.UWY_NF AND b.UW_NT = a.UW_NT  AND b.UWY_NF = a.UWY_NF AND b.END_NT = a.END_NT 
	  AND b.DIV_NT = a.DIV_NT AND b.END_NT = a.END_NT AND b.SEC_NF = a.SEC_NF AND A.CTRTYP_CT = 'F'
		 ...
		 AND A.CR_NF = B.CR_NF
	--	AND (isnull(GRPIFRSSEG_CT,"") <> isnull(C.MAINLOB,"") OR isnull(GRPIFRSSEG_LL ,"") <> isnull(C.IFRS17SEGMENT,"") )
		AND B.SGTTYP_NT = @p_sgttyp_nt AND B.CTRTYP_CT = 'F'
		AND A.CRUW_NT = B.CRUW_NT AND A.CRUWY_NF= B.CRUWY_NF
		AND A.CRUW_NT = C.CRUW_NT AND A.CRUWY_NF= C.CRUWY_NF
...
if object_id('PtEGPICALCI17G') is not null
  print '<<< CREATED PROC PtEGPICALCI17G >>>'
else
  print '<<< FAILED CREATING PROC PtEGPICALCI17G >>>'
go
grant execute on PtEGPICALCI17G TO GOMEGA
go
grant execute on PtEGPICALCI17G TO GDBBATCH
go
