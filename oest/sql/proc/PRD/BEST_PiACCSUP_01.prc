use BEST
go
if object_id('dbo.PiACCSUP_01') is not null
begin
  drop PROC dbo.PiACCSUP_01
  print '<<< DROPPED PROC dbo.PiACCSUP_01 >>>'
end
go
create procedure PiACCSUP_01
  (
  @p_trn_nt          numeric
 ,@p_acctrn_nt       numeric
 ,@p_acctyp_nf       tinyint
...
go
if object_id('dbo.PiACCSUP_01') is not null
  print '<<< CREATED PROC dbo.PiACCSUP_01 >>>'
else
  print '<<< FAILED CREATING PROC dbo.PiACCSUP_01 >>>'
go
grant execute on dbo.PiACCSUP_01 TO GOMEGA
go
