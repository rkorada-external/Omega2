use BEST
go
if object_id('PsPeriTrtIni') is not null
begin
   drop procedure PsPeriTrtIni
   if object_id('PsPeriTrtIni') is not null
     print '<<< FAILED DROPPING procedure PsPeriTrtIni >>>'
   else
     print '<<< DROPPED procedure PsPeriTrtIni >>>'
end
go
create procedure PsPeriTrtIni
  (
  @p_clo_date char(8),
		@p_x_days int,
...
Description:
    - Descent of the acceptance perimeter of treaties bases for the INI perimeter
Comments:
MODIFICATIONS
...
[006] ??
[007] ??
[008] FCI spira 109507 I17 - Modify rule of CSM and LC pattern computation for multi year contracts 
[009] MZM   28/07/2025 :US 6250  : 112796 Cut-off management : Contract recognized day of cut-off should be taken into account
*****************************************************/
...
create table #TFAMRSVP(
    ...
    POLED_D      datetime      NULL,   -- MODIF 007 
...
select a.CTR_NF, a.UWY_NF, a.UW_NT, a.END_NT, a.SEC_NF, a.ERNPRMADM_B, a.POLDURMTH_NF, a.INSPOL_R, a.URRCAL_R, a.POLED_D, a.MULTICAN_D  --MODIF 008 
...
/* Création d'un index sur la table temporaire #TCLI    */
/* ---------------------------------------------------- */
...
BEGIN
	...
	SELECT @v_pos_booking_minus_days = dateadd(day,1,dateadd(day, @p_x_days * -1, @v_pos_booking_d) )   --009 dateadd(day, @p_x_days * -1, @v_pos_booking_d)
END
ELSE 
BEGIN
	SELECT @v_pos_booking_minus_days = dateadd(day, 1, convert(datetime, @p_quarter_end, 103) )  --009
END
...
,CONTR.MULTUWY_NF
,convert(char(8), CONTR.CTREXP_D, 112) -- Modif 008 EXP2_D
,convert(char(8), FAMRSVP.MULTICAN_D, 112) -- Modif 008	  
...
go
if object_id('PsPeriTrtIni') is not null
  print '<<< CREATED procedure PsPeriTrtIni >>>'
else
  print '<<< FAILED CREATING procedure PsPeriTrtIni >>>'
go
grant execute on PsPeriTrtIni TO GDBBATCH, GOMEGA
go
