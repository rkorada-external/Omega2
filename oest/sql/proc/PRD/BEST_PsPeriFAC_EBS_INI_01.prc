use BEST
go
if object_id('PsPeriFAC_EBS_INI_01') is not null
begin
   drop procedure PsPeriFAC_EBS_INI_01
   if object_id('PsPeriFAC_EBS_INI_01') is not null
     print '<<< FAILED DROPPING procedure PsPeriFAC_EBS_INI_01 >>>'
   else
     print '<<< DROPPED procedure PsPeriFAC_EBS_INI_01 >>>' 
end

go
create procedure PsPeriFAC_EBS_INI_01
(
...
go
if object_id('PsPeriFAC_EBS_INI_01') is not null
  print '<<< CREATED procedure PsPeriFAC_EBS_INI_01 >>>'
else
  print '<<< FAILED CREATING procedure PsPeriFAC_EBS_INI_01 >>>'
go
grant execute on PsPeriFAC_EBS_INI_01 TO GDBBATCH, GOMEGA
go
