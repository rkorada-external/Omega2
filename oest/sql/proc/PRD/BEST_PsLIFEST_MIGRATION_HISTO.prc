USE BEST
go
IF OBJECT_ID('dbo.PsLIFEST_MIGRATION_HISTO') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.PsLIFEST_MIGRATION_HISTO
    IF OBJECT_ID('dbo.PsLIFEST_MIGRATION_HISTO') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.PsLIFEST_MIGRATION_HISTO >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.PsLIFEST_MIGRATION_HISTO >>>'
END
go
/*
...
create procedure dbo.PsLIFEST_MIGRATION_HISTO
   (
...
Programme: PsLIFEST_MIGRATION_HISTO

Fichier script associé : PsLIFEST_MIGRATION_HISTO.prc
...
create index I_LIFEST_04 on #lifest_final(CTR_NF,END_NT,SEC_NF,UWY_NF,UW_NT,CRE_D,BALSHEY_NF,BALSHTMTH_NF,ACY_NF,PRS_CF, ACMTRS_NT, DETTRNCOD_CF, GAAP_NT) 
   ...
   select a.CTR_NF,
           ...
           a.PRS_CF,
           ACMTRS_NT+1,
           convert(char,convert(int,DETTRNCOD_CF)+100) DETTRNCOD_CF,
           ' ',
           a.SSD_CF,
           a.CUR_CF,
           -1* ESTMNT_M,
...
return 0
go
EXEC sp_procxmode 'dbo.PsLIFEST_MIGRATION_HISTO', 'unchained'
go
IF OBJECT_ID('dbo.PsLIFEST_MIGRATION_HISTO') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PsLIFEST_MIGRATION_HISTO >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PsLIFEST_MIGRATION_HISTO >>>'
go
GRANT EXECUTE ON dbo.PsLIFEST_MIGRATION_HISTO TO GOMEGA
go
GRANT EXECUTE ON dbo.PsLIFEST_MIGRATION_HISTO TO GDBBATCH
go
