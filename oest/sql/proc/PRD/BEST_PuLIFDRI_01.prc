...
IF OBJECT_ID('dbo.PuLIFDRI_01') IS NOT NULL
   BEGIN
   DROP PROC dbo.PuLIFDRI_01
   PRINT '<<< DROPPED PROC dbo.PuLIFDRI_01 >>>'
END
go
...
IF OBJECT_ID('dbo.PuLIFDRI_01') IS NOT NULL
   PRINT '<<< CREATED PROC dbo.PuLIFDRI_01 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROC dbo.PuLIFDRI_01 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PiLIFDRI_01
 */
GRANT EXECUTE ON dbo.PuLIFDRI_01 TO GOMEGA
go
