use BEST
go
if object_id('PsPeriFACACCEPT_EBS_INI_01_02') is not null
begin
   drop procedure PsPeriFACACCEPT_EBS_INI_01_02
   if object_id('PsPeriFACACCEPT_EBS_INI_01_02') is not null
     print '<<< FAILED DROPPING procedure PsPeriFACACCEPT_EBS_INI_01_02 >>>'
   else
     print '<<< DROPPED procedure PsPeriFACACCEPT_EBS_INI_01_02 >>>' 
end

go
create procedure PsPeriFACACCEPT_EBS_INI_01_02
(
...
Description du programme:
    Creation d'un Pericase Dedié au BBNI pour FAC ACCEPTED((Bound But Not yet Incurred)
 ...
 FROM BFAC..TSECTION SECTION, BFAC..TCONTR CONTR, BFAC..TFAMLIA FAMLIA, BFAC..TFAMCHG FAMCHG
  ...
  WHERE SECSTS_CT in ( 14)   
    ...
    and SECTION.UW_NT*=SECIFRS.UW_NT     
     ...
     --and CONTR.CTRINC_D >= dateadd(day, 1, @v_clo_date )   -- BBNI Critere pour FAC AccEPTED
   	 and ( @norme_cf = 'EBS' ) 
...
go
if object_id('PsPeriFACACCEPT_EBS_INI_01_02') is not null
  print '<<< CREATED procedure PsPeriFACACCEPT_EBS_INI_01_02 >>>'
else
  print '<<< FAILED CREATING procedure PsPeriFACACCEPT_EBS_INI_01_02 >>>'
go
grant execute on PsPeriFACACCEPT_EBS_INI_01_02 TO GDBBATCH, GOMEGA
go
