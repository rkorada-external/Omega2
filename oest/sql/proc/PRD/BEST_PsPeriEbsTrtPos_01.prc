use BEST
go
if object_id('PsPeriEbsTrtPos_01') is not null
begin
   drop procedure PsPeriEbsTrtPos_01
   if object_id('PsPeriEbsTrtPos_01') is not null
     print '<<< FAILED DROPPING procedure PsPeriEbsTrtPos_01 >>>'
   else
     print '<<< DROPPED procedure PsPeriEbsTrtPos_01 >>>'
end
go
create procedure PsPeriEbsTrtPos_01
  (
...
Description:
    - Descent of the acceptance perimeter of treaties bases for the POS BOOKING perimeter
...
go
if object_id('PsPeriEbsTrtPos_01') is not null
  print '<<< CREATED procedure PsPeriEbsTrtPos_01 >>>'
else
  print '<<< FAILED CREATING procedure PsPeriEbsTrtPos_01 >>>'
go
grant execute on PsPeriEbsTrtPos_01 TO GDBBATCH, GOMEGA
go
