USE BEST
go
IF OBJECT_ID('dbo.PsPeriRet_EBS_INI_01') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.PsPeriRet_EBS_INI_01
    IF OBJECT_ID('dbo.PsPeriRet_EBS_INI_01') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.PsPeriRet_EBS_INI_01 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.PsPeriRet_EBS_INI_01 >>>'
END
go
create procedure PsPeriRet_EBS_INI_01
(
...
US 7847
Description du programme: Descente du p�rim�tre r�tron EBS INI   
...
where RETSEC.SSD_CF=ESTSSD.SSD_CF
    ...
    and ( RETIFRS.SIIFSTCLO_D is null ) and (( RETIFRS.SIIINISTS_CT = 1)  OR (  RETIFRS.SIIINISTS_CT is null ) )
...
return 0
go
EXEC sp_procxmode 'dbo.PsPeriRet_EBS_INI_01', 'unchained'
go
IF OBJECT_ID('dbo.PsPeriRet_EBS_INI_01') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PsPeriRet_EBS_INI_01 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PsPeriRet_EBS_INI_01 >>>'
go
GRANT EXECUTE ON dbo.PsPeriRet_EBS_INI_01 TO GOMEGA
go
GRANT EXECUTE ON dbo.PsPeriRet_EBS_INI_01 TO GDBBATCH
go
