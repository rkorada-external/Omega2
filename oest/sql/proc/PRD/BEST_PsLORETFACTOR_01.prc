USE BEST
go
IF OBJECT_ID('dbo.PsLORETFACTOR_01') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.PsLORETFACTOR_01
    IF OBJECT_ID('dbo.PsLORETFACTOR_01') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.PsLORETFACTOR_01 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.PsLORETFACTOR_01 >>>'
END
go
 ...
 */
CREATE PROCEDURE dbo.PsLORETFACTOR_01
(
...
Programme:              PsLORETFACTOR_01
Fichier script associé: PsLORETFACTOR_01.PRC
...
SELECT distinct
           ...
           ' ' CUR_CF,
            ...
            LOFACTOR = case When                -- [006]
                            (case       
                                    when (b.RETACCTYP_CT in (1,2,4) OR b.RETACCTYP_CT IS NULL)  OR (c.ctrexp_d IS NULL AND c.scoexp_d IS NULL) --(b.RETACCTYP_CT in (3,5) OR b.RETACCTYP_CT IS NULL)
                                        then 1
                                    when (  ( (a.ACCADMTYP_CT in (1,3,4,5) ) AND (cast(datediff(DAY, ISNULL(c.ctrexp_d, c.scoexp_d), b.CTRINCUWY_D) AS numeric(5,0)) > 0 ) ) OR (cast(datediff(DAY, c.ctrinc_d, b.ctrexp_d) AS numeric(5,0)) < 0)  )
                                        then 0	
                                        
                                    when (a.ACCADMTYP_CT in (1,3,4,5) AND  (cast(datediff(DAY, c.ctrinc_d, dateadd(day, 1, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0)) != 0) AND 
                                          (cast(datediff(DAY, dbo.Max2dates(b.CTRINCUWY_D, c.ctrinc_d), dateadd(day, 1,dbo. Min2dates(ISNULL(c.ctrexp_d, c.scoexp_d), b.ctrexp_d)) ) AS numeric(5,0)) >= cast(datediff(DAY, c.ctrinc_d, dateadd(day, 1, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0)) ))              --01
                                        then 1                                         
                                         
                                    when (  a.ACCADMTYP_CT in (1,3,4,5) AND  (cast(datediff(DAY, c.ctrinc_d, dateadd(day, 1, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0)) != 0) 
                                            AND (cast(datediff(DAY, dbo.Max2dates(b.CTRINCUWY_D, c.ctrinc_d), dateadd(day, 1, dbo.Min2dates(ISNULL(c.ctrexp_d, c.scoexp_d), b.ctrexp_d)) ) AS numeric(5,0)) <= cast(datediff(DAY, c.ctrinc_d, dateadd(day, 1, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0)) ) 
                                            --AND (cast(datediff(DAY, dbo.Max2dates(b.CTRINCUWY_D,c.ctrinc_d), dateadd(day, 1, dbo.Min2dates(c.ctrexp_d, b.ctrexp_d)) ) AS numeric(5,0)) >= 0 ) 
                                          )
                                        then cast(datediff(DAY, dbo.Max2dates(b.CTRINCUWY_D, c.ctrinc_d), dateadd(day, 1, dbo.Min2dates(ISNULL(c.ctrexp_d, c.scoexp_d), b.ctrexp_d)) ) AS numeric(5,0)) / cast(datediff(DAY, c.ctrinc_d, dateadd(day, 1, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0))                --001
                                                                                                                 
                                        
                                    when ( ((a.ACCADMTYP_CT = 2) AND  (cast(datediff(DAY, c.ctrinc_d, dateadd(day, 366, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0)) != 0 )) AND 
                                          (cast(datediff(DAY, dbo.Max2dates(b.CTRINCUWY_D, c.ctrinc_d), dateadd(day, 1, dbo.Min2dates(dateadd(day, 365, ISNULL(c.ctrexp_d, c.scoexp_d)), b.ctrexp_d)) ) AS numeric(5,0)) >= cast(datediff(DAY, c.ctrinc_d, dateadd(day, 366, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0))) )  --001
                                        then 1                                             
                                    
                                    when ((a.ACCADMTYP_CT = 2) AND  (cast(datediff(DAY, c.ctrinc_d, dateadd(day, 366, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0)) != 0) 
                                          AND (cast(datediff(DAY, dbo.Max2dates(b.CTRINCUWY_D, c.ctrinc_d), dateadd(day, 1, dbo.Min2dates(dateadd(day, 365, ISNULL(c.ctrexp_d, c.scoexp_d)), b.ctrexp_d)) ) AS numeric(5,0)) <= cast(datediff(DAY, c.ctrinc_d, dateadd(day, 366, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0)) ) )
                                          --AND (cast(datediff(DAY, dbo.Max2dates(b.CTRINCUWY_D, c.ctrinc_d), dateadd(day, 1, dbo.Min2dates(dateadd(day, 365, c.ctrexp_d), b.ctrexp_d)) ) AS numeric(5,0)) >= 0 )
                                        then cast(datediff(DAY, dbo.Max2dates(b.CTRINCUWY_D, c.ctrinc_d), dateadd(day, 1, dbo.Min2dates(dateadd(day, 365, ISNULL(c.ctrexp_d, c.scoexp_d)), b.ctrexp_d)) ) AS numeric(5,0)) / cast(datediff(DAY, c.ctrinc_d, dateadd(day, 366, ISNULL(c.ctrexp_d, c.scoexp_d))) AS numeric(5,0))                --001
                                                                                                                                                          
                                    else 0 
                                end) = 1 then 1 else 0 end,     -- [006]
						convert(char(8), b.CTRINCUWY_D, 112) retinc_d -- Inception Retro date                                                                                       				                       
        --INTO #LORETFACTOR
        FROM
            bret..tcession a,
         ...
         WHERE
            ...
            ((CTRSTS_CT IN (16, 18, 19) and CTRLCK_B != 0) OR 
        ...
        RETURN 0
go
EXEC sp_procxmode 'dbo.PsLORETFACTOR_01', 'unchained'
go
IF OBJECT_ID('dbo.PsLORETFACTOR_01') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PsLORETFACTOR_01 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PsLORETFACTOR_01 >>>'
go
GRANT EXECUTE ON dbo.PsLORETFACTOR_01 TO GOMEGA
go
GRANT EXECUTE ON dbo.PsLORETFACTOR_01 TO GDBBATCH
go
