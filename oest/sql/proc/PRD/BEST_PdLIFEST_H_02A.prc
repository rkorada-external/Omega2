...
IF OBJECT_ID('dbo.PdLIFEST_H_02A') IS NOT NULL
BEGIN
  DROP PROCEDURE dbo.PdLIFEST_H_02A
  IF OBJECT_ID('dbo.PdLIFEST_H_02A') IS NOT NULL
    PRINT '<<< FAILED DROPPING PROCEDURE dbo.PdLIFEST_H_02A >>>'
  ELSE
    PRINT '<<< DROPPED PROCEDURE dbo.PdLIFEST_H_02A >>>'
END
GO

CREATE PROCEDURE dbo.PdLIFEST_H_02A
(
...
BEGIN
	WHILE @gaap < 6
	BEGIN
		PRINT 'copie de balshtmth %1! < balshtmth <= %2!, GAAP_NT=%3!', @balshmth_min, @balshmth_max, @gaap
		...
		WHERE   BALSHEY_NF = @bal_year 
		AND     BALSHTMTH_NF > @balshmth_min AND BALSHTMTH_NF <= @balshmth_max
		AND     SSD_CF >= @ssd_cf            AND SSD_CF       <= @ssdMax_cf 
		AND     GAAP_NT = @gaap
		...
		COMMIT TRAN
	END
	SELECT @balshmth_min = @balshmth_max, @balshmth_max = @balshmth_max + 3, @gaap = 1
END
...
EXEC sp_procxmode 'dbo.PdLIFEST_H_02A', 'unchained'
IF OBJECT_ID('dbo.PdLIFEST_H_02A') IS NOT NULL
  PRINT '<<< CREATED PROCEDURE dbo.PdLIFEST_H_02A >>>'
ELSE
  PRINT '<<< FAILED CREATING PROCEDURE dbo.PdLIFEST_H_02A >>>'
GO
GRANT EXECUTE ON dbo.PdLIFEST_H_02A TO GOMEGA
GO
GRANT EXECUTE ON dbo.PdLIFEST_H_02A TO GDBBATCH
GO
