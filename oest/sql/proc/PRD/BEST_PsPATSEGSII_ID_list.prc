use BEST
go
if object_id('dbo.PsPATSEGSII_ID_list') is not null
begin
  drop PROC dbo.PsPATSEGSII_ID_list
  print '<<< DROPPED PROC dbo.PsPATSEGSII_ID_list >>>'
end
go
create procedure PsPATSEGSII_ID_list
  (
  @p_PATCAT_CT char(5)
 ,@p_PATTYP_CT char(5)
 ,@p_SSD_CF    USSD_CF
...
go
if object_id('dbo.PsPATSEGSII_ID_list') is not null
  print '<<< CREATED PROC dbo.PsPATSEGSII_ID_list >>>'
else
  print '<<< FAILED CREATING PROC dbo.PsPATSEGSII_ID_list >>>'
go
grant execute on dbo.PsPATSEGSII_ID_list TO GOMEGA
go
