use BEST
go
if object_id('dbo.PiLIFEST_01') is not null
begin
  drop procedure dbo.PiLIFEST_01
  if object_id('dbo.PiLIFEST_01') is not null
    print '<<< FAILED DROPPING procedure dbo.PiLIFEST_01 >>>'
  else
    print '<<< DROPPED procedure dbo.PiLIFEST_01 >>>'
end
go
create procedure PiLIFEST_01
  (
...
2  M GEORGET            si on se trouve dans accepation alors autupd_b = 1 sinon si rétro alors autupd_b = 0
...
5  G BUISSON 24/05/2004 Génération automatique des libérations de CNA en Ex + 1 / Ac + 1 sur les contrats de type 3
6  G BUISSON 04/09/2006 :spot:12720 Le code MAJ AUTO des traités acceptation dépend de la rétro interne (0 si rétro interne, 1 sinon)
7  D OURMIAH 22/12/2008 :spot:16652 Plantage lors de saisie sur le poste 1073
8  Florent   27/11/2009 :spot:16973 si demande de libération ajoute 1 à poste cumul !
9  T.RIPERT  30/09/2010 :spot 18235 on ajoute une second si on a modifié soit le poste 1900 ou 1901
10 T.RIPERT	 23/11/2010 :spot 18235 origine = CALC pour le champ calculé SR
11 T.RIPERT	 11/02/2011 :spot ????? Libération du poste 1093/2093
12 Florent   08/09/2011 :spot:22315 gestion de la libération en EXE+1,Le type 1 a déjà eu sa libération créée par l'application, pour les autres il faut la générer
                        et il faut incrémenter l'exercice des type 1 s'ils font partie des postes concernés à l'insertion reçue de l'application
13 Florent   24/01/2012 :spot:20562 interdit la création de ligne si avant le bilan en cours
14 Florent   23/04/2012 :spot:23688 l'exercice=année de compte pour les type 1 et si pas libération en exe+1 alors exe=année de compte -1!
*****************************************************/
declare
  @erreur      int
 ,@tran_imbr   bit
...
select @date_jour=getdate(), @erreur=0, @tran_imbr=1, @ssd_cf=convert(int,substring(@p_ctr_nf,1,2))
if @@trancount = 0
begin
  select @tran_imbr = 0
  begin tran
...
begin
  if @p_acmtrs_nt%10=4
    -- l'exercice=année de compte pour les type 1 et si pas libération en exe+1 alors exe=année de compte -1!
...
-- Le type 1 et 4 ont déjà eu leur libération créée par l'application, pour les autres il faut la générer
if @lst_accadmtyp_ct not in(1,4) and @libe_exe_p1 > 0 AND (@p_acy_nf <= @p_balshey_nf + 2)
begin
...
-- S'il n'existe pas de TLIFDRI pour le TLIFEST créé, création
-- Remarque : dans tous les cas, exercice de souscription = année de compte
-- on positionne l'indicateur de rétro interne pour l'acceptation en fonction de la cédante du contrat
...
go
if object_id('dbo.PiLIFEST_01') is not null
  print '<<< CREATED procedure dbo.PiLIFEST_01 >>>'
else
  print '<<< FAILED CREATING procedure dbo.PiLIFEST_01 >>>'
go
grant execute on dbo.PiLIFEST_01 TO GOMEGA
go
