use BEST
go
if object_id('PsPeriTrtIni_02') is not null
begin
   drop procedure PsPeriTrtIni_02
   if object_id('PsPeriTrtIni_02') is not null
     print '<<< FAILED DROPPING procedure PsPeriTrtIni_02 >>>'
   else
     print '<<< DROPPED procedure PsPeriTrtIni_02 >>>'
end
go
create procedure PsPeriTrtIni_02
  (
		@p_clo_date char(8),
		@p_x_days int,
...
select a.CTR_NF, a.UWY_NF, a.UW_NT, a.END_NT, a.SEC_NF, a.ERNPRMADM_B, a.POLDURMTH_NF, a.INSPOL_R, a.URRCAL_R, a.POLED_D  --MODIF 006 
...
print 'Select BTRAV..TFAMRSVP : %1!' , @lignes
...
select @erreur = @@error, @lignes=@@rowcount
if @erreur != 0  goto fin
print 'Select BCLI..TCLIENT : %1!' , @lignes
...
print '@p_quarter_end = %1!' , @p_quarter_end
...
print '@month = 3, @p_next_clo_date = %1!' , @p_next_clo_date
END

IF (@month = 6)
BEGIN
SELECT @p_next_clo_date = CAST(@year*10000+(@month+3)*100+30  AS CHAR(8))
print '@month = 6, @p_next_clo_date = %1!' , @p_next_clo_date
END

IF (@month = 9)
BEGIN
SELECT @p_next_clo_date = CAST(@year*10000+(@month+3)*100+31  AS CHAR(8))
print '@month = 9, @p_next_clo_date = %1!' , @p_next_clo_date
END

IF (@month = 12)
BEGIN
SELECT @p_next_clo_date =CAST((@year+1)*10000+03*100+31  AS CHAR(8))
print '@month = 12, @p_next_clo_date = %1!' , @p_next_clo_date
END
...
go
if object_id('PsPeriTrtIni_02') is not null
  print '<<< CREATED procedure PsPeriTrtIni_02 >>>'
else
  print '<<< FAILED CREATING procedure PsPeriTrtIni_02 >>>'
go
grant execute on PsPeriTrtIni_02 TO GDBBATCH, GOMEGA
go
