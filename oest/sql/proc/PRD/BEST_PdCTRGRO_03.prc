use BEST
go
if object_id('dbo.PdCTRGRO_03') is not null
begin
  drop PROC dbo.PdCTRGRO_03
  print '<<< DROPPED PROC dbo.PdCTRGRO_03 >>>'
end
go
create procedure PdCTRGRO_03
  (
  @p_ssd_cf    USSD_CF
...
go
if object_id('dbo.PdCTRGRO_03') is not null
  print '<<< CREATED PROC dbo.PdCTRGRO_03 >>>'
else
  print '<<< FAILED CREATING PROC dbo.PdCTRGRO_03 >>>'
go
grant execute on dbo.PdCTRGRO_03 TO GOMEGA
go
