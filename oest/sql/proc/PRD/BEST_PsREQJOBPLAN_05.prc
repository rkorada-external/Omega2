USE BEST
go
IF OBJECT_ID('dbo.PsREQJOBPLAN_05') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.PsREQJOBPLAN_05
    IF OBJECT_ID('dbo.PsREQJOBPLAN_05') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.PsREQJOBPLAN_05 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.PsREQJOBPLAN_05 >>>'
END
go
create procedure dbo.PsREQJOBPLAN_05
  (
  @p_ssd_cf        varchar(2),
  @p_reqcod_ct     char(1),
  @p_clodat_d      datetime,
  ...
  @p_site_cf       varchar(10) = Null, -- [003] Site selected in GUI
  @p_vrs_nf integer
  )
as
/***************************************************
Programme               : PsREQJOBPLAN_05
Fichier script associé  : PsREQJOBPLAN_05.prc
Domaine                 : (ES) Estimation
Base principale         : BEST
Auteur                  : T. RIPERT
Date de creation        : 08/09/2010
Description du programme: Contrôle en Saisie des Demandes d'Inventaires Renvoie un Code Erreur TMESSAGE
...
DATE           :  09/11/2010
AUTEUR         :  T.RIPERT
DESCRIPTION    :  SPOT 19070 Optimisation Scheduler
...
DESCRIPTION    :  SPOT 19070 Optimisation Scheduler
[002] 06/03/2013 Amit Deshpande : We have modified the return values alias names for this procedure so as to map the results to our custom entity. Earlier result from this proc was without any column like 1,2,3   
[100] 30/09/2013 P. Pezout   :spot:25427  - Modifications pour omega2 -1b sur treqjob et treqjobplan
[003] 28/07/2014 C. Despret  :spot:26764  - New parameter : p_site_cf. The selected site in the GUI. 
_________________
MODIFICATION   :  [007]
SPOT           :  MODIF007
DATE           :  14/11/2014 (DD/MMM/YYYY)
AUTEUR         :  AbdulWaajed Shaikh
DESCRIPTION    :  Fix for SCOR defect #032518. SITE_CF added in where clause for particular error message condition ESTIMATION, 2037.
_________________
MODIFICATION   :  [008]
SPOT           :  MODIF008
DATE           :  02/06/2015 (DD/MMM/YYYY)
AUTEUR         :  Kevin Barnoin
DESCRIPTION    :  EST54 update to manage request type "R" manual insertion
_________________
MODIFICATION   :  [009]
SPOT           :  MODIF009
DATE           :  25/07/2016 (DD/MMM/YYYY)
AUTEUR         :  Avanti Pannala
DESCRIPTION    :  Fix for Spira defect #44796. Allowing users to launch  D request and T EBS request on the same date. 
_________________

MODIFICATION   :  [010]
SPOT           :  MODIF010
DATE           :  26/07/2016 (DD/MMM/YYYY)
AUTEUR         :  Avanti Pannala
DESCRIPTION    :  Fix for Spira defect #33208. Control made on Job already created 
_________________

MODIFICATION   :  [011]
SPOT           :  MODIF011
DATE           :  1/12/2016 (DD/MMM/YYYY)
AUTEUR         :  Riyadh ABDOULAMIDE
DESCRIPTION    :  Fix for Spira defect #33208. Control made on Job already created. Revert the MODIF011
_________________

MODIFICATION   :  [012]
SPOT           :  MODIF012
DATE           :  24/09/2017 (DD/MMM/YYYY)
AUTEUR         :  Riyadh ABDOULAMIDE
DESCRIPTION    :  Fix for Spira defect #33208. Control made on Job already created. Revert the MODIF011
_________________
...
MODIFICATION   :  [013]
SPOT           :  MODIF013
DATE           :  26/04/2019 (DD/MMM/YYYY)
AUTEUR         :  Alice CELIGNY
DESCRIPTION    :  Fix for Spira defect #70116. Allowing schedule an T-EBS when a C request is already scheduled

_________________

MODIFICATION   :  [014]
SPOT           :  MODIF014
DATE           :  31/03/2022 (DD/MMM/YYYY)
AUTEUR         :  Riyadh ABDOULAMIDE
DESCRIPTION    :  Spira 102967
...
declare
 ...
 ,@r_UPDUSR_CF    UUSR_CF
 ,@pi_ssd_cf      tinyint
 ,@r_Account_d    datetime
 ,@r_Specend_d    datetime
 ,@pos_SSD_CF     int 
 ,@Tmp_SSD_CF     varchar(20)
-- -------------------------------------------------------------------------------
...
select @p_date_t=getdate(), @pi_ssd_cf=convert(tinyint, @p_ssd_cf), @erreur=0, @r_REQCOD_CT=null

-- MOD01 : create TABLE #TLSTSSD
create TABLE #TLSTSSD (SSD_CF USSD_CF NOT null)

-- [003]
-- If SITE is unknown then a test is done to find the user's site
-- Can only be used for batch users : ubeu, ubas and ubam 
-- GUI procedure call has to set the p_site_cf parameter 
if @p_site_cf is null 
begin
  execute @erreur=BEST..PsSITE_01 @suser_Name,'0',@p_site_cf output
...
SELECT DISTINCT  @r_SSD_CF = SSD_CF,  @r_BALSHEYEA_NF = BALSHEYEA_NF, @r_BALSHTMTH_NF = BALSHTMTH_NF, @r_CLODAT_D = CLODAT_D,
		...
		AND convert(char(8),dbclo_d,112)      =  convert(char(8),@p_dbclo_d,112)
...
IF (@r_REQCOD_CT != null)
    BEGIN
        SELECT @v_codeerrmsg = 2016     --  2015 Il existe déjà une Demande de Stat/Reporting pour cette Filiale
        SELECT @v_MsgBloquant = 1         -- Oui
        GOTO fin
    END
-- MODIF010 end		  
	*/	  
-- 1. Vérifier s'il existe déjà une Demande d'inventaire pour cette Filiale
if (@p_reqcod_ct = 'L' or @p_reqcod_ct = 'I' or @p_reqcod_ct = 'J' or @p_reqcod_ct= 'R') -- [008]
    begin
        select DISTINCT  @r_SSD_CF = SSD_CF,  @r_BALSHEYEA_NF = BALSHEYEA_NF, @r_BALSHTMTH_NF = BALSHTMTH_NF, @r_CLODAT_D = CLODAT_D,
               @r_REQCOD_CT = REQCOD_CT, @r_CRE_D = CRE_D, @r_DBCLO_D = DBCLO_D, @r_LAUNCH_D = LAUNCH_D, @r_CLOPER_LS = CLOPER_LS,
               @r_VRS_NF = VRS_NF,  @r_UPDUSR_CF = UPDUSR_CF
        from   BEST..TREQJOBPLAN
        where  ( (reqcod_ct IN('I','J') and ssd_cf=@pi_ssd_cf) or 
                 (reqcod_ct IN('D','E') and ','+cloper_ls+',' like '%[,]'+@p_ssd_cf+'[,]%') -- [002]
               )
          and  launch_d    =  null
          and  cre_d       <> @p_cre_d
          and  convert(char(8),dbclo_d,112)  =  convert(char(8),@p_dbclo_d,112)
...
if (@p_reqcod_ct = 'I') or (@p_reqcod_ct = 'J')
    begin
        ...
        where  reqcod_ct IN ('L')
         and   launch_d    = null
          and (ssd_cf=@pi_ssd_cf or ','+cloper_ls+',' like '%[,]'+@p_ssd_cf+'[,]%')
          and  convert(char(8),dbclo_d,112)  =  convert(char(8),@p_dbclo_d,112)
    end
if (@r_REQCOD_CT != null)
    begin
...
if (@p_reqcod_ct = 'D' or @p_reqcod_ct = 'E')  -- [002]
    begin
...
if (@p_reqcod_ct = 'J') or (@p_reqcod_ct = 'L') or (@p_reqcod_ct = 'R') or (@p_reqcod_ct = 'E')  -- [002] -- [008]
   begin -- B01
    ...
    if (@p_reqcod_ct = 'D') 
        ...
        if (@r_REQCOD_CT != null)
            begin
                select @v_codeerrmsg = 2036    --  2037 Demande PostOmega Conso/Social impossible. ~r~nUne Autre demande d'Inventaire est déjà en Attente !
                select @v_MsgBloquant = 1         -- Oui
                goto fin
            end
		end
            
        if (@p_reqcod_ct = 'T' AND @p_vrs_nf=0) 
          begin
          select DISTINCT  @r_SSD_CF = SSD_CF,  @r_BALSHEYEA_NF = BALSHEYEA_NF, @r_BALSHTMTH_NF = BALSHTMTH_NF, @r_CLODAT_D = CLODAT_D,
           @r_REQCOD_CT = REQCOD_CT, @r_CRE_D = CRE_D, @r_DBCLO_D = DBCLO_D, @r_LAUNCH_D = LAUNCH_D, @r_CLOPER_LS = CLOPER_LS,
           @r_VRS_NF = VRS_NF,  @r_UPDUSR_CF = UPDUSR_CF
...
if (@p_reqcod_ct = 'L') or (@p_reqcod_ct = 'I') or (@p_reqcod_ct = 'J')
    begin
        ...
        if EXISTS (select 1 from BEST..TREQJOBPLAN where reqcod_ct IN ('D','E') and launch_d = null)  -- [002]
        begin
...
-- 4. Si Demande Inventaire en Comptabilisation Mensuelle pour les demandes différentes de V et Z
if @p_reqcod_ct <> 'V' and @p_reqcod_ct <> 'Z'
begin
   select   @A_traiter=count(*)
   from     BREF..TCALEND
   where    closing_b = 0
     and    convert(char(10), account_d , 112) = convert(char(10), @p_dbclo_d, 112)
...
if (@p_reqcod_ct = 'I') or (@p_reqcod_ct = 'J')
    begin
        ...
        where  convert(char(10), clodat_d, 112) = convert(char(10), @p_clodat_d, 112)
          and  convert(char(10), dbclo_d, 112) = convert(char(10), @p_dbclo_d, 112)
    ...
    begin
        ...
        where  convert(char(10), clodat_d, 112) = convert(char(10), @p_clodat_d, 112)
          ...
          and  ((ssd_cf = @pi_ssd_cf and REQCOD_CT IN ('I', 'J'))
               or (CLOPER_LS like (@VDSR) and REQCOD_CT = 'D')
               )
...
if (@p_reqcod_ct = 'C') or (@p_reqcod_ct = 'I')  or (@p_reqcod_ct = 'J')  or (@p_reqcod_ct = 'L')  or (@p_reqcod_ct = 'E')  -- [002]
    begin
        select DISTINCT  @r_SSD_CF = SSD_CF,  @r_BALSHEYEA_NF = BALSHEYEA_NF, @r_BALSHTMTH_NF = BALSHTMTH_NF, @r_CLODAT_D = CLODAT_D,
           @r_REQCOD_CT = REQCOD_CT, @r_CRE_D = CRE_D, @r_DBCLO_D = DBCLO_D, @r_LAUNCH_D = LAUNCH_D, @r_CLOPER_LS = CLOPER_LS,
           @r_VRS_NF = VRS_NF,  @r_UPDUSR_CF = UPDUSR_CF
...
-- MODIF013
		if (@p_vrs_nf = 1) and (@p_reqcod_ct = 'T')
		begin
		select DISTINCT  @r_SSD_CF = SSD_CF,  @r_BALSHEYEA_NF = BALSHEYEA_NF, @r_BALSHTMTH_NF = BALSHTMTH_NF, @r_CLODAT_D = CLODAT_D,
           @r_REQCOD_CT = REQCOD_CT, @r_CRE_D = CRE_D, @r_DBCLO_D = DBCLO_D, @r_LAUNCH_D = LAUNCH_D, @r_CLOPER_LS = CLOPER_LS,
           @r_VRS_NF = VRS_NF,  @r_UPDUSR_CF = UPDUSR_CF
        from BEST..TREQJOBPLAN
        where  launch_d is null
          and  reqcod_ct IN ('I', 'J', 'L', 'E', 'T', 'F')  -- [002]
          and  convert(char(8),dbclo_d,112)      =  convert(char(8),@p_dbclo_d,112)
		  and  site_cf = @p_site_cf --MODIF007
				end
		else
		begin
-- end MODIF013
        ...
        from BEST..TREQJOBPLAN
        where  launch_d is null
          and  reqcod_ct IN ('C', 'I', 'J', 'L', 'E', 'T', 'F')  -- [002]
          and  convert(char(8),dbclo_d,112)      =  convert(char(8),@p_dbclo_d,112)
		  and  site_cf = @p_site_cf --MODIF007
        end
...
begin
        ...
        select DISTINCT  @r_SSD_CF = SSD_CF,  @r_BALSHEYEA_NF = BALSHEYEA_NF, @r_BALSHTMTH_NF = BALSHTMTH_NF, @r_CLODAT_D = CLODAT_D,
           @r_REQCOD_CT = REQCOD_CT, @r_CRE_D = CRE_D, @r_DBCLO_D = DBCLO_D, @r_LAUNCH_D = LAUNCH_D, @r_CLOPER_LS = CLOPER_LS,
           @r_VRS_NF = VRS_NF,  @r_UPDUSR_CF = UPDUSR_CF
        from BEST..TI17REQJOBPLAN --0014
        where  launch_d is null
          and  reqcod_ct IN ('Y')  -- [002]
          and  convert(char(8),dbclo_d,112)      =  convert(char(8),@p_dbclo_d,112)
		  and  site_cf = @p_site_cf --MODIF007
...
fin:
-- Added aliases
select @v_codeerrmsg as v_codeerrmsg, @v_MsgBloquant as v_MsgBloquant, @r_SSD_CF as r_SSD_CF, @r_BALSHEYEA_NF as r_BALSHEYEA_NF, @r_BALSHTMTH_NF as r_BALSHTMTH_NF, 
     convert(char(10), @r_CLODAT_D, 112) as r_CLODAT_D, @r_REQCOD_CT as r_REQCOD_CT,
       convert(char(10), @r_CRE_D, 112) as r_CRE_D, convert(char(10), @r_DBCLO_D, 112) as r_DBCLO_D,
       convert(char(10), @r_LAUNCH_D, 112) as r_LAUNCH_D, @r_CLOPER_LS as r_CLOPER_LS, @r_VRS_NF as r_VRS_NF, @r_UPDUSR_CF as r_UPDUSR_CF

return 0
go
EXEC sp_procxmode 'dbo.PsREQJOBPLAN_05', 'unchained'
go
IF OBJECT_ID('dbo.PsREQJOBPLAN_05') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PsREQJOBPLAN_05 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PsREQJOBPLAN_05 >>>'
go
GRANT EXECUTE ON dbo.PsREQJOBPLAN_05 TO public
go
