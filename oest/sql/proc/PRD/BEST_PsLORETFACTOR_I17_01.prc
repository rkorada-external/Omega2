USE BEST
go
IF OBJECT_ID('dbo.PsLORETFACTOR_I17_01') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.PsLORETFACTOR_I17_01
    IF OBJECT_ID('dbo.PsLORETFACTOR_I17_01') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.PsLORETFACTOR_I17_01 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.PsLORETFACTOR_I17_01 >>>'
END
go
 ...
 */
CREATE PROCEDURE dbo.PsLORETFACTOR_I17_01
(
...
Programme:              PsLORETFACTOR_I17_01
Fichier script associé: PsLORETFACTOR_I17_01.PRC
...
[019] 17/10/2025 MZM   US 7046 : Cut off management - actuarial segment is empty on contracts taken into account the day of cut off
*****************************************************/
...
BEGIN
  ...
  SELECT @v_pos_booking_minus_days = dateadd(day, 1, dateadd(day, @p_x_days * -1, @v_pos_booking_d) ) -- [019]
END
ELSE 
BEGIN
  SELECT @v_pos_booking_minus_days = dateadd(day, 1, convert(datetime, @p_quarter_end, 103) ) --[019]
END
...
SELECT distinct
        ...
        RETURN 0
go
EXEC sp_procxmode 'dbo.PsLORETFACTOR_I17_01', 'unchained'
go
IF OBJECT_ID('dbo.PsLORETFACTOR_I17_01') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PsLORETFACTOR_I17_01 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PsLORETFACTOR_I17_01 >>>'
go
GRANT EXECUTE ON dbo.PsLORETFACTOR_I17_01 TO GOMEGA
go
GRANT EXECUTE ON dbo.PsLORETFACTOR_I17_01 TO GDBBATCH
go
