use BEST
go
if object_id('dbo.PuACCSUP_01') is not null
begin
  drop PROC dbo.PuACCSUP_01
  print '<<< DROPPED PROC dbo.PuACCSUP_01 >>>'
end
go
create procedure PuACCSUP_01
  (
  @p_trn_nt          numeric
 ,@p_acctrn_nt       numeric
 ,@p_acctyp_nf       tinyint
...
go
if object_id('dbo.PuACCSUP_01') is not null
  print '<<< CREATED PROC dbo.PuACCSUP_01 >>>'
else
  print '<<< FAILED CREATING PROC dbo.PuACCSUP_01 >>>'
go
grant execute on dbo.PuACCSUP_01 TO GOMEGA
go
