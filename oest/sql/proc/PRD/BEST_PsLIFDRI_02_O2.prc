USE BEST
go
IF OBJECT_ID('dbo.PsLIFDRI_02_O2') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.PsLIFDRI_02_O2
    IF OBJECT_ID('dbo.PsLIFDRI_02_O2') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.PsLIFDRI_02_O2 >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.PsLIFDRI_02_O2 >>>'
END
go
/*
...
Create Procedure dbo.PsLIFDRI_02_O2 (@p_end_nt        UEND_NT,
                              @p_sec_nf        USEC_NF,
                              @p_uw_nt         UUW_NT,
                              @p_uwy_nf        UUWY_NF,
	                          @p_visu_mois     tinyint,
...
Fichier script associï¿½      : BEST_PsLIFDRI_02_O2.prc
...
Description du programme    : Sï¿½lection d'enregistrement dans TLIFEST : ESTIMATIONS VIE: 
                              Correspond ï¿½ type comptable <> 1 et <> 4 et ï¿½ un exercice donnï¿½.
...
Date                        : 27/02/2014
Version                     : 
Description                 : As per SGLA06 evo card 9 accounting years will displayed
...
Date                        : 25/04/2014
Version                     : 
Description                 : Added RESPROPAG_B  for EST 22 Card
...
/*--------------------------------------------------*/
/* Selection dans TLIFDRI : on ne prend poar annï¿½e  */ 
        ...
        Else
            Begin
                ...
                If Not Exists (Select Null 
                               From   #liste
			                   Where ACY_NF = @acy_nf_8)
                    Insert Into #liste Values(@acy_nf_8, 1, 0, 0, @conretctr_b,0,0) -- Modif 5 - SGLA06 change for 9 years -- Modif 4 - OMEGA2 Correction of OMEGA1 defect consistency -- MOD7 - Changed default value of AUTUPD_B boolean
                    
                If Not Exists (Select Null 
                               From   #liste
			                   Where ACY_NF = @acy_nf_9)
                    Insert Into #liste Values(@acy_nf_9, 1, 0, 0, @conretctr_b,0,0) -- Modif 5 -  Modif 5 - SGLA06 change for 9 years -- MOD7 - Changed default value of AUTUPD_B boolean
...
Return 0
go
EXEC sp_procxmode 'dbo.PsLIFDRI_02_O2', 'unchained'
go
IF OBJECT_ID('dbo.PsLIFDRI_02_O2') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.PsLIFDRI_02_O2 >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.PsLIFDRI_02_O2 >>>'
go
GRANT EXECUTE ON dbo.PsLIFDRI_02_O2 TO GOMEGA
go
GRANT EXECUTE ON dbo.PsLIFDRI_02_O2 TO GDBBATCH
go
