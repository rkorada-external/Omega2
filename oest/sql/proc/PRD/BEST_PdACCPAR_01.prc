...
DROP PROC dbo.PdACCPAR_01
go

IF OBJECT_ID('dbo.PdACCPAR_01') IS NOT NULL
   BEGIN
   DROP PROC dbo.PdACCPAR_01
   PRINT '<<< DROPPED PROC dbo.PdACCPAR_01 >>>'
END
go
...
Programme: PdACCPAR_01

Fichier script associé : ESDACC01.PRC
...
Description du programme: 
      ...
      suppression d'enregistrement dans TACCPAR
...
exec sp_SCOR_INSPRC 'ESDACC01', 'PdACCPAR_01', 'BEST', 'ANB'
go

IF OBJECT_ID('dbo.PdACCPAR_01') IS NOT NULL
   PRINT '<<< CREATED PROC dbo.PdACCPAR_01 >>>'
ELSE
   PRINT '<<< FAILED CREATING PROC dbo.PdACCPAR_01 >>>'
go
/*
 * Granting/Revoking Permissions on dbo.PdACCPAR_01
 */
GRANT EXECUTE ON dbo.PdACCPAR_01 TO GOMEGA
go
