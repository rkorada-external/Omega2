...
if object_id('PsOmegaExtract_TRT') is not null
	begin
		drop procedure PsOmegaExtract_TRT
		if object_id('PsOmegaExtract_TRT') is not null
			print '<<< FAILED DROPPING procedure PsOmegaExtract_TRT >>>'
		else
			print '<<< DROPPED procedure PsOmegaExtract_TRT >>>'
	end
go

create procedure PsOmegaExtract_TRT
  (
   @p_date     varchar(8),
			@norme_cf  char(4), --[002]
...
Description du programme:
    feed #OMEGA2EXTRACT_tmp with treaty omega extract. #OMEGA2EXTRACT_tmp is a temporary table generated in the calling stored procedure BEST_PsOmega2Extract.prc
Parametres:
	@p_date     varchar(8)
 @p_erreur 		 varchar(64)=null output
modifications:
	...
	[003] Spira 90339:  Transition: First closing scope Mylti-Year definition
*****************************************************/
...
--[003] suppression of WHERE clause c.MULTUWY_NF IS NULL
IF(@norme_cf = 'I17G')
BEGIN
 ...
 WHERE ((a.trncod_cf LIKE ''11%'') OR (a.trncod_cf LIKE ''14%''))
...
INNER JOIN BTRT..TSECIFRS b ON a.CTR_NF = b.CTR_NF AND a.UWY_NF = b.UWY_NF AND a.UW_NT = b.UW_NT AND a.END_NT = b.END_NT
...
INNER JOIN BTRT..TCONTR c ON a.CTR_NF = c.CTR_NF AND a.uwy_nf = c.uwy_nf AND a.uw_nt = c.uw_nt AND a.END_NT = c.END_NT
INNER JOIN BTRT..TSECTION b ON a.CTR_NF = b.CTR_NF AND a.SEC_NF = b.SEC_NF AND a.UWY_NF = b.UWY_NF AND a.UW_NT = b.UW_NT AND a.END_NT = b.END_NT
...
WHERE b.SECSTS_CT IN (14, 16, 17, 18, 19) --[003] suppression of WHERE clause c.MULTUWY_NF IS NULL
...
if object_id('PsOmegaExtract_TRT') is not null
	print '<<< CREATED PROC PsOmegaExtract_TRT >>>'
else
	print '<<< FAILED CREATING PROC PsOmegaExtract_TRT >>>'
go

grant execute on PsOmegaExtract_TRT TO GOMEGA
go

grant execute on PsOmegaExtract_TRT TO GDBBATCH
go