...
select 						
			...
			, null as GT_AMT_M          
			, null as GT_CED_NF         
			, ta.BRK_NF as GT_BRK_NF         
			, null as GT_PAY_NF         
			, null as GT_KEY_NF         
			, ta.RETCTR_NF as GT_RETCTR_NF    
			, null as GT_RETEND_NT      
			, ta.RETSEC_NF      
			, ta.RTY_NF         
			, null as GT_RETUW_NT       
			...
			, null as GT_RCL_NF         
			, ta.CUR_CF as GT_RETCUR_CF       
			, ta.TRN_M  as GT_RETAMT_M       
			, ta.PLC_NT as GT_PLC_NT       
			, ta.RTO_NF as GT_RTO_NF        
			, null as GT_INT_NF         
     ...
     bsar..tboprslnk tk, 
     bret..tracctrn ta
where tk.ACMTRSL3_NT = 1010 
and   tk.DETTRS_CF = ta.trncod_cf
and   tt.dettrs_cf = tk.dettrs_cf
and   tt.PRS_CF = 751
and  	substring(ta.trncod_cf,1,2) in ('2A', '2E', '21', '24')
and   ta.LOB_CF<>'30' and ta.LOB_CF<>'31'
and   ta.BLCSHT_D <= @p_ICLODAT_D
--and   Datepart(Year, @p_ICLODAT_D) = Datepart(Year, ta.BLCSHT_D)
--and   Datepart(month, @p_ICLODAT_D) >= Datepart(month,  ta.BLCSHT_D) 
--order by RETCTR_NF, RTY_NF, RETSEC_NF, SSD_CF   desc 
UNION
select 						
			ta.SSD_CF as GT_SSD_CF  				
			, ta.ESB_CF as GT_ESB_CF         
...
FROM bref..TTRSLNK tt,
     bsar..tboprslnk tk, 
     bret..tracctrn ta
where tk.DETTRS_CF = ta.trncod_cf
and   tt.dettrs_cf = tk.dettrs_cf
and   tt.PRS_CF = 750
and 	tk.ACMTRSL2_NT = 103  
and 	tk.TRNTYP_CT < 100 
and  	substring(ta.trncod_cf,1,2) in ('2A', '2E', '21', '24') 
and   Datepart(Year, @p_ICLODAT_D) = Datepart(Year, ta.BLCSHT_D)
and   Datepart(month, @p_ICLODAT_D) >= Datepart(month,  ta.BLCSHT_D)
and   ta.LOB_CF<>'30' and ta.LOB_CF<>'31'
order by RETCTR_NF, RETSEC_NF,  RTY_NF,  SSD_CF   desc
end
