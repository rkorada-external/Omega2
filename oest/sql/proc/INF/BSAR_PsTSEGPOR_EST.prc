use BSAR
go
if object_id('dbo.PsTSEGPOR_EST') is not null
begin
  drop PROC dbo.PsTSEGPOR_EST
  print '<<< DROPPED PROC dbo.PsTSEGPOR_EST >>>'
end
go
create procedure PsTSEGPOR_EST
  (
  @P_SSD_CF    USSD_CF
...
begin --Dommage
  ...
  ,CTRNAT_CT=case when c.CTRTYP_CT=2 then 'F' when c.CTRTYP_CT=1 and s.NAT_CF < '30' then 'P' else 'N' end
  ,CTRRET_B=isnull((select case when i.CLISSD_CF=null then 0 else 1 end from BCLI..TCLIENT i where i.CLI_NF=(select max(y.CED_NF) from BMIS..TCONTR y where y.CTR_NF=c.CTR_NF
   ...
   from BMIS..TSECTION s, BMIS..TCONTR c
...
go
if object_id('dbo.PsTSEGPOR_EST') is not null
  print '<<< CREATED PROC dbo.PsTSEGPOR_EST >>>'
else
  print '<<< FAILED CREATING PROC dbo.PsTSEGPOR_EST >>>'
go
grant execute on dbo.PsTSEGPOR_EST TO GOMEGA, GDBBATCH
go
