...
where exists ( select * from best..tlifest lif2, bret..tretsec sec 
        where lif1.ctr_nf = sec.retctr_nf
and lif1.uwy_nf = sec.rty_nf
and lif1.sec_nf = sec.retsec_nf
and sec.lob_cf = '30'
...
and lif1.ACMTRS_NT in (2503,2504)
)
go
...
begin
        ...
        update best..tlifest
        set acmtrs_nt = case when dettrncod_cf = '41000' then 2503
                             when dettrncod_cf = '41100' then 2504
                        else
                             lif.acmtrs_nt
                        end
        from best..tlifest lif, bret..tretsec sec
        where lif.ctr_nf = sec.retctr_nf
        and lif.uwy_nf = sec.rty_nf
        and lif.sec_nf = sec.retsec_nf
        and sec.lob_cf = '30'
        and lif.dettrncod_cf in ('41000','41100')
        and lif.acmtrs_nt in (2063,2064)
