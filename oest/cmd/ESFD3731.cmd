...
#     [001]      07/03/2019    Linh DOAN       77663      Upload TTECLEDSII IFRS17#       
#     [002]      21/01/2020    Linh DOAN       79100      Manage retro dummy contracts in closing    
#     [003]      02/06/2020    Linh DOAN       79070      Remove 3500 Retro One Day Gain, 3320 Initial CSM dans GTSII
#     [004]      28/09/2020    Linh DOAN       79100      Merge ESF_GTSII_DUMMY_STD into GTSII_STD
#     [005]      20/01/2021    Linh DOAN       91531 	  Fix param
#     [006]      15/03/2021    JYP             94170 	  add NORME for I17G/P/L Inception
#     [007]      26/04/2021    Linh DOAN       85522      reactive copy Onerous/dummy
#     [008]      03/05/2021    Linh DOAN       85522      move copy Onerous/dummy to 3630
#     [009]      29/03/2022    MZM             85522      Activation ONEROUS et DUMMY et Desactivation MAPPING Norms L et P pour eviter les doublons et Filtre 
#     [010]      30/03/2022    MZM             103397 Error in the LKI INI & DSI amounts : - Copy NDC STD File (Revert)
#====================================================================================================
#set -x
...
ECHO_LOG "#===> CRE_D................................: ${CRE_D} "                               >>$FLOG

ECHO_LOG "#===> ESF_GTSII_IFRS17_CSM.................: ${ESF_GTSII_IFRS17_CSM} "                >>$FLOG
ECHO_LOG "#===> ESF_GTSII_CSM_CASHFLOW...............: ${ESF_GTSII_CSM_CASHFLOW} "              >>$FLOG
ECHO_LOG "#===> ESF_GTSII_ONE_STD....................: ${ESF_GTSII_ONE_STD} "                   >>$FLOG
ECHO_LOG "#===> ESF_GTSII_DUMMY_STD..................: ${ESF_GTSII_DUMMY_STD} "                 >>$FLOG
ECHO_LOG "#===> ESF_GTSII_NDC_STD....................: ${ESF_GTSII_NDC_STD} "                   >>$FLOG
...
if [ ! -f ${ESF_GTSII_CASHFLOW_INI} ]
then
        ECHO_LOG "ESF_GTSII_CASHFLOW_INI=${ESF_GTSII_CASHFLOW_INI}  does not exist, take an empty file"            >> $FLOG
        EXECKSH "touch ${ESF_GTSII_CASHFLOW_INI}"

fi

if [ ! -f ${ESF_GTSII_CASHFLOW_STD} ]
then
        ECHO_LOG "ESF_GTSII_CASHFLOW_STD=${ESF_GTSII_CASHFLOW_STD}  does not exist, take an empty file"            >> $FLOG
        EXECKSH "touch ${ESF_GTSII_CASHFLOW_STD}"

fi

if [ ! -f ${ESF_GTSII_ALL_INI} ]
then
        ECHO_LOG "ESF_GTSII_ALL_INI=${ESF_GTSII_ALL_INI}  does not exist, take an empty file"            >> $FLOG
        EXECKSH "touch ${ESF_GTSII_ALL_INI}"

fi

if [ ! -f ${ESF_GTSII_ALL_STD} ]
then
        ECHO_LOG "ESF_GTSII_ALL_STD=${ESF_GTSII_ALL_STD}  does not exist, take an empty file"            >> $FLOG
        EXECKSH "touch ${ESF_GTSII_ALL_STD}"
fi

if [ ! -f ${ESF_GTSII_DUMMY_STD} ]
then
        ECHO_LOG "ESF_GTSII_DUMMY_STD=${ESF_GTSII_DUMMY_STD}  does not exist, take an empty file"            >> $FLOG
...
then
        ECHO_LOG "ESF_GTSII_ONE_STD=${ESF_GTSII_ONE_STD}  does not exist, take an empty file"            >> $FLOG
        EXECKSH "touch ${ESF_GTSII_ONE_STD}"

fi 

if [ ! -f ${ESF_GTSII_NDC_STD} ]
then
        ECHO_LOG "ESF_GTSII_NDC_STD=${ESF_GTSII_NDC_STD}  does not exist, take an empty file"            >> $FLOG
        EXECKSH "touch ${ESF_GTSII_NDC_STD}"
...
NSTEP=${NJOB}_20
#------------------------------------------------------------------------------------
LIBEL="Remove CSM LKI grouping 3500 from GTSII at INI"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${ESF_GTSII_ALL_INI} 2000 1"
SORT_O="${DFILT}/${NSTEP}_${IB}_SORT_GTSII_ALL_INI_O.dat 2000 1"
...
/KEYS   CTR_NF,
        ...
        UWY_NF,
        UW_NT
/CONDITION CSM_LKI (PATCAT_CT3 = "CSM" and  PATTYP_CT3="LKI" and ACMTRS3_NT = "3500" )
/OUTFILE ${SORT_O}
/OMIT CSM_LKI
exit
EOF
...
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_20_${IB}_SORT_GTSII_ALL_INI_O.dat 2000 1"
SORT_O="${DFILT}/${NSTEP}_${IB}_SORT_GTSII_FTECLEDSII_INI.dat 2000 1"
...
/CONDITION COND_ALLNO NORME_CF = "ALLNO" AND ( PATTYP_CT = "RMNTP" OR PATTYP_CT = "INF" )
/CONDITION COND_NOTALLNO NORME_CF != "ALLNO" AND ( PATCAT_CT3 = "CSF" OR PATCAT_CT3 = "ICR" )
...
/DERIVEDFIELD COMMENT_NEW  if COND_ALLNO    then "${NORME_CF}" else COMMENT
/DERIVEDFIELD NORME_CF_NEW if COND_NOTALLNO then "${NORME_CF}" else NORME_CF
/DERIVEDFIELD CLODAT_D "${CLODAT_D}~"
/DERIVEDFIELD CLOTYP_CT "INI~"
...
#[009]SORT_I2="${ESF_GTSII_DUMMY_STD} 2000 1"
#[009]SORT_I3="${ESF_GTSII_ONE_STD} 2000 1"
# [010] Ajout du fichier NDC_STD  Reverte


NSTEP=${NJOB}_27

LIBEL="Filter I17 ONEROUS DUMMY BEFORE COPY TO : STD"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${ESF_GTSII_DUMMY_STD} 2000 1"
SORT_I2="${ESF_GTSII_ONE_STD} 2000 1"
#SORT_I3="${ESF_GTSII_NDC_STD} 2000 1"
SORT_O="${DFILT}/${NSTEP}_${IB}_SORT_GTSII_ONE_DUMMY.dat 2000 1"
INPUT_TEXT ${SORT_CMD} << EOF
/FIELDS 
        SSD_CF            1:1 -  1:EN,
        ESB_CF            2:1 -  2:EN,
...
/CONDITION ONLY_I17_CSF ( NORME_CF != "") 
/OUTFILE ${SORT_O}
/OMIT ONLY_I17_CSF
...
LIBEL="Merge IFRS17 cashflow and discount files : STD"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${ESF_GTSII_ALL_STD} 2000 1"
SORT_I2="${DFILT}/${NJOB}_27_${IB}_SORT_GTSII_ONE_DUMMY.dat 2000 1"

SORT_O="${DFILT}/${NSTEP}_${IB}_SORT_GTSII_FTECLEDSII_STD.dat 2000 1"
...
NSTEP=${NJOB}_35
#------------------------------------------------------------------------------------
LIBEL="Merge INI and STD"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`

SORT_I="${DFILT}/${NJOB}_25_${IB}_SORT_GTSII_FTECLEDSII_INI.dat 2000 1"
SORT_I2="${DFILT}/${NJOB}_30_${IB}_SORT_GTSII_FTECLEDSII_STD.dat 2000 1"
