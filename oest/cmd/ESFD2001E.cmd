...
#[020] 09/04/2020 MZM          :spira:42212: TRAITES DECALES - Prise en compte de la Dernière date Comptable vi le fichier FDATDERCPA
#[021] 21/04/2020 Thomas/JYP   :spira 84317: bugfix sort 1000 , UAT KO
#[022] 25/06/2020 MZM          :spira:42212: TRAITES DECALES - Prise en compte de la Dernière date Comptable vi le fichier FDATDERCPA : MAJ de la cle de lointure
#[023] 03/03/2021 R. Cassis    :spira:92356: Auto-booking of Fac Premium - Ajout du programme ESTM7623
#[023] 03/03/2021 M.NAJI       :spira:92356: Ajout 1000 1 dans SORT_I
#[024] 22/11/2021 R. Cassis    :spira:100493: Ajout d'un tri non numérique sur fichier EST_FTFAMPRMDDUES pour s'aligner avec les autres en synchro
#========================================================================
...
SORT_O="${DFILT}/${NSTEP}_${IB}_ESTC1005_IADPERICASE_O2.dat 1000 1"
...
# Begin sort
#-----------------------------------------------------------------------------
LIBEL="Reformat ESTC1005_PERICASE Extended with DATDERCPA_D"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_90_${IB}_ESTC1005_IADPERICASE_O2.dat 1000 1"
SORT_O="${DFILT}/${NSTEP}_${IB}_ESTC1005_IADPERICASE_O2.dat 1000 1"
INPUT_TEXT $SORT_CMD <<EOF
/FIELDS 
	PER_CTR_NF 							3:1 - 3:,
	...
	ALL_F1 									1:1 - 218:,
	CTR_NF_F2    						1:1 - 1:,
	PER_DATDERCPA_D_F2  		2:1 - 2:
/JOINKEYS PER_CTR_NF 
/INFILE 	${EST_FDATDERCPA}  1000 1 "~"
/JOINKEYS CTR_NF_F2                  
/JOIN UNPAIRED LEFTSIDE                
/OUTFILE ${SORT_O}
/REFORMAT LEFTSIDE: ALL_F1, RIGHTSIDE: PER_DATDERCPA_D_F2
exit
EOF
SORT
...
# Begin sort
#-----------------------------------------------------------------------------
LIBEL="SORT ESTC1005_PERICASE Extended with DATEDERCPA"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_92_${IB}_ESTC1005_IADPERICASE_O2.dat 1000 1"
...
PRG=ESTM1010
export ${PRG}_I1=${EST_DSUMGTAASNEM_ESTC1005A}
...
#-----------------------------------------------------------------------------
LIBEL="Deletion of temporary files"
#RMFIL ${EST_DSUMGTAASNEM_ESTC1005A}
...
#[011] [020] Ajout Date Derniere Cpa dans le Pericase
#-----------------------------------------------------------------------------
...
#export ${PRG}_I1=${DFILT}/${NJOB}_90_${IB}_ESTC1005_IADPERICASE_O2.dat
export ${PRG}_I1=${DFILT}/${NJOB}_95_${IB}_SORT_IADPERICASE_O2.dat #[021]
export ${PRG}_I2=${EST_DSUMGTAA}
export ${PRG}_I3=${EST_IADPERIPRMD_CONV}
export ${PRG}_I4=${EST_CTRULT02}
export ${PRG}_I5=${EST_DLGTAAFPRE}
export ${PRG}_I6=${EST_DLGTAFACPNAE}
export ${PRG}_I7=${DFILT}/${NJOB}_140_${IB}_SORT_SAISPERICASE_O.dat
export ${PRG}_I8=${EST_FTHRHLDUWY}
export ${PRG}_O1=${DFILT}/${NSTEP}_${IB}_${PRG}_DLGTAAPRE.dat    #[023]
export ${PRG}_O2=${DFILT}/${NSTEP}_${IB}_${PRG}_FTTR_O2.dat
export ${PRG}_O3=${EST_FTFAC}
export ${PRG}_O4=${EST_NPSAIS}                                                 #[014]
export ${PRG}_O5=${DFILT}/${NSTEP}_${IB}_${PRG}_PNA_O5.dat
EXECPRG

#[023]
NSTEP=${NJOB}_151
#Tri du fichier EST_DTSTATGTAA sur la cle section pour postes 1110400
#-----------------------------------------------------------------------------
LIBEL="Tri du fichier EST_DTSTATGTAA sur la cle section pour postes 1110400 et 11104100..."
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${EST_DTSTATGTAA}   1000 1"
SORT_O=${DFILT}/${NSTEP}_${IB}_SORT_DTSTATGTAA_O.dat 
INPUT_TEXT ${SORT_CMD} <<EOF
...
/KEYS CTR_NF,
      ...
      BALSHRMTH_NF,
      BALSHRDAY_NF
/CONDITION trncod TRNCOD_CF = "11104000" OR TRNCOD_CF = "11104100" OR TRNCOD_CF = "11108000"
/INCLUDE trncod
exit
EOF
SORT
...
#[024]
NSTEP=${NJOB}_151b
#Tri du fichier EST_FTFAMPRMDDUES en non numerique pour s'accorder avec les autres fichiers...
#-----------------------------------------------------------------------------
LIBEL="Tri du fichier EST_FTFAMPRMDDUES en non numerique pour s'accorder avec les autres fichiers..."
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${EST_FTFAMPRMDDUES}   255 1"
SORT_O=${DFILT}/${NSTEP}_${IB}_SORT_FTFAMPRMDDUES_O.dat 
INPUT_TEXT ${SORT_CMD} <<EOF
/FIELDS CTR_NF      1:1 - 1:,
        END_NT      2:1 - 2:,
        SEC_NF      3:1 - 3:,
...
# Begin C program
#-----------------------------------------------------------------------------
LIBEL="Calcul of Fac Premium OverDue"
PRG=ESTM7623
FPRM=`CFTMP`
INPUT_TEXT ${FPRM} << EOF
...
EOF
export ${PRG}_PRM=${FPRM}
export ${PRG}_I1=${DFILT}/${NJOB}_150_${IB}_ESTM1007_DLGTAAPRE.dat
export ${PRG}_I2=${DFILT}/${NJOB}_95_${IB}_SORT_IADPERICASE_O2.dat
#export ${PRG}_I3=${EST_FTFAMPRMDDUES}  #[024]
export ${PRG}_I3=${DFILT}/${NJOB}_151b_${IB}_SORT_FTFAMPRMDDUES_O.dat
export ${PRG}_I4=${DFILT}/${NJOB}_151_${IB}_SORT_DTSTATGTAA_O.dat
export ${PRG}_I5=${EST_FCURQUOT}
export ${PRG}_O1=${DFILT}/${NSTEP}_${IB}_${PRG}_DLGTAAPRE.dat
EXECPRG

#[023]
NSTEP=${NJOB}_153
#Selection des postes autres que 11104002 pour les remettre dans DLGTAAPRE
#-----------------------------------------------------------------------------
LIBEL="Selection des postes autres que 11104002 pour les remettre dans DLGTAAPRE"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_150_${IB}_ESTM1007_DLGTAAPRE.dat 1000 1"
SORT_O=${DFILT}/${NSTEP}_${IB}_SORT_DLGTAAPRE_O.dat
INPUT_TEXT ${SORT_CMD} <<EOF
/FIELDS TRNCOD_CF   6:1 -  6:
/CONDITION trncod TRNCOD_CF != "11104002"
/INCLUDE trncod
exit
EOF
SORT
...
#[023]
NSTEP=${NJOB}_154
#Fusion des postes != 11104002 et ceux traités dans ESTM7623
#-----------------------------------------------------------------------------
LIBEL="Fusion des postes != 11104002 et ceux traités dans ESTM7623"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_152_${IB}_ESTM7623_DLGTAAPRE.dat  1000 1"
SORT_I2="${DFILT}/${NJOB}_153_${IB}_SORT_DLGTAAPRE_O.dat   1000 1"
SORT_O=${EST_DLGTAAPRE}
INPUT_TEXT ${SORT_CMD} <<EOF
/FIELDS BALSHEY_NF    3:1 -  3: EN,
        BALSHRMTH_NF  4:1 -  4: EN,
        BALSHRDAY_NF  5:1 -  5: EN,
...
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_160_${IB}_ESTM1008_FTTR_O1.dat 1000 1"
SORT_I2="${DFILT}/${NJOB}_150_${IB}_ESTM1007_FTTR_O2.dat 1000 1"
...
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_185_${IB}_SORT_IADPERICASE_O.dat 1000 1"
SORT_O="${DFILT}/${NSTEP}_${IB}_SORT_IADPERICASE_O.dat 1000 1"
...
PRG=ESTC1013
export ${PRG}_PRM=${FPRM}
export ${PRG}_I1=${DFILT}/${NJOB}_185_${IB}_SORT_IADPERICASE_NP_TYPE3_O.dat
export ${PRG}_I2=${DFILT}/${NJOB}_170_${IB}_SORT_FTTR_O.dat
...
#PRG=ESTC1013
#export ${PRG}_PRM=${FPRM}
#export ${PRG}_I1=${DFILT}/${NJOB}_204_${IB}_SORT_IADPERICASE_DECENNALE_O2.dat
#export ${PRG}_I2=${DFILT}/${NJOB}_170_${IB}_SORT_FTTR_O.dat
...
PRG=ESTC1027
export ${PRG}_I1=${DFILT}/${NJOB}_90_${IB}_ESTC1005_IADPERICASE_O2.dat
export ${PRG}_I2=${EST_DLGTAFACPNAE}
...
SORT_I3=${DFILT}/${NJOB}_150_${IB}_ESTM1007_PNA_O5.dat
...
#-----------------------------------------------------------------------------
LIBEL="TL Merge & Sort for facultatives & treaties UPR, PPE and PPW in progress"
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
...
NSTEP=${NJOB}_400
#-----------------------------------------------------------------------------
LIBEL="GTCUMUL file sort by Segment/UW Year/Occurence Year/CTR/END/SEC/UW in progress..."
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I=${EST_DLCGTAA}
...
NSTEP=${NJOB}_455
#-----------------------------------------------------------------------------
LIBEL="Temp IADPERIFR perimeter deletion ..."
