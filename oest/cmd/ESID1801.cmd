...
# nom du script SHELL		: ESID1801.cmd
...
#   01/ 06 / 04 J. Ribot ajout step 08 178 et 188 pour garder les enregistrements
#                        des filiales non presentes dans l'inventaire (SOPT 4935)
#-----------------------------------------------------------------------------
#   06/07/2012  JF VDV   [23390] - Amenagements SOLVENCY II  Ajout colonnes ORICOD_LS + 15 colonnes a vide
#[004] 24/07/2012 R. Cassis :spot:23802 Solvency correction sur les 15 champs
#[005] 05/10/2015 -=Dch=-  	:spot:29162 - Ajout du fichier périmètre dans l'appel de ESTC2303 (pour ajout CTR_CF et CTRNAT_CF) 
#[006] 12/04/2016 Florent :spot:29066 formatage du fichier GT
#[007] 18/03/2021  B.Lagha:spot:81531 Remplacer les noms des fichiers perm par des varibales step 010 et 02
#[008] 11/09/2023 JYP     :spira:110487 bugfix 69 cols instead of 71
#[009] 08/11/2023 MZM     :spira:110474 PROD6 AEs entries not found in SAP : Force RETUW_NT to 1 when it's not null
#[010] 24/11/2023 JYP/MZM/Florian :Spira:110901 add parameter Y_N for RET OVERRIDE exclude some TC when RAICOM_B=0  
#[011] 10/04/2024 JYP     SPIRA 110932 parameter A-AE for RET OVERRIDE exclude some TC when RAICOM_B=0  
#[012] 15/05/2024 JYP     SPIRA 110932 parameter A-AE for RET OVERRIDE exclude some TC when RAICOM_B=0 
#[013] 23/09/2024 MZM   SPIRA :112214 Force RETUW_NT to 1 when it's different to 1 (Complement :110474 ) 
#[014] 10/03/2025 MZM   SPIRA :112836 Q25 PRD - Program wrongly adds Retro order 1 when AE booked on Assumed only : "Force RETUW_NT to 1 when ( RETUW_NT != "1" )	AND  (RETCTR_NF != "")"
#===============================================================================
...
NSTEP=${NJOB}_010
#------------------------------------------------------------------------------
LIBEL="move EST_DLSGTAA ==> DFILT _WRK_DLSGTAA.dat"
EXECKSH "cp ${EST_DLSGTAA} ${DFILT}/${NSTEP}_${IB}_WRK_DLSGTAA.dat"


NSTEP=${NJOB}_011
#------------------------------------------------------------------------------
LIBEL="move EST_DLSGTAR ==> DFILT _WRK_DLSGTAR.dat"
EXECKSH "cp ${EST_DLSGTAR} ${DFILT}/${NSTEP}_${IB}_WRK_DLSGTAR.dat"

NSTEP=${NJOB}_012
#------------------------------------------------------------------------------
LIBEL="move EST_DLSGTR ==> DFILT _WRK_DLSGTR.dat"
EXECKSH  "cp ${EST_DLSGTR} ${DFILT}/${NSTEP}_${IB}_WRK_DLSGTR.dat"
...
######################################################
# Modif FGL du 18/10/2000
# BALSHEY Date  (YEAR/MTH_DAY ) remplacee par CLODAT
######################################################
...
#[013] #[014]  Force RETUW_NT TO 1 WHEN DIFFERENT TO ""
# Begin sort
#-----------------------------------------------------------------------------
...
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_50_${IB}_ESTC2304_GTART1_O1.dat 1000 1"
SORT_I2="${DFILT}/${NJOB}_50_${IB}_ESTC2304_GTART1MAJ_O2.dat 1000 1"
SORT_O="${DFILT}/${NSTEP}_${IB}_SORT_GTART1_O.dat 1000 1"
INPUT_TEXT $SORT_CMD <<EOF
/FIELDS SSD_CF 1:1 - 1:,
        ESB_CF 2:1 - 2:,
        BALSHEY_NF 3:1 - 3:,
...
SORT_WDIR=${SORTWORK}
SORT_CMD=`CFTMP`
SORT_I="${DFILT}/${NJOB}_60_${IB}_SORT_GTART1_O.dat 1000 1"
SORT_I2="${DFILT}/${NJOB}_100_${IB}_SORT_GTART2_O.dat 1000 1"
SORT_I3="${DFILT}/${NJOB}_140_${IB}_SORT_GTART3_O.dat 1000 1"
SORT_I4="${DFILT}/${NJOB}_160_${IB}_SORT_GTART4_O.dat 1000 1"
SORT_I5="${DFILT}/${NJOB}_170_${IB}_SORT_GTART5_O.dat 1000 1"
SORT_I6="${DFILT}/${NJOB}_178_${IB}_SORT_DLSGTAR_O.dat 1000 1"
SORT_O="${EST_DLSGTAR} 1000 1"
INPUT_TEXT $SORT_CMD <<EOF
/COPY
exit
EOF
