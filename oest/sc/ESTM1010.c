/*==============================================================================
Nom de l'application          : Nouveau cumuls pour les SNEMs
Nom du source                 : ESTM1010.c
Revision                      : $Revision: 1.2 $
Date de creation              : 30/07/1997
Auteur                        : CGI
APReferences des specifications :
Squelette de base             : batch
------------------------------------------------------------------------------
Description :
On recumule des postes cumuls sur les postes speciaux SNEMs.
postes cumuls originaux :	postes cumul SNEMs:	Signification:
(-20000)+ (-20030)		-20000			SP+SAP comptes complets
(+20000)+ (-20030)		+20000			SP comptes incomplets
------------------------------------------------------------------------------
Historique des modifications :
   <jj/mm/aaaa>   <auteur>    <description de la modification>
    27/03/2008   J. Ribot    SPOT 15219  ASE15 : recompilation des programmes C
	   ...           ...            ...              ...
==============================================================================*/

/*--------------------------------------------------*/
/* Inclusion des interfaces des composants importes */
/*--------------------------------------------------*/

#include <utctlib.h>
#include <stdarg.h>
#include "struct.h"


/*----------------------*/
/* Variables de travail */
/*----------------------*/

FILE 		   *Kp_OutputFile; /* Pointeur sur le fichier de sortie       */
T_RUPTURE_VAR  	   *pbd_Rupture; /* Pointeur sur la structure de la rupture   */


char Ksz_MessageErr[256]; /* Message d'erreur */
float 	Kd_SPSAPcomplets,
	Kd_SPincomplets,
	Kd_PNA;

/*-----------------------------*/
/* Fonctions du fichier maitre */
/*-----------------------------*/

int n_InitRupture	 (T_RUPTURE_VAR  *pbd_Rupture);
int n_TestRupture	 	(char *ptsz_LigneSuiv[], char *ptsz_LigneCour[]);
int n_ActionPremiereRupture	(char *ptsz_LigneCour[]);
int n_ActionDerniereRupture	(char *ptsz_LigneCour[]);
int n_ActionLigneRupture 	(char *ptsz_LigneCour[]);

/**************************************************************************/
/*** Objet : synchronisation entre le fichier perimetre annexe     	***/
/***         et le fichier IADPERICASE     				***/
/***									***/
/*** Nom : main		     						***/
/***									***/
/*** Parametres:							***/
/***	i argc : nombre de parametres					***/
/***	i argv : tableau de pointeurs sur les parametres		***/
/***									***/
/*** Retour:								***/
/***	OK si pas d'erreur,						***/
/***	ERR si erreur.							***/
/**************************************************************************/

int main(
   int argc,
   char *argv[]
)
{
   pbd_Rupture=malloc(sizeof(T_RUPTURE_VAR));

/* Initialisation des signaux */
   InitSig();

   if (n_BeginPgm(argc, argv) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_BeginPgm");
   }

/* Ouverture du fichier de sortie */
   if (n_OpenFileAppl("ESTM1010_O1", "wt", &Kp_OutputFile) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_OpenFileAppl");
   }

/* Initialisation de la structure de rupture */
   if (n_InitRupture(pbd_Rupture) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_InitRupture");
   }

/* Lancement du traitement du fichier maitre */
   if (n_ProcessingRuptureVar(pbd_Rupture) == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_ProcessingRuptureVar");
   }

   if (n_CloseFileAppl("ESTM1010_I1", &(pbd_Rupture->pf_InputFil)) == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTM1010_O1", &Kp_OutputFile) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_EndPgm() == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_EndPgm");
   }

   free(pbd_Rupture);

   exit(OK);
}


/**************************************************************************/
/*** Objet : initialisation de la structure de rupture			***/
/***									***/
/*** Nom : n_InitRupture     						***/
/***									***/
/*** Parametres:							***/
/***	i pbd_Rupture : pointeur sur la structure de rupture		***/
/***									***/
/*** Retour:								***/
/***	OK si pas d'erreur,						***/
/***	ERR si erreur.							***/
/**************************************************************************/

int n_InitRupture(
   T_RUPTURE_VAR *pbd_Rupture
)
{
   DEBUT_FCT("n_InitRupture");

   memset(pbd_Rupture, 0, sizeof(T_RUPTURE_VAR));

/* Ouverture du fichier maitre */
   if (n_OpenFileAppl("ESTM1010_I1", "rt", &(pbd_Rupture->pf_InputFil)) == ERR) {
      RETURN_VAL(ERR);
   }
   pbd_Rupture->n_NbRupture=1;
   pbd_Rupture->n_ActionFirst[0]=n_ActionPremiereRupture;
   pbd_Rupture->n_ActionLast[0]=n_ActionDerniereRupture;
   pbd_Rupture->n_ConditionRupture[0]=n_TestRupture;
   pbd_Rupture->n_ActionLigne=n_ActionLigneRupture;
   pbd_Rupture->c_Separ= '~';

   RETURN_VAL(OK);
}

/**************************************************************************/
/*** Objet : fonction de test de rupture				***/
/***									***/
/*** Nom : n_TestRupture     						***/
/***									***/
/*** Parametres:							***/
/***	i ptsz_LineSuiv : pointeur sur la ligne suivante,		***/
/***	i ptsz_LineCour : pointeur sur la ligne precedente.		***/
/***									***/
/*** Retour:								***/
/***	0 si pas de rupture,						***/
/***	1 si rupture.							***/
/**************************************************************************/

int n_TestRupture(
   char *ptsz_LigneSuiv[],
   char *ptsz_LigneCour[]
)
{
   static int n_Ret;

   if (n_Ret=strcmp(ptsz_LigneSuiv[GTE_CTR_NF], ptsz_LigneCour[GTE_CTR_NF])) {
      return n_Ret;
   }
   if (n_Ret=strcmp(ptsz_LigneSuiv[GTE_END_NT], ptsz_LigneCour[GTE_END_NT])) {
      return n_Ret;
   }
   if (n_Ret=strcmp(ptsz_LigneSuiv[GTE_SEC_NF], ptsz_LigneCour[GTE_SEC_NF])) {
      return n_Ret;
   }
   if (n_Ret=strcmp(ptsz_LigneSuiv[GTE_UWY_NF], ptsz_LigneCour[GTE_UWY_NF])) {
      return n_Ret;
   }
   return (strcmp(ptsz_LigneSuiv[GTE_UW_NT], ptsz_LigneCour[GTE_UW_NT]));
}


/**************************************************************************/
/*** Objet : fonction lancee pour chaque ligne en rupture premiere du 	***/
/***         fichier maitre						***/
/***									***/
/*** Nom : n_ActionPremiereRupture					***/
/***									***/
/*** Parametres:							***/
/***	i ptsz_LigneCour : pointeur sur la ligne courante		***/
/***									***/
/*** Retour:								***/
/***	OK si pas d'erreur,						***/
/***	ERR si erreur.							***/
/**************************************************************************/

int n_ActionPremiereRupture(char *ptsz_LigneCour[])
{
   DEBUT_FCT("n_ActionPremiereRupture");

/* Initialisation des valeurs des SP-SAP comptes complets et SP comptes Incomplets: */
Kd_SPSAPcomplets=0;
Kd_SPincomplets=0;
Kd_PNA=0;

   RETURN_VAL(OK);
}

/**************************************************************************/
/*** Objet : fonction lancee pour chaque ligne en rupture premiere du 	***/
/***         fichier maitre						***/
/***									***/
/*** Nom : n_ActionDerniereRupture					***/
/***									***/
/*** Parametres:							***/
/***	i ptsz_LigneCour : pointeur sur la ligne courante		***/
/***									***/
/*** Retour:								***/
/***	OK si pas d'erreur,						***/
/***	ERR si erreur.							***/
/**************************************************************************/

int n_ActionDerniereRupture(char *ptsz_LigneCour[])
{
CS_CHAR Ksz_Montant[25];

   DEBUT_FCT("n_ActionDerniereRupture");


if (Kd_SPSAPcomplets!=0){
	ptsz_LigneCour[GTE_ACMTRS_NT]="-20000";
	sprintf(Ksz_Montant,"%-.3lf",Kd_SPSAPcomplets);
	ptsz_LigneCour[GTE_ACMAMT_M]=Ksz_Montant;
        n_WriteCols(Kp_OutputFile, ptsz_LigneCour,'~', 0);
	}

if (Kd_SPincomplets!=0){
	ptsz_LigneCour[GTE_ACMTRS_NT]="20000";
	sprintf(Ksz_Montant,"%-.3lf",Kd_SPincomplets);
	ptsz_LigneCour[GTE_ACMAMT_M]=Ksz_Montant;
        n_WriteCols(Kp_OutputFile, ptsz_LigneCour,'~', 0);
	}

if (Kd_PNA!=0){
	ptsz_LigneCour[GTE_ACMTRS_NT]="10030";
	sprintf(Ksz_Montant,"%-.3lf",Kd_PNA);
	ptsz_LigneCour[GTE_ACMAMT_M]=Ksz_Montant;
        n_WriteCols(Kp_OutputFile, ptsz_LigneCour,'~', 0);
	}

   RETURN_VAL(OK);
}


/**************************************************************************/
/*** Objet : fonction lancee pour chaque ligne du fichier maitre	***/
/***									***/
/*** Nom : n_ActionLigneRupture						***/
/***									***/
/*** Parametres:							***/
/***	i ptsz_LigneCour : pointeur sur la ligne courante		***/
/***									***/
/*** Retour:								***/
/***	OK si pas d'erreur,						***/
/***	ERR si erreur.							***/
/**************************************************************************/

int n_ActionLigneRupture(char *ptsz_LigneCour[])
{
  DEBUT_FCT("n_ActionLigneRupture");

  if (strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"-20000")==0 ||
      strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"-20030")==0)
    Kd_SPSAPcomplets=Kd_SPSAPcomplets+atof(ptsz_LigneCour[GTE_ACMAMT_M]);

  if (strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"20000")==0 ||
      strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"-20030")==0)
    Kd_SPincomplets=Kd_SPincomplets+atof(ptsz_LigneCour[GTE_ACMAMT_M]);

  if (strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"10000")==0 ||
      strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"10010")==0 ||
      strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"10020")==0 ||
      strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"10130")==0 ||
      strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"10430")==0 ||
      strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"28030")==0)
    n_WriteCols(Kp_OutputFile, ptsz_LigneCour,'~', 0);

  if (strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"10030")==0 ||
      strcmp(ptsz_LigneCour[GTE_ACMTRS_NT],"10040")==0)
    Kd_PNA=Kd_PNA+atof(ptsz_LigneCour[GTE_ACMAMT_M]);

  RETURN_VAL(OK);
}
