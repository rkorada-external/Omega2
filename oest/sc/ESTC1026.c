...
/* if (n_OpenFileAppl ("ESTC1026_I2","rb",&Kp_Libelle) == ERR)
    exit(ERR); */

 if (n_OpenFileAppl("ESTC1026_I3","rb",&Kp_Cours) == ERR )
	ExitPgm ( ERR_XX , "" ); 
 ...
 /*if (n_ChargLib(Kp_Libelle) == ERR) 
  ...
  if (n_OpenFileAppl("ESTC1026_O1","wt",&Kp_GoelPeopOFil) == ERR )
	ExitPgm ( ERR_XX , "" );
 ...
 /*traitement*/
  ...
  if (n_CloseFileAppl("ESTC1026_I1",&(Kbd_Rupt.pf_InputFil)) == ERR)
    ExitPgm (ERR_XX ,"");
  
  /* Fermeture du fichier des libelles */
  /*if (n_CloseFileAppl("ESTC1026_I2",&Kp_Libelle) == ERR)
    ExitPgm (ERR_XX ,"");*/
    
  if (n_CloseFileAppl("ESTC1026_O1",&Kp_GoelPeopOFil) == ERR)
    ExitPgm (ERR_XX ,"");
    
  if (n_CloseFileAppl("ESTC1026_I3",&Kp_Cours) == ERR)
    ExitPgm (ERR_XX ,""); 
...
int n_InitPeop(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl ("ESTC1026_I1","rt",&(pbd_Rupt->pf_InputFil))){
	printf("Erreur ouverture ESTC1026_I1_n");
  	RETURN_VAL (ERR); 
}
...
int n_DecodPste(char **ptb_InRec_Cur)
{
	...
	case 5  : ptb_InRec_Cur[TECLEDA_TRNCOD_CF] = "rec.cloture";
		break;
	case 6  : ptb_InRec_Cur[TECLEDA_TRNCOD_CF] = "prime.acquise";
		break;
...
*/
/*=======================================================================================
objet  : conversion des monnaies des filiales
...
int n_conversion(char **ptb_InRec_Cur)
{
	...
	/*if ( ptb_InRec_Cur[TECLEDA_CUR_CF] == 'UsD' )
		ptb_InRec_Cur[TECLEDA_CUR_CF] = "USD";*/
	
	if ( strcmp(ptb_InRec_Cur[TECLEDA_CUR_CF],"UsD") == 0 ) 
		ptb_InRec_Cur[TECLEDA_CUR_CF] ="USD";
...
int n_Write_out(char **ptb_InRec_Cur) 
{
	fprintf(Kp_GoelPeopOFil,"%s~%s.%s~brut~%s~%s~%s~%s\n",
