...
[10]  12/08/2020 JYP : SPIRA 89218 : taille tableau FBOPRSLNK
================================================================================*/
...
#define Kn_MaxLigFBOTRSLNK   60000     // [002]
...
//printf("==> s_prs %i\n",s_Prs);	
	...
	if ( n_OpenFileAppl ( "ESTC1051_I4","rb",&Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );

	/* ouverture du fichier en entree des postes cumuls FTRSLNK */
	if ( n_OpenFileAppl ( "ESTC1051_I3","rb",&Kp_InputFilTrsLnk ) == ERR )
		ExitPgm( ERR_XX , "" );
		
    if (n_OpenFileAppl("ESTC1051_I5", "rb", &Kp_FBOTRSLNK) == ERR )
        ExitPgm(ERR_XX ,"");
	...
	if ( n_OpenFileAppl ( "ESTC1051_O1","wt",&Kp_OutputFilResSii ) == ERR )
		ExitPgm( ERR_XX , "" );
	...
	if ( n_CloseFileAppl( "ESTC1051_I1", &( bd_RuptPerUw.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESTC1051_I2", &( bd_RuptStatGta.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESTC1051_I3", &Kp_InputFilTrsLnk ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESTC1051_I4", &Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESTC1051_O1", &Kp_OutputFilResSii ) == ERR )
		ExitPgm( ERR_XX , "" );
...
int n_InitPerUw(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC1051_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		return ERR;
...
int n_InitStatGta( T_RUPTURE_SYNC_VAR  *pbd_Rupt )
{
	...
	if ( n_OpenFileAppl( "ESTC1051_I2", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		return ERR;
...
//printf("---> avant n_RechPoste - %s\n",sz_Trncod);
	// [001]
		...
		if (i==-1)
			...
			l_AcmTrs = Ktbd_TrsLnk[i].ACMTRS_NT;
