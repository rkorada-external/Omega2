...
nom du source                 : ESTF0001.c
revision                      : $Revision: 1.2 $
date de creation              : 01/04/1998
auteur                        : M.HA-THUC
...
historique des modifications :
   <jj/mm/aaaa><auteur>    <description de la modification>
	...
	16/10/2018  Ajout de l'exercice  :spira5605 ,REQ.P.03.2
...
#define CTRGRO_UWY_NF 21 //dernier champs
/*----------------------------------*/
...
int main(int argc  , char *argv[])
{
	...
	if ( n_OpenFileAppl ( "ESTF0001_O1", "wt", &Kp_OutputFilAno ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
	...
	if ( Kn_CtrAno != 0 )
		n_EcrireAno( CTRGRO_CTR_NF+1, Kn_CtrAno ) ;

	if ( Kn_EndAno != 0 )
		n_EcrireAno( CTRGRO_END_NT+1, Kn_EndAno ) ;

	if ( Kn_SecAno != 0 )
		n_EcrireAno( CTRGRO_SEC_NF+1, Kn_SecAno ) ;

	if ( Kn_SsdAno != 0 )
		n_EcrireAno( CTRGRO_SSD_CF+1, Kn_SsdAno ) ;

	if ( Kn_SegtypAno != 0 )
		n_EcrireAno( CTRGRO_SEGTYP_CT+1, Kn_SegtypAno ) ;

	if ( Kn_SegAno != 0 )
		n_EcrireAno( CTRGRO_SEG_NF+1, Kn_SegAno ) ;

	if ( Kn_NbrChampsAno != 0 )
		n_EcrireAnoNbrChamps( Kn_NbrChampsAno ) ;

	if ( n_CloseFileAppl( "ESTF0001_I1", &( bd_RuptCtrGro.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTF0001_O1", &Kp_OutputFilAno ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
...
int n_InitCtrGro(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTF0001_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		return ERR ;
  ...
  char **pbd_InRec_Cur  ) /* adresse de la ligne courante */
{
	...
	if ( ( ret = strcmp( pbd_InRec[CTRGRO_VRS_NF], pbd_InRec_Cur[CTRGRO_VRS_NF] ) ) != 0 ) return ret ;
	...
	if ( ( ret = strcmp( pbd_InRec[CTRGRO_UWY_NF], pbd_InRec_Cur[CTRGRO_UWY_NF] ) ) != 0 ) return ret ;
...
int n_ActionLastRuptCtrGro( char **ptb_InRec_Cur )
{
	...
	if ( Kn_Doublons > 1 )
		fprintf( Kp_OutputFilAno, "%s~%s~%s : %d %s %s/%s/%s/%s/%s/%s/%s/%s~%s\n",
		         Ksz_Usr,
		         Ksz_LogTyp,
		         Ksz_CtrGro_Typ,
		         Kn_Doublons,
...
int n_NbrChamps( char **pbd_InRec_Cur )
{
	...
	if ( pbd_InRec_Cur[CTRGRO_UWY_NF] == NULL )
		return ( FALSE ) ;
	else
	{
		/* test sur le suivant */
		if ( pbd_InRec_Cur[CTRGRO_UWY_NF + 1] == NULL )
			return ( TRUE ) ;
		else	return ( FALSE ) ;
	}
}
