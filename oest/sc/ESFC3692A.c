/**==================================================================================================
APPLICATION NAME          		: IFRS17 REVENUE CALCULATION RETRO NP
PROGRAM NAME                 	: ESFC3692A.c
REVISION                      	: V1
CREATION DATE              		: 10/2020
AUTHOR                        	: L.ELFAHIM
------------------------------------------------------------------------------------------------------
DESCRIPTION :
	THIS PROGRAM AIMS TO HANDLE IFRS17 REVENUE CALCULATION FOR RETRO NP PROPORTIONAL CONTRACTS
------------------------------------------------------------------------------------------------------
CHANGE HISTORY :
...
06/10/2020    	LEL    	87722	SPLIT ESFC3692 PROGRAM to ( ESFC3692A : RETRO NP && ESFC3692B : RETRO P )
...
[001] 20/05/2022      DAD     104374  I17 revenue- Undue DSC calculation  
...
int main( int argc,  char *argv[] )
{
	...
	if ( n_OpenFileAppl ( "ESFC3692A_O1","wt",&Kp_OutputFil_REVENUE ) == ERR )
		ExitPgm( ERR_XX , "" );
	...
	/***************************************************************************/
	if ( n_CloseFileAppl( "ESFC3692A_I1", &( Kbd_Rup_LockedInRate.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3692A_I2", &( Kbd_Rupt_FORWARD.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3692A_O1", &Kp_OutputFil_REVENUE ) == ERR ) 
		ExitPgm( ERR_XX , "" );
...
int n_Init_LockedInRate(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl ("ESFC3692A_I1", "rt", &(pbd_Rupt->pf_InputFil)))
		RETURN_VAL (ERR);
...
int n_Init_FORWARD(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3692A_I2", "rt", &(pbd_Sync->pf_InputFil));
		...
		else if ( ACMTRS3 == 2090 && strcmp(pbd_InRecChild[EGPI_R1],"") != 0)
		{
			...
			if( strcmp(pbd_InRecOwner[CSM_PREVQ],"") != 0 )
			{
				...
				n_WriteCols(Kp_OutputFil_REVENUE,  pbd_InRecOwner, '~', 0);	
                                ...
                                CSM_RAT = atof(pbd_InRecOwner[CSM_Q]);
				sprintf(amount , "%.3f" , atof(pbd_InRecOwner[CML_ACMAMT_MC]) * CSM_RAT );
				pbd_InRecOwner[CML_ACMAMT_MC] = amount;				
				sprintf(total_mnt , "%.3f" , atof(pbd_InRecOwner[CML_TOTAUX_MC]) * CSM_RAT );
				pbd_InRecOwner[CML_TOTAUX_MC] = total_mnt; 
	...
	{	 
		...
		if( ACMTRS3 == 2090 && strcmp(pbd_InRecChild[EGPI_R1],"") != 0 )
		{
			...
			if( strcmp(pbd_InRecChild[CSM_PREVQ],"") != 0 && fabs(atof(pbd_InRecChild[CSM_PREVQ])) > 0 )
			{
				...
				CSM_RAT = atof(pbd_InRecChild[CSM_Q])  / atof(pbd_InRecChild[CSM_PREVQ]);
