...
//  n_parm_SITE = atoi(psz_GetCharArgv(1));       /* indique si on est sur site emetteur ou recepteur pour TRFCROSSREF  */
	...
	if (n_OpenFileAppl ("ESTX7005_O1","wb",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7005_O2","wb",&Kp_crostrfa) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_Processing (&bd_utctlib) != CS_SUCCEED)
		ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7005_O1",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7005_O2",&Kp_crostrfa) == ERR )
           ExitPgm (ERR_XX, "");
