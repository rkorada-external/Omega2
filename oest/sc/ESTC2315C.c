...
[14] 27/05/2021 L. DOAN    	:spira 91532 recompilation des programmes C
[15] 04/06/2021 MZM             :spira:96833  Modification type de variable ANO TNR NB Postes DETTRS
[16] 22/03/2023 MZM  SPIRA 108791 PROD - Missing Internal Assumed generated from AE booked on Internal Retro /  Prise en compte du fichier des AE  
[17] 17/12/2024 JYP  SPIRA 112425 Opening : EBS acceptation should not be generated from retro opening AE
[18] 16/07/2025 M.NAJI  : US 5559 SERQS - RA/SAP interface -Phase 1 , update MAX_SSDACTR
...
/*----------------------------------*/
#define PLACE_SSDRTO_B  71
#define PLACE_RETOVRCOM_B 72
#define SSD_ACTR_SSD_NF 73
#define SSD_ACTR_CTR_NF 74
#define SSD_ACTR_UWY_NF 75
#define SSD_ACTR_UW_NT 76
#define SSD_ACTR_SEC_NF 77
#define SSD_ACTR_END_NT 78
#define SSD_ACTR_CLISSD_NF 79  
#define SSD_ACTR_RTOSSD_CF 80
...
int  Kn_DetTrs_Nbp ; //[15] short Kn_DetTrs_Nbp ;     /* compteur du nombre de postes du tableau Ktbd_DetTrs */
...
char Ksz_norme_cf[5];    //[16] Norme_CF
...
//[16]
  ...
  if ( n_OpenFileAppl ( "ESTC2315C_I2", "rb", &Kp_InputFilDetTrs ) == ERR )
    ExitPgm( ERR_XX , "" ) ;

  /* ouverture du fichier en sortie GT */
  if ( n_OpenFileAppl ( "ESTC2315C_O1", "wt", &Kp_OutputFilGt ) == ERR )
    ExitPgm( ERR_XX , "" ) ;

  /* ouverture du fichier en sortie GT */
  if ( n_OpenFileAppl ( "ESTC2315C_O2", "wt", &Kp_OutputFilGt2 ) == ERR )
    ExitPgm( ERR_XX , "" ) ;
  ...
  if ( n_CloseFileAppl( "ESTC2315C_I1", &( bd_RuptGtrr.pf_InputFil ) ) == ERR )
    ExitPgm( ERR_XX , "" ) ;


  if ( n_CloseFileAppl( "ESTC2315C_I2", &Kp_InputFilDetTrs ) == ERR )
    ExitPgm( ERR_XX , "" ) ;

  if ( n_CloseFileAppl( "ESTC2315C_O1", &Kp_OutputFilGt ) == ERR )
    ExitPgm( ERR_XX , "" ) ;

  if ( n_CloseFileAppl( "ESTC2315C_O2", &Kp_OutputFilGt2 ) == ERR )
    ExitPgm( ERR_XX , "" ) ;
...
int n_InitGtrr(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if ( n_OpenFileAppl( "ESTC2315C_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
    return ERR ;
...
int n_ActionLigneGtrr( char **ptb_InRec_Cur )
{
  ...
  /* filtre sur 2eme caractere du TRNCOD >3 */   //[16] Exception pour I17
  if ( ( *ptb_InRec_Cur[PLACE_SSDRTO_B] == '1' && fabs(atof(ptb_InRec_Cur[GT_RETAMT_M])) >= 0.001 ) 
       && (  
           ( (ptb_InRec_Cur[GT_TRNCOD_CF][1] != '7')  &&  (ptb_InRec_Cur[GT_TRNCOD_CF][1] != '9')
				...
				&&  ! ( ptb_InRec_Cur[GT_TRNCOD_CF][1] == 'J' && strncmp(Ksz_norme_cf, "EBS",3) == 0  )
       		 ) 
       	 || ( !strncmp(Ksz_norme_cf, "I17",3) && ptb_InRec_Cur[GT_TRNCOD_CF][1] != '7' )  )	
      )
  {
 
    /* si la recherche dans TSSDACTR n'aboutit pas, generation
    d'une anomalie et pas d'ecriture en sortie */
    if ( ! *ptb_InRec_Cur[SSD_ACTR_SSD_NF]  )
    {
    ...
    else
    {
      ...
      {
        if ( *ptb_InRec_Cur[PLACE_RETOVRCOM_B]  == '1' )  
        {
          if ( strncmp( sz_TrnCodAccpt, "2112110", 7 ) == 0 )
            strcpy( sz_TrnCodAccpt, "1112100" ) ;
          else
            strcpy( sz_TrnCodAccpt, "1412100" ) ;

        }
        else
        {
          if ( strncmp( sz_TrnCodAccpt, "2112110", 7 ) == 0 )
            strcpy( sz_TrnCodAccpt, "1112120" ) ;
          else
            strcpy( sz_TrnCodAccpt, "1412120" ) ;
        }
      }
      else
      {
      ...
      if ( strcmp(Ksz_typetrt_ct, "GT_SII") != 0 )
      {
        ...
        fprintf( Kp_OutputFilGt, "%s~~%s~%s~%s~%s~~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%-.3lf~%s~~%s~A~~~~~~~~~~~~~~~~~~%s~%s~%s~%s~%s~%s~%s~%s\n",//[12]
                ptb_InRec_Cur[SSD_ACTR_RTOSSD_CF],
      ...
      fprintf( Kp_OutputFilGt, "%s~~%s~%s~%s~%s~~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%-.3lf~%d~~%d~A~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%-.3lf~%d~%d~%s~%s~%s~%-.3lf~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s\n",  //[009]//[12]
                 ptb_InRec_Cur[SSD_ACTR_RTOSSD_CF],
                 ptb_InRec_Cur[GT_BALSHEY_NF],
                 ptb_InRec_Cur[GT_BALSHRMTH_NF],
                 ptb_InRec_Cur[GT_BALSHRDAY_NF],
