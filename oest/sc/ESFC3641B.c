...
int main( int argc,  char *argv[] )
{
	...
	if ( n_OpenFileAppl ( "ESFC3641B_O1","wt",&Kp_OutputFilRatio ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	/* Initialisation de la variable Kbd_Rup_RATIO */
	...
	/***************************************************************************/
	if ( n_CloseFileAppl( "ESFC3641B_I1", &( Kbd_Rup_RATIO.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3641B_O1", &Kp_OutputFilRatio ) == ERR ) 
		ExitPgm( ERR_XX , "" );
...
int n_Init_RATIO(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl ("ESFC3641B_I1", "rt", &(pbd_Rupt->pf_InputFil)))
		RETURN_VAL (ERR);
...
int n_IsRupt_RATIO(char **ptb_InRec, char **ptb_InRec_Cur)
{
	...
	if ( ( ret = strcmp( ptb_InRec[RP_CTR_NF], ptb_InRec_Cur[RP_CTR_NF] ) ) != 0 ) 				return ret;
	if ( ( ret = atoi( ptb_InRec[RP_SEC_NF] ) - atoi( ptb_InRec_Cur[RP_SEC_NF] ) ) != 0 ) 		return ret;
	if ( ( ret = strcmp( ptb_InRec[RP_UWY_NF], ptb_InRec_Cur[RP_UWY_NF] ) ) != 0 ) 				return ret;
	if ( ( ret = atoi( ptb_InRec[RP_UW_NT] ) - atoi( ptb_InRec_Cur[RP_UW_NT] ) ) != 0 ) 			return ret;
	if ( ( ret = strcmp( ptb_InRec[RP_RETCTR_NF], ptb_InRec_Cur[RP_RETCTR_NF] ) ) != 0 ) 			return ret;
	if ( ( ret = atoi( ptb_InRec[RP_RETSEC_NF] ) - atoi( ptb_InRec_Cur[RP_RETSEC_NF] ) ) != 0 ) 	return ret;
	if ( ( ret = strcmp( ptb_InRec[RP_RETUWY_NF], ptb_InRec_Cur[RP_RETUWY_NF] ) ) != 0 ) 				return ret;
	if ( ( ret = atoi( ptb_InRec[RP_RETUW_NT] ) - atoi( ptb_InRec_Cur[RP_RETUW_NT] ) ) != 0 ) 	return ret;
...
{
	DEBUT_FCT("n_ActionLastRupt_RATIO");
	...
	//only when more than one CSUO ( Contract / section / UWY / Order ) + RCSUO ( Retro Contract / section / UWY / Order )
	if ( n_RuptCount > 1 ) {
	...
	fprintf(Kp_OutputFilRatio,
			...
			pbd_InRecOwner[RP_CTR_NF],
			"", //pbd_InRecOwner[RP_END_NT], 
			pbd_InRecOwner[RP_SEC_NF], 
			pbd_InRecOwner[RP_UWY_NF], 
			pbd_InRecOwner[RP_UW_NT],
			pbd_InRecOwner[RP_RETCTR_NF],
			"", //pbd_InRecOwner[RP_RETEND_NT], 
			pbd_InRecOwner[RP_RETSEC_NF], 
			pbd_InRecOwner[RP_RETUWY_NF], 
			pbd_InRecOwner[RP_RETUW_NT],
			"", //pbd_InRecOwner[RP_PLC_NT],
			EGPI_R1,
			EGPI_R2,
			EARP_R1,
			d_FUTURE_PREM_PREVQ,
...
int n_ActionLigne_RATIO( char **pbd_InRecOwner )
{
	DEBUT_FCT("n_ActionLigne_RATIO");
		...
		d_FUTURE_PREM_PREVQ += atof(pbd_InRecOwner[RP_FUTURE_PREM_PREVQ]);
		d_FUTURE_PREM_Q += atof(pbd_InRecOwner[RP_FUTURE_PREM_Q]);
		d_PREM_ESTM_PREVQ += atof(pbd_InRecOwner[RP_PREM_ESTM_PREVQ]);
		d_PREM_ESTM += atof(pbd_InRecOwner[RP_PREM_ESTM]);
		d_REMAIN_ESTM_PREVQ += atof(pbd_InRecOwner[RP_REMAIN_ESTM_PREVQ]);
		d_REMAIN_ESTM += atof(pbd_InRecOwner[RP_REMAIN_ESTM]);
		d_FIXED_CHARGE_ACT += atof(pbd_InRecOwner[RP_FIXED_CHARGE_ACT]);
		d_ITD_PREM_ACT += atof(pbd_InRecOwner[RP_ITD_PREM_ACT]);
		d_ITD_PREM += atof(pbd_InRecOwner[RP_ITD_PREM]);
		d_UPR_PREVQ += atof(pbd_InRecOwner[RP_UPR_PREVQ]);
		d_UPR_Q += atof(pbd_InRecOwner[RP_UPR_Q]);
