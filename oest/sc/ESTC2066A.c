...
MOD9 29/06/2022 M.NAJI : SPIRA 86220 : Optimisation 
MOD10 11/08/2022 JYP  SPIRA : 86220 bugfix missing dates fields
MOD11 18/10/2022 JYP  SPIRA : 103258 bugfix I17(L/P) DSC DSI patterns 
MOD12 23/01/2023 MiS  SPIRA : 108037 Update to make I17S run as I17G
MOD13 22/02/2023 JYP  SPIRA : 108922 some patterns not applied for I17L/I17P
MOD14 01/09/2023 DAD  SPIRA : 110165 Initial amount LKI = Total amount of the original Cashflow for RAD
==============================================================================*/
...
/*----------------------------------------*/
static char VERSION_ESTC2066A_C[150] = "__version__: ESTC2066A.c version [013] some patterns not applied for I17L/I17P " ;
...
//d�finition des variables d'input
char sz_Clodat_d[9] = "";
char sz_ClodatY_d[5] = "";
char sz_ClodatM_d[3] = "";
char sz_ClodatD_d[3] = "";
...
#define CML_GRPRATEINDEX_CT_A 124
#define CML_GRPRATEINDEX_CT_R 125
#define CML_illiquidity_A	  126
#define CML_illiquidity_R	  127
...
// d�coupage du fichier de pattern en SEG_NF
...
int * GetTblPatternDiscount(const char * CTR_NF, const char * SEC_NF, const char * UWY_NF, const char * UW_NT, const char * END_NT, const char * SEGNAT_CT, const char * devise, const char * NORME_CF, int ssd_cf, int esb_cf, const char * ctrtyp,char * ill);
int * GetTblPatternLKI(const char * CTR_NF, const char * SEC_NF, const char * UWY_NF, const char * UW_NT, const char * END_NT, const char * SEGNAT_CT, const char * devise, const char * NORME_CF, int ssd_cf, const char * ctrtyp, int esb_cf,char * rateindex);
long  lignes = 0; 

//void     ExtractLineCSWithNoSync();      // sortie des pattern cash flow non utilis�e
/*---------------------------------------------*/
/* Definition des constantes et macros privees */
/*--------------------------------------------*/
int nb_Devise = 0 ; // pour stocker le nombre de devise charg�e dynamiquement
int nb_Contrat = 0; // pour stocker le nombre de Illiquidity charg�e dynamiquement
int nb_RIndex = 0 ; // pour stocker le nombre de Rate index charg�e dynamiquement
int nb_Pattern = 0;
int nb_Cumul = 0;
char * sz_retour = NULL;
...
int main(int argc, char **argv)
{
  ...
  if (n_OpenFileAppl("ESTC2066A_O1", "wt", &Kp_OutputBatch)) ExitPgm(ERR_XX, "Probl�me lors de l'ouverture du 1er fichier (ESCOMPTE)." );
  ...
  //MOD12
  if(strcmp(psz_GetCharArgv(4),"I17S") == 0)
  {
    strcpy(sz_Norm_print_cf, psz_GetCharArgv(4));
    strcpy(sz_Norm_cf, "I17G");
  }
  else
  {
 ...
 if (strlen(sz_Clodat_d) == 8 ) 
  ...
  if (n_OpenFileAppl("ESTC2066A_I2", "rt", &Kp_InputPattern) == ERR )   ExitPgm(ERR_XX, "Probl�me lors de l'ouverture du fichier de cashflow." );
  if (n_OpenFileAppl("ESTC2066A_I3", "rt", &Kp_InputCurrency) == ERR )   ExitPgm(ERR_XX, "Probl�me lors de l'ouverture du fichier des devises." );
  ...
  if (n_CloseFileAppl("ESTC2066A_I3", &Kp_InputCurrency))  ExitPgm(ERR_XX, "Probl�me lors de la fermeture du fichier de Devise (CUR_CF).");
  ...
  if (n_InitRfrBatchIN(&Kbd_ruptRfrBatchIN)) ExitPgm(ERR_XX, "Probl�me lors de l'ex�cution de la m�thode n_InitRfrBatchIN");
  if (n_ProcessingRuptureVar(&Kbd_ruptRfrBatchIN) != OK) ExitPgm(ERR_XX, "Erreur lors du traitement ligne � ligne." );

  // Fermeture des fichiers ouverts
  if (n_CloseFileAppl("ESTC2066A_I1", &(Kbd_ruptRfrBatchIN.pf_InputFil)))  ExitPgm(ERR_XX, "Probl�me lors de la fermeture du fichier d'input.");
  if (n_CloseFileAppl("ESTC2066A_I2", &Kp_InputPattern))  ExitPgm(ERR_XX, "Probl�me lors de la fermeture du fichier de Pattern Discount."); 
  if (n_CloseFileAppl("ESTC2066A_O1", &Kp_OutputBatch))          ExitPgm(ERR_XX, "Probl�me lors de la fermeture du fichier ESCOMPTE.");
...
/*  Description : Charge en m�moire le fichier contenant les Currency         */
/* ----------------------------------------------------------------------------*/
void ChargementCurrency()
{
  // on va d�terminer le nombre de ligne , et donc le nombre de devise � charger
  ...
  if (nb_Devise > 0 )
  {
    ...
    { // tab contient maintenant les donn�es de la ligne en cours :
...
/*  Description : recherche et renvoie la devise correspondant � la r�f�rence  */
/* ----------------------------------------------------------------------------*/
char * GetDevisebyRef(const char * ref)
{
  // on va recherche la devise correspondant � la r�f�rence
...
/*  ATTENTION   : Remet la position au d�but du  fichier            */
/* -----------------------------------------------------------------------*/
long getFileNbLigne(FILE * fl)
{
  long nbligne = 1;
...
char** split(char* chaine, const char* delim, int vide)
{
  ...
  char* largestring;             //chaine � traiter
...
/*==============================================================================
 Objet :            Chargement des donn�es provenant du fichier de cash flow dans les structures qui vont bien
 Parametre(s) :     pCS est la structure sur laquelle faire le chargement
          ...
          Les champs ont d�j� �t� splitt� avec le s�parateur qui va bien
...
/*==============================================================================
 Objet :            Chargement des donn�es provenant du fichier de Discount
 Parametre(s) :     aucun
 Retour :           aucun
==============================================================================*/
void ChargementDiscount()
...
int n_InitRfrBatchIN(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTC2066A_I1", "rt", &(pbd_Rupt->pf_InputFil)))
    return ERR;
...
/*==============================================================================
 ...
 Retour :           En cas de probleme retourne un tableau vide
                    sinon retourne contenant le pattern associ�e 
==============================================================================*/
int * GetTblPatternDiscount(const char * CTR_NF, const char * SEC_NF, const char * UWY_NF, const char * UW_NT, const char * END_NT, const char * SEGNAT_CT, const char * devise, const char * NORME_CF, int ssd_cf, int esb_cf, const char * ctrtyp,char * illiquidity)
{
  int* tbl = NULL;
  int* TblEsc = NULL;
  int indexMax = 0;
  ...
  char CurrNorme[21];
  int indNature = 0; // index de la SEGNAT_CT trouv�e , si il y en a une
  int indNorme = 0;
  int x = 0;
  int y = 0;
  int index;
  ...
  if ( strcmp(sz_Norm_cf,"EBS") != 0){
	...
	//}
	  ...
	  // si aucun illiquidity trouv�e on retourne un tableau vide
  ...
  for (x = 0 ; x < nb_Pattern; x++) /* Updated for Phase1b Migration */
  {
	...
	{
	  // on va r�cup�rer le nombre de pattern qui correspondent � la devise
	  nbDisc++;
  ...
  if (tbl == NULL)
  {
    printf("Erreur de chargement dynamique de donn�es\n");
    ExitPgm(ERR_XX, "Probl�me d'allocation de m�moire");
  }
  ...
  for (x = 0 ; x < nb_Pattern; x++)
  {
    if (strncmp(devise, pDiscount[x].db_pat.CUR_CF, lenCurr) == 0 && ( strcmp(pDiscount[x].db_pat.RATEINDEX,illiquidity) == 0 || strcmp(sz_Norm_cf,"EBS") == 0 ))
    {
		// match SSD and ESB for I17 L/P 
		if (   ( strcmp(sz_Norm_cf,"I17L") == 0 || strcmp(sz_Norm_cf,"I17P") == 0) 
			&& ( atoi(pDiscount[x].db_pat.SSD_CF) != ssd_cf || atoi(pDiscount[x].db_pat.ESB_CF) != esb_cf )
		)
		{	 
			tbl[j] = -1 ; 		
		}
  ...
  if (indNorme)
  {
    // on va supprimer les pattern dont la NORME_CF est diff�rente
  ...
  if (indNature)
  {
    // on va supprimer les pattern dont la SEGNAT_CT est diff�rente
    for (x = 0 ; x < j; x++) /* Updated for Phase1b Migration */
    {
  ...
  // on va v�rifier que l'on ait qu'une seule NORME_CF
  for (x = 0; x < j ; x++) /* Updated for Phase1b Migration */
  {
    // on ne va v�rifier que les valides
    if (tbl[x] != -1)
    {
      // on remet la NORME_CF � blanc
      ...
      for (y = 0; y < nbDisc; y++) /* Updated for Phase1b Migration */
      {
        ...
        if (tbl[y] != -1)
        {
          //on va �viter de comparer la NORME_CF en cours ... a elle m�me
...
int * GetTblPatternLKI(const char * CTR_NF, const char * SEC_NF, const char * UWY_NF, const char * UW_NT, const char * END_NT, const char * SEGNAT_CT, const char * devise, const char * NORME_CF, int ssd_cf, const char * ctrtyp, int esb_cf,char * rateindex)
{
 ...
 //}
  // si aucun rateindex trouv�e on retourne un tableau vide
  ...
  // on cherche les ligne de discount qui match 
    if (  (BuffPattern != 0 && strncmp(devise, pDiscount[BuffPattern].db_pat.CUR_CF, lenCurr) == 0 && strcmp(pDiscount[BuffPattern].db_pat.RATEINDEX,rateindex) == 0)
       && (strcmp(sz_Norm_cf,"I17G") == 0 || strcmp(sz_Norm_cf,"EBS") == 0 || (strcmp(sz_Norm_cf,"I17G") != 0  && atoi(pDiscount[BuffPattern].db_pat.SSD_CF) == ssd_cf && atoi(pDiscount[BuffPattern].db_pat.ESB_CF) == esb_cf)) 		 
       )  
    {
  ...
  // on recarde si on a trouv� des Discount sinon on retourne un tableau vide
  if (nbDisc == 0)
  {
	  TblEsc = realloc(NULL, sizeof(int) * 1);
	  nb_CurrPattern = indexMax;
...
int n_ActionLigneRfrBatchIN(char **ptb_InRec_Cur)
{
  ...
  char*     monnaie;
  char      jointure[100] ;
  int     isOK = 0;     // pour v�rifier la synchro , en cas d'erreur on renvoi la ligne dans un fichier nosync
  char    ratingDevise[6];
  ...
  {
    // on r�cup�re la devise de r�f�rence
  ...
  if (strlen(monnaie) != 0 && strncmp(ptb_InRec_Cur [CML_PATTERN_ID],"NODSC",5) !=0 )
  {
	...
	else if (strcmp(sz_Pattyp_ct,"LKI")== 0 || strcmp(sz_Pattyp_ct,"FWD")== 0 )
	{
		...
		if(strncmp(ptb_InRec_Cur[CML_TYP_CT],"A",1) == 0){
			rateindex = ptb_InRec_Cur[CML_GRPRATEINDEX_CT_A] ;
			tbl = GetTblPatternLKI(ptb_InRec_Cur[CML_CTR_NF] , ptb_InRec_Cur [CML_SEC_NF], ptb_InRec_Cur[CML_UWY_NF], ptb_InRec_Cur[CML_UW_NT], ptb_InRec_Cur[CML_END_NT], ptb_InRec_Cur[CML_NAT_CF], monnaie, sz_Norm_cf, atoi(ptb_InRec_Cur [CML_SSD_CF]), ptb_InRec_Cur [CML_TYP_CT], atoi(ptb_InRec_Cur [CML_ESB_CF]),rateindex);
		}
		//if CML_CTR_NF is empty it will be pure RETRO contract
		else {
			rateindex = ptb_InRec_Cur[CML_GRPRATEINDEX_CT_R] ;
			tbl = GetTblPatternLKI(ptb_InRec_Cur[CML_RETCTR_NF] , ptb_InRec_Cur [CML_RETSEC_NF], ptb_InRec_Cur[CML_RTY_NF], ptb_InRec_Cur[CML_RETUW_NT], ptb_InRec_Cur[CML_RETEND_NT], ptb_InRec_Cur[CML_NAT_CF], monnaie, sz_Norm_cf, atoi(ptb_InRec_Cur [CML_SSD_CF]), ptb_InRec_Cur [CML_TYP_CT], atoi(ptb_InRec_Cur [CML_ESB_CF]),rateindex);
		}
	}
	else
	{
		LkiBool = 0;
		if(strncmp(ptb_InRec_Cur[CML_TYP_CT],"A",1) == 0){
			tbl = GetTblPatternDiscount(ptb_InRec_Cur[CML_CTR_NF] , ptb_InRec_Cur [CML_SEC_NF], ptb_InRec_Cur[CML_UWY_NF], ptb_InRec_Cur[CML_UW_NT], ptb_InRec_Cur[CML_END_NT], ptb_InRec_Cur [CML_NAT_CF], monnaie, sz_Norm_cf, atoi(ptb_InRec_Cur[CML_SSD_CF]), atoi(ptb_InRec_Cur[CML_ESB_CF]), ptb_InRec_Cur[CML_TYP_CT],ptb_InRec_Cur[CML_illiquidity_A]);
		}
		//if CML_CTR_NF is empty it will be pure RETRO contract
		else {
			tbl = GetTblPatternDiscount(ptb_InRec_Cur[CML_RETCTR_NF] , ptb_InRec_Cur [CML_RETSEC_NF], ptb_InRec_Cur[CML_RTY_NF], ptb_InRec_Cur[CML_RETUW_NT], ptb_InRec_Cur[CML_RETEND_NT], ptb_InRec_Cur[CML_NAT_CF], monnaie, sz_Norm_cf, atoi(ptb_InRec_Cur[CML_SSD_CF]), atoi(ptb_InRec_Cur[CML_ESB_CF]), ptb_InRec_Cur[CML_TYP_CT],ptb_InRec_Cur[CML_illiquidity_R]);
		}
	}
    ...
    for (idx = 0 ; idx < nb_CurrPattern; idx++) /* Updated for Phase1b Migration */
    {
      if (tbl[idx] != -1)
      {
		...
		//puis tout le reste des colonnes qui pr�c�dent les montants annuels
		...
		if(strcmp(ptb_InRec_Cur[CML_PATTYP_CT],"RMNTP") == 0 && strcmp(ptb_InRec_Cur[CML_PATCAT_CT],"BDT") == 0 ){
				if (strcmp(sz_Pattyp_ct,"LKI")== 0){
					...
					buf,
					sz_Norm_print_cf, // NORME_CF MOD12
		...
		// Puis la liste des taux calcul�s
		// on va d'abord la 1er ann�e ( an 0)
		...
		for (i_an = 1; i_an < PATTERNSII_ANNEES  ; i_an++) /* Updated for Phase1b Migration */
		{
		  ...
		  // on va stocker le r�sultat de chaque ann�e pour le calcul du cumul rmpt
		...
		// Ecriture de la ligne courante , � laquelle on ajoute les 4 colonnes de commentaires
		isOK = 1; // on signale que la synchro est trouv�e  , m�me si on n'�crit pas la ligne
  ...
  // si on a pas trouv� le RATING_CF, on sort la ligne en erreur
  if (isOK == 0)
  {
    char TempACMTRS3[5];
    ...
    // on met les PATTERNSII_ANNEES ann�es � 0
    // d'abord l'ent�te
    sprintf(buf, "%s", ptb_InRec_Cur[CML_SSD_CF]);
    //puis tout le reste des colonnes qui pr�c�dent les montants annuels
	...
	if (LkiBool == 1)
		...
		if(strncmp(ptb_InRec_Cur[CML_PATTERN_ID],"NODSC",5) == 0 ){
			...
			sprintf(buf, "%s~%s~~BDT~LKI~NODSC", buf, sz_Norm_print_cf); //MOD12
			}
			else {
				sprintf(buf, "%s~%s~~DSC~LKI~NODSC", buf, sz_Norm_print_cf); //MOD12
			}
		}
		//MOD2
		else if(strcmp(ptb_InRec_Cur[CML_PATCAT_CT],"BDT") == 0 ){
			sprintf(buf, "%s~%s~~BDT~LKI~ERR", buf, sz_Norm_print_cf); //MOD12
			}
		else
		{
			sprintf(buf, "%s~%s~~%s~%s~ERR", buf, sz_Norm_print_cf, sz_Patcat_ct, sz_Pattyp_ct); //MOD12
		}
	}
	else
	{
	//MOD2
		if(strcmp(ptb_InRec_Cur[CML_PATCAT_CT],"BDT") == 0 ){
			...
			{
				if(strncmp(ptb_InRec_Cur[CML_PATTERN_ID],"NODSC",5) == 0){
					sprintf(buf, "%s~%s~~BDT~BDT~NODSC", buf, sz_Norm_print_cf); //MOD12
				}
				else {
					sprintf(buf, "%s~%s~~BDT~BDT~ERR", buf, sz_Norm_print_cf); //MOD12
				}
			}
		}
		...
		{
			if(strncmp(ptb_InRec_Cur[CML_PATTERN_ID],"NODSC",5) == 0){
				sprintf(buf, "%s~%s~~DSC~DSI~NODSC", buf, sz_Norm_print_cf); //MOD12
			}
			else {
				sprintf(buf, "%s~%s~~%s~DSI~ERR", buf, sz_Norm_print_cf, sz_Patcat_ct); //MOD12
			}
		}
    }
