...
int main(int argc, char **argv)
{
  ...
  if (n_OpenFileAppl("ESTC1071_O1", "wt", &Kp_OutputRemainToPay)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du fichier sortie REMAINTOPAY. " );

  if (n_OpenFileAppl("ESTC1071_I3", "rt", &Kp_InputPattern) == ERR )   ExitPgm(ERR_XX, "Problème lors de l'ouverture du fichier de cashflow." );
  if (n_OpenFileAppl("ESTC1071_I2", "rt", &Kp_InputCurrency) == ERR )   ExitPgm(ERR_XX, "Problème lors de l'ouverture du fichier de CUR_CF." );

  //Chargement des CUR_CF dans un tableau
  ChargementCurrency();
  // on peut fermer le fichier , les donnees sont dans pDevise
  if (n_CloseFileAppl("ESTC1071_I2", &Kp_InputCurrency))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier de Devise (CUR_CF).");
  //
  ...
  if (n_CloseFileAppl("ESTC1071_I1", &Kbd_ruptRfrBatchIN.pf_InputFil))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier d'input.");
  if (n_CloseFileAppl("ESTC1071_I2", &Kp_InputPattern))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier de Pattern Discount.");
  if (n_CloseFileAppl("ESTC1057_O1", &Kp_OutputRemainToPay))          ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier sortie RemainToPay.");
...
int n_InitRfrBatchIN(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTC1071_I1", "rt", &(pbd_Rupt->pf_InputFil)))
    return ERR;
    ...
    while (GetTblPatternDiscount(monnaie, &pDSC, &index) == 0)
    {
      if (pDSC != NULL)
      {
        ...
        for (col = 1; col <= CML_TYP_CT; col++) /* Updated for Phase1b Migration */
        {
          ...
          {
            sprintf(buf2Pay, "%s~%.3f", buf2Pay, total);
          }
          else
          {
