...
------------------------------------------------------------------------------
description :
   FILTRE DU GTAA A PARTIR DU PERIMETRE IADVPERICASE
...
int n_TestRuptPerUw		( char **pbd_InRec, char **pbd_InRec_Cur ) ;
...
int main(int argc  , char *argv[])
{
	...
	if ( n_OpenFileAppl ( "ESTC7604_O1","wt",&Kp_OutputFilGt ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
	...
	if ( n_CloseFileAppl( "ESTC7604_I1", &( bd_RuptPerUw.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTC7604_I2", &( bd_RuptGt.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTC7604_O1", &Kp_OutputFilGt ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
...
int n_InitPerUw(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC7604_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		return ERR ;

	pbd_Rupt->n_NbRupture = 1 ;

	/* fonction du test de rupture de niveau 1 */
	pbd_Rupt->n_ConditionRupture[0] = n_TestRuptPerUw ;
	...
	char **pbd_InRec_Cur  ) /* adresse de la ligne courante */
{
	...
	DEBUT_FCT( "TestRuptPerUw" ) ;

	if ( ( ret = strcmp( pbd_InRec[PER_CTR_NF], pbd_InRec_Cur[GT_CTR_NF] ) ) != 0 ) return ret ;
...
int n_InitGt( T_RUPTURE_SYNC_VAR  *pbd_Rupt )
{
	...
	if ( n_OpenFileAppl( "ESTC7604_I2", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		return ERR ;
	...
	char **ptb_InRecChild ) /* adresse de la ligne de l'esclave */
{
	...
	fprintf( Kp_OutputFilGt,"Contrat : %s Section : %s\n", ptb_InRecChild[GT_SEC_NF],ptb_InRecChild[GT_CTR_NF] );
