...
nom du source                 : ESTC8931.c
revision                      :
date de creation              : 7/1997
auteur                        : Kuhna
references des specifications : #################
squelette de base             : batch
------------------------------------------------------------------------------
description :
 1ere etape avant la generation du GTARR pour les retrocessionnaires internes

 Injection de l'information retrocession interne depuis le fichier des
 placements vers le GTRr

------------------------------------------------------------------------------
historique des modifications :
   <jj/mm/aaaa>   <auteur>    <description de la modification>

Changement du nom du prog C ( anciennement INVC8931.c ) effectue par M.Ha-Thuc
le 27/02/1998
...
/*---------------------------------------------*/
#define LASTCOLGTR GT_RETKEY_CF
...
T_RUPTURE_SYNC_VAR bd_RuptGtr;     /*variable de gestion de la synchro sur
                                     contrat/Exercice/Placement entre le fic
                                     des placements et le GTRr*/
...
objet :
 Injection de l'information retrocession interne depuis le fichier des
 placements vers le GTRr : ajout d'une colonne supplementaire dans le GTRr
 contenant l'indicateur rerocessionnaire dans le groupe (SSDRTO_B du fic
 des placements, a 1 si retrocession interne)
...
int main(int argc,char *argv[])
{
  ...
  if (n_OpenFileAppl ("ESTC8931_O1","wt",&Kp_OutputFil) == ERR)
    ExitPgm(ERR_XX ,"");
  ...
  if (n_CloseFileAppl("ESTC8931_I1",&(bd_RuptPlc.pf_InputFil)) == ERR)
    ExitPgm(ERR_XX,"");

  if (n_CloseFileAppl("ESTC8931_I2",&(bd_RuptGtr.pf_InputFil)) == ERR)
    ExitPgm(ERR_XX,"");

  if (n_CloseFileAppl("ESTC8931_O1",&Kp_OutputFil) == ERR)
    ExitPgm(ERR_XX,"");
...
int n_InitPlc(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTC8931_I1","rt",&(pbd_Rupt->pf_InputFil)) == ERR)
    return ERR;
...
int n_InitGtr(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl ("ESTC8931_I2","rt",&(pbd_Rupt->pf_InputFil)) == ERR)
    ExitPgm(ERR_XX,"");
  ...
  pbd_Rupt->n_ActionLigne = n_ActionLigneGtr;
				...
				/* Fonction d'action sur la ligne courante
                                   du fichier esclave GTRr sans pere*/
...
{
                         /* Ajout du pteur sur le champ "indicateur
                            retrocessionnaire dans le groupe ?" de fic des
                            placements en fin de tab des pteurs sur champs
                            de GTR*/
  ptb_InRecChild[LASTCOLGTR + 1] = ptb_InRecOwner[PLA_SSDRTO_B];
...
/*==========================================================================
objet :
	fonction lancee quand le pare n'a pas de fils
retour :
	OK ---> traitement correctement effectue
	ERR --> probleme rencontre
==============================================================================*/
  ...
  char **ptb_InRecOwner)
{
  char MsgAno[200];      /* message d'anomalie */
                         /* indic retrocessionnaire dans le groupe ?" = 0*/
  ptb_InRecOwner[LASTCOLGTR + 1] = "0";

                         /* Copie ds le fic en sortie*/
  n_WriteCols(Kp_OutputFil,ptb_InRecOwner,SEPARATEUR,0);


  sprintf(MsgAno,"No row in GTRr corresponding to (RETCTR/RTY/PLC) = (%s,%s,%s)\
