...
------------------------------------------------------------------------------
description :
   PREPARATION DE L'ETAT SYNTHETIQUE DE CONTROLE D'INVENTAIRE ACCEPTATION - ETAPE 2
...
int main(int argc  , char *argv[])
{
	...
	if ( n_OpenFileAppl ( "ESTR7608_O1","wt",&Kp_OutputFilReport ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
	...
	if ( n_CloseFileAppl( "ESTR7608_O1", &Kp_OutputFilReport ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
...
int n_InitFti(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTR7608_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		return ERR ;
...
int n_ActionLigneFti( char **ptb_InRec_Cur )
{
	...
	{
	case 10000 :
		Kd_Amt_fl[ACMTRS_10000] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 10030 :
		Kd_Amt_fl[ACMTRS_10030] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 10031 :
		Kd_Amt_fl[ACMTRS_10031] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 10100 :
		Kd_Amt_fl[ACMTRS_10100] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 10130 :
		Kd_Amt_fl[ACMTRS_10130] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 10400 :
		Kd_Amt_fl[ACMTRS_10400] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 10430 :
		Kd_Amt_fl[ACMTRS_10430] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 20000 :
		Kd_Amt_fl[ACMTRS_20000] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 20030 :
		Kd_Amt_fl[ACMTRS_20030] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 20031 :
		Kd_Amt_fl[ACMTRS_20031] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 22000 :
		Kd_Amt_fl[ACMTRS_22000] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 23000 :
		Kd_Amt_fl[ACMTRS_23000] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 24030 :
		Kd_Amt_fl[ACMTRS_24030] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;

	case 24031 :
		Kd_Amt_fl[ACMTRS_24031] = atof( ptb_InRec_Cur[FTIA_AMT_M] ) ;
		break ;
	}
...
int n_ActionLastRuptFti( char **ptb_InRec_Cur )
{
	...
	if ( Kd_Amt_fl[ACMTRS_10000] != 0 || Kd_Amt_fl[ACMTRS_10030] != 0 || Kd_Amt_fl[ACMTRS_10031] != 0 ||
	     Kd_Amt_fl[ACMTRS_10100] != 0 || Kd_Amt_fl[ACMTRS_10130] != 0 || Kd_Amt_fl[ACMTRS_10400] != 0 ||
	     Kd_Amt_fl[ACMTRS_10430] != 0 || Kd_Amt_fl[ACMTRS_20000] != 0 || Kd_Amt_fl[ACMTRS_20030] != 0 ||
	     Kd_Amt_fl[ACMTRS_20031] != 0 || Kd_Amt_fl[ACMTRS_22000] != 0 || Kd_Amt_fl[ACMTRS_23000] != 0 ||
		...
		fprintf( Kp_OutputFilReport, "%s~%s~%s~%s~%s~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f~%-.3f\n",
			...
			ptb_InRec_Cur[FTIA_CTRNAT_CT], ptb_InRec_Cur[FTIA_WRKCAT_CT], Kd_Amt_fl[ACMTRS_10000],
			Kd_Amt_fl[ACMTRS_10030], Kd_Amt_fl[ACMTRS_10031], Kd_Amt_fl[ACMTRS_10100],
			Kd_Amt_fl[ACMTRS_10130], Kd_Amt_fl[ACMTRS_10400], Kd_Amt_fl[ACMTRS_10430],
			Kd_Amt_fl[ACMTRS_20000], Kd_Amt_fl[ACMTRS_20030], Kd_Amt_fl[ACMTRS_20031],
			Kd_Amt_fl[ACMTRS_22000], Kd_Amt_fl[ACMTRS_23000], Kd_Amt_fl[ACMTRS_24030],
			Kd_Amt_fl[ACMTRS_24031] ) ;
