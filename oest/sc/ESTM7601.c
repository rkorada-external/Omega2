...
[10] 28/01/2022 Roger    :spira:98240 Ajout GAAPCOD_NT, I17PRDCOD_CT dans la clé de rupture et code Annulation 'A' dans col GT_NEWCOLS5_NF
==============================================================================*/
...
)
{
  ...
  if (n_OpenFileAppl("ESTM7601_O1", "wt", &Kp_OutputFile) == ERR) {
  ...
  if (n_CloseFileAppl("ESTM7601_I1", &(pbd_Rupture->pf_InputFil)) == ERR) {
    ExitPgm(ERR_XX, "Erreur appel fonction n_CloseFileAppl");
  }

  if (n_CloseFileAppl("ESTM7601_O1", &Kp_OutputFile) == ERR) {
    ExitPgm(ERR_XX , "Erreur appel fonction n_CloseFileAppl");
  }
...
)
{
  ...
  if (n_OpenFileAppl("ESTM7601_I1", "rt", &(pbd_Rupture->pf_InputFil)) == ERR) {
    RETURN_VAL(ERR);
  }
...
)
{
  ...
  if (strcmp(ptsz_LigneSuiv[GT_TRN_NT], ptsz_LigneCour[GT_TRN_NT]) != 0) {
    RETURN_VAL(1);
  }
...
int n_ActionLigneRupture(char *ptsz_LigneCour[])
{
  ...
  if (Ks_Analyse) {
    ...
    Kd_MontantRetInt += atof(ptsz_LigneCour[GT_RETINTAMT_M]);
  }
...
//   char sz_SCOENDMTH_NF[3]; /* Chaine contenant la fin de la periode */
  ...
  if (Ks_Analyse) {
    ...
    ptsz_LigneCour[GT_NEWCOLS5_NF] = "A";  // [10]
    ...
    /**************************************/
    if ( *ptsz_LigneCour[GT_CTR_NF] != 0 && *ptsz_LigneCour[GT_RETCTR_NF] != 0
         && (( Kd_MontantRet != 0.0 &&  fabs(Kd_MontantRet) > 0.000001 ) || ( Kd_MontantRet != 0.0 &&  fabs(Kd_MontantRet) > 0.000001 ) ))      // 18187
      n_WriteCols(Kp_OutputFile, ptsz_LigneCour, '~', 0);
  }
  RETURN_VAL(OK);
}
