...
------------------------------------------------------------------------------
description :
   TRAITEMENT DES RECONDUCTIONS DU BILAN 1997 SUR L'ANCIEN SYSTEME - GTAR -
...
int main(int argc  , char *argv[])
{
	...
	if ( n_OpenFileAppl ( "ESTM2903_I3","rb",&Kp_InputFilAccTrsf ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	/* ouverture du fichier en entree FDETTRS */
	if ( n_OpenFileAppl ( "ESTM2903_I4","rb",&Kp_InputFilDetTrs ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	/* ouverture du fichier de sortie des reconductions */
	if ( n_OpenFileAppl ( "ESTM2903_O1","wt",&Kp_OutputFilRecond ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
	...
	if ( n_CloseFileAppl( "ESTM2903_I1", &( bd_RuptPer.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2903_I2", &( bd_RuptRecond.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2903_I3", &Kp_InputFilAccTrsf ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2903_I4", &Kp_InputFilDetTrs ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2903_O1", &Kp_OutputFilRecond ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
...
int n_InitPer(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTM2903_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		RETURN_VAL(  ERR ) ;
...
int n_InitRecond( T_RUPTURE_SYNC_VAR  *pbd_Rupt )
{
	...
	if ( n_OpenFileAppl( "ESTM2903_I2", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		RETURN_VAL (ERR )  ;
	...
	char **ptb_InRecChild ) /* adresse de la ligne de l'esclave */
{
	...
	char	sz_montantacc[30] ;	/* zone de travail */
	...
	if ( strcmp( sz_PosteDetail, "44000" ) == 0 || strcmp( sz_PosteDetail, "44100" ) == 0 )
	{
		...
		sprintf( sz_montantacc, "%-.3f", ( -1 * atof( ptb_InRecChild[GT_AMT_M] ) ) ) ;
		ptb_InRecChild[GT_AMT_M] = sz_montantacc ;
		...
		sprintf( sz_montantacc, "%-.3f", ( -1 * atof( sz_montantacc ) ) ) ;
