...
----------------------------------------------------------------------------------------------------------------------------------
DESCRIPTION :
	CE PROGRAMME EST DESTINE A FAIRE LE CALCUL DE IFRS17 REVENUE POUR FWD SANS LKI CORRESPONDANT

HISTORIQUE DES MODIFICATIONS :
	<JJ/MM/AAAA>   	<AUTEUR>	<SPIRA>		<DESCRIPTION DE LA MODIFICATION>
	01/10/2020   	LEL       	90051    	DEVELOPPEMENT DE LA VERSION INITIALE
	10/12/2020    	LEL       	90446		IMPLEMENTATION OF FIRST CLOSING CONDITIONS OF CSUOE
	24/12/2020    	LEL  		91111		IMPLEMENTATION OF SPIRA	
...
int main( int argc,  char *argv[] )
{
	...
	if ( n_OpenFileAppl ( "ESFC3694_O1","wt",&Kp_OutputFil_REVENUE ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_OpenFileAppl ( "ESFC3694_O2","wt",&Kp_OutputFil_TRACE ) == ERR )
		ExitPgm( ERR_XX , "" );

	/* Ouverture du fichier en entree des cours de change FCURQUOT */
	if ( n_OpenFileAppl ( "ESFC3694_I7","rb",&Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );
	...
	/***************************************************************************/
	if ( n_CloseFileAppl( "ESFC3694_I1", &( Kbd_Rup_LockedInRate.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_I2", &( Kbd_Rupt_CASHFLOW_Q.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_I3", &( Kbd_Rupt_CASHFLOW_PREVQ.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_I4", &( Kbd_Rupt_ITD_PREM.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_I5", &( Kbd_Rupt_UPR_Q.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_I6", &( Kbd_Rupt_UPR_PREVQ.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_I7", &Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_I8", &( Kbd_Rupt_CASHFLOW_INI.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_O1", &Kp_OutputFil_REVENUE ) == ERR ) 
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3694_O2", &Kp_OutputFil_TRACE ) == ERR ) 
		ExitPgm( ERR_XX , "" );
...
int n_Init_LockedInRate(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl ("ESFC3694_I1", "rt", &(pbd_Rupt->pf_InputFil)))
		RETURN_VAL (ERR);
...
int n_ActionLigne_LockedInRate( char **ptb_InRec_Cur )
{
	...
	int 	ACMTRS3, j;
	double	d_Ratio, d_Assumed_Ratio;
	...
	if (n_DEBUG_LEVEL > 1)
	{
		fprintf(Kp_OutputFil_TRACE,
			"CSUOE %s~%s~%s~%s~%s|ACMTRSL3 %d|FP Q_1 %.3f|FP Q %.3f|WP %.3f|WP ACT %.3f|FC ACT %.3f|UPR Q_1 %.3f|UPR Q %.3f|P ESTM Q_1 %.3f|P ESTM Q %.3f|R ESTM Q_1 %.3f|R ESTM Q %.3f \n", 
	...
	if( fabs(d_FUTURE_PREM_PREVQ - d_UPR_PREVQ) > 1 )
	{
		...
		{	
			...
			{
				d_Assumed_Ratio = ( d_FUTURE_PREM_Q + d_PREM_ESTM + d_REMAIN_ESTM + d_ITD_PREM_ACT )/( d_FUTURE_PREM_PREVQ + d_PREM_ESTM_PREVQ + d_REMAIN_ESTM_PREVQ );
				sprintf(amount , "%.3f" , atof(ptb_InRec_Cur[CML_ACMAMT_MC])* d_Assumed_Ratio );
				ptb_InRec_Cur[CML_ACMAMT_MC] = amount;				
				sprintf(total_mnt , "%.3f" , atof(ptb_InRec_Cur[CML_TOTAUX_MC]) * d_Assumed_Ratio );
				ptb_InRec_Cur[CML_TOTAUX_MC] = total_mnt; 
				for( j = CML_AN1; j<= CML_AM_FIN; ++j )   
				{
					sprintf(ank_mnt [j-CML_AN1] , "%.3f" , atof(ptb_InRec_Cur[j]) * d_Assumed_Ratio );
		...
		d_Assumed_Ratio = ( d_FUTURE_PREM_Q + d_ITD_PREM - d_UPR_PREVQ )/( d_FUTURE_PREM_PREVQ - d_UPR_PREVQ );
		...
		*********************************************************************************/
		...
		if( strcmp(ptb_InRec_Cur[CML_PATCAT_CT],"DSC") == 0 && ACMTRS3 == 3115 )
		{
			ptb_InRec_Cur[CML_PATCAT_CT] = "EXP";
			ptb_InRec_Cur[CML_PATTYP_CT] = "EGPME";
			n_WriteCols(Kp_OutputFil_REVENUE,  ptb_InRec_Cur, '~', 0);
			...
			ptb_InRec_Cur[CML_PATTYP_CT] = "EARME";
			n_WriteCols(Kp_OutputFil_REVENUE,  ptb_InRec_Cur, '~', 0);
...
int n_Init_CASHFLOW_Q(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3694_I2", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_CASHFLOW_PREVQ(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3694_I3", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_CASHFLOW_INI(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3694_I8", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_ITD_PREM(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3694_I4", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_UPR_Q(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3694_I5", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_UPR_PREVQ(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3694_I6", "rt", &(pbd_Sync->pf_InputFil));
