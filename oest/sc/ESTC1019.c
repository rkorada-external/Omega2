...
[002] 11/04/2018 S.Behague     :spira 65703 FORCAGE IBNR : Ajouter l'obligation de renseigner un commentaire lorsque le mode de gestion est FORCE
...
[005] 29/08/2019 R. Cassis :spira:65656 - prs_cf is added as parameter for IFRS4 (710) or EBS (730) (et conversion du .c au format Unix et pas dos)
[006] 24/02/2020 MZM    :spira:82420 TRNCOD Standart AT INCEPTION pour les LOSSCORRIDOR 1A200712
[007] 16/04/2020 JYP : 83851 bugfix spira 82420 TRNCOD Standart AT INCEPTION pour les LOSSCORRIDOR 1A200712
[008] 24/02/2021 MZM    :spira:90000 Issues on some contracts in initial loss corridor calculation : Ajout du tri sur No esction
...
T_RUPTURE_SYNC_VAR 	bd_RuptFUTUR, /* variable syncro futur */
				 	bd_RuptUPRDAC; /* variable syncro futur */

char				Ksz_Batch[2]; // [004]
...
char Ksz_Prs[4] ;	    /* parametre de la chaine: type de poste '710'(IFRS4) ou '730'(EBS) [005] */
char	Annee_bilan[5] ;
char	Mois_bilan[3] ;
...
char  Kc_Norme_Suf[2] ; // = 'Z';  // SUFFICE DE LA Norme pour construire le TRNCOD des FUTURES AT INCEPTION (Kc_Norme_Suf : GROUP --> 'I' ; PARENT --> 'P' ; LOCAL --> 'M')
char  Ksz_Type[2];
int   I17;
...
int main(int argc  , char *argv[])
{
	...
	strcpy(Ksz_NORME, psz_GetCharArgv(4));
	printf("NB_ARGUMENT Ksz_Argc %d Ksz_NORME = %s\n", Ksz_Argc, Ksz_NORME);
	strcpy( Ksz_Prs, psz_GetCharArgv( 5 ) ) ;  // [005]	

	if (Ksz_Argc == 6)
	{
		strcpy( Ksz_Type, psz_GetCharArgv( 6 ) ) ;
	}
	else
	{
		Ksz_Type[0] = 0;
	}
	//if (strcmp(Ksz_NORME, "IFRS4"))
	//{ 
  	 // strcpy(Ksz_NORME, psz_GetCharArgv(4));
//	}
	//else
...
//	}
	...
	if ( strncmp(Ksz_NORME, "I17G", 4) == 0 || strncmp(Ksz_NORME, "I17S", 4) == 0)    //[009]
	{
		I17 = 1;
  ...
  strcat( Ksz_Cre, sz_SysTime ) ;
	...
	if (I17 || Ksz_Type[0] == 'F')
	{
		if (n_OpenFileAppl("ESTC1019_I10", "rb", &Kp_FBOTRSLNK) == ERR )
	...
	if ( n_OpenFileAppl ( "ESTC1019_O2","wt",&Kp_OutputFilCtrEst ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	/* ouverture du fichier des devises */
   if ( n_OpenFileAppl ("ESTC1019_I7","rb",&Kp_CoursFil) == ERR )
     ExitPgm ( ERR_XX , "" );

	/* ouverture du fichier de sortie GT des PAP, PB, Loss Corridor */
	if ( n_OpenFileAppl ( "ESTC1019_O1","wt",&Kp_OutputFilGtPbPapLos ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
...
//Spot 28140 Initialisation fonction n_CalculExerciceSeuil		
	if(n_initCalculExerciceSeuil("ESTC1019_I8"))
       		 ExitPgm( ERR_XX , "" ) ;
	...
	if ( n_CloseFileAppl( "ESTC1019_I1", &( bd_RuptPerUw.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTC1019_I2", &( bd_RuptPerFci.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if (!I17 && Ksz_Type[0] != 'F')
		if ( n_CloseFileAppl( "ESTC1019_I3", &( bd_RuptGtCumTot.pf_InputFil ) ) == ERR )
			ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTC1019_I4", &( bd_RuptGtPa.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTC1019_I5", &( bd_RuptGtCum.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if (!I17 && Ksz_Type[0] != 'F')
		if ( n_CloseFileAppl( "ESTC1019_I6", &( bd_RuptCtrEst.pf_InputFil ) ) == ERR )
			ExitPgm( ERR_XX , "" ) ;

  if ( n_CloseFileAppl ("ESTC1019_I7",&Kp_CoursFil)== ERR)
    ExitPgm ( ERR_XX , "" );

	if (I17 || Ksz_Type[0] == 'F')
		if ( n_CloseFileAppl ("ESTC1019_I10", &Kp_FBOTRSLNK)== ERR)
			ExitPgm ( ERR_XX , "" );
	if ( n_CloseFileAppl( "ESTC1019_O1", &Kp_OutputFilGtPbPapLos ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTC1019_O2", &Kp_OutputFilCtrEst ) == ERR )
		ExitPgm( ERR_XX , "" );
...
int n_InitPerUw(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC1019_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		return ERR ;
...
// Spot 28140 Modification des parametes d'appel de la fonction n_CalculExerciceSeuil et Ajout du  test sur le champ sinistralite
    ...
    if (atoi(ptb_InRec_Cur[PER_ESTLOSCORTYP_CT]) == 1 || atoi(ptb_InRec_Cur[PER_ESTLOSCORTYP_CT]) == 2 || atoi(ptb_InRec_Cur[PER_ESTLOSCORTYP_CT]) == 4)
    {
		...
		//if ( strcmp(ptb_InRec_Cur[PER_CTR_NF], "17T007143") == 0 && atoi(ptb_InRec_Cur[PER_UWY_NF]) == 2019 && atoi(ptb_InRec_Cur[PER_SEC_NF]) == 18)
		//	printf(" DEB  008 Kd_Clmc = %f ;Kd_FuFixedPrem = %f ; Kd_FuClaim = %f ; Kd_Loss = %f ptb_InRec_Cur[PER_LOSCOREXI_B] = %s; ptb_InRec_Cur[PER_SEGSA_B] = %s ; Ksz_Type[0] = %d \n", Kd_Clmc, Kd_FuFixedPrem, Kd_FuClaim, Kd_Loss, ptb_InRec_Cur[PER_LOSCOREXI_B], ptb_InRec_Cur[PER_SEGSA_B], Ksz_Type[0] );
			...
			d_ClmRed = d_CalculReducLossCorr( (char)( atoi( ptb_InRec_Cur[PER_LOSCOREXI_B] ) ),
				...
				atof( ptb_InRec_Cur[PER_LOSCORRAT_R] ) * 10.0, Kd_FuClaim, (-Kd_UPR + Kd_FuFixedPrem)) ; // use FUTUR CLAIM / FUTUR FIXED PREM / UPR - Raf
		}
		else
		{			
			d_ClmRed = d_CalculReducLossCorr( (char)( atoi( ptb_InRec_Cur[PER_LOSCOREXI_B] ) ),
		...
		// add syncro with output 1065  + add var d_taux for devise - Raf
		...
		// [004]
			...
			// [004]
			if ((I17 == 1 || Ksz_Type[0] == 'F') && atof(ptb_InRec_Cur[PER_LOSCORRAT_R]) > 0 && d_RetAmt)
				n_WriteGt( Gt, ptb_InRec_Cur, d_RetAmt) ;
			else if ( I17 != 1 && Ksz_Type[0] != 'F'&& ( d_RetAmt - Kd_Loss ))
      ...
      if ( tbd_PbPap[i].PBEX == 1 && tbd_PbPap[i].PAPEX == 0  && d_PbRetAmt != 0
        ...
        && (tbd_PbPap[i].CTBTYP_CT) != 4 )    // JR 29/06/2006
				fprintf( Kp_OutputFilCtrEst, "%s~%s~%s~%d~%d~%s~%s~%s~%d~%s~%s~%-.3f~%-.3f~%-.3f~%c~%s~%s~%s~~%s~~\n",
					pbd_InRec_Cur[PER_CTR_NF], pbd_InRec_Cur[PER_END_NT], pbd_InRec_Cur[PER_SEC_NF],
					Ktbd_EstGt[Kn_Par_Nbp + 1 + i].UWY_NF, Ktbd_EstGt[Kn_Par_Nbp + 1 + i].UW_NT, Ksz_Cre, Ksz_Prs,  // [005] "710",
					...
					d_PbRetAmt, Ktbd_EstGt[Kn_Par_Nbp + 1 + i].PBADMMOD_CT, Ksz_CloDat, sz_PbOriCod, "ESTC1019", Ksz_Cre ) ;
		}
...
int n_InitCtrEst( T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC1019_I6", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		return ERR ;
...
int n_InitGtCumTot(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC1019_I3", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		return ERR ;
...
int n_InitGtCum(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
	...
	/* ouverture du fichier esclave Gt selectionne sur sinistres */
	if ( n_OpenFileAppl( "ESTC1019_I5", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		return ERR ;
...
int n_InitGtPa(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC1019_I4", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		return ERR ;
...
int n_InitPerFci(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC1019_I2", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		return ERR ;
	...
	char **ptb_InRecChild ) /* adresse de la ligne de l'esclave */
{
	...
	if ( atoi( ptb_InRecOwner[PER_CTBTYP_CT] ) == 2 )
	{
		...
		if ( Kn_Fam_Nbp >= NB_FAM_MAX )
		{
			sprintf(sz_message, "Depassement de capacite du tableau Ktbd_FamCha, agrandir le nombre de postes NB_FAM_MAX dans ESTC1019.h et estserv.*");
...
int n_WriteCtrEst( char **CtrEst, char **ptb_InRec_Cur, double d_ClmRed, double d_RetAmt )
{
	...
	CtrEst[EST_PRS_CF] = Ksz_Prs ;  // [005]	"710" ;
...
int n_WriteGt( char **Gt, char **ptb_InRec_Cur, double d_Amt )
{
	...
	if (Ksz_Type[0] == 'F')
	{
		...
		if (I17 == 1) 
		{
			strcpy(sz_Trncod, "1120071");
			strcat(sz_Trncod, Kc_Norme_Suf);
		}
		else
		{
			strcpy(sz_Trncod, "1A200712");
		} 
		*/ 
		
		strcpy(sz_Trncod, "1A200712"); // [006]
		
		Gt[GT_ACMTRS_NT] = "3202";
	}
	/* calcul de la periode de compte et de l'annee de compte */
...
int n_InitUPR_DAC(T_RUPTURE_SYNC_VAR *pbd_Rupt)
{
	...
	memset(pbd_Rupt, 0, sizeof(T_RUPTURE_SYNC_VAR));
	/* ouverture du fichier esclave famille de charges iterees  */
	if (n_OpenFileAppl("ESTC1019_I11", "rt", &(pbd_Rupt->pf_InputFil)) == ERR)
		return ERR;
	/* nombre de rupture a gerer sur le fichier de travail */
	pbd_Rupt->n_NbRupture = 0;
	/* fonction du test de synchronisation de la ligne du maitre avec l'esclave */
...
int n_InitFutur(T_RUPTURE_SYNC_VAR *pbd_Rupt)
{
	...
	memset(pbd_Rupt, 0, sizeof(T_RUPTURE_SYNC_VAR));
	/* ouverture du fichier esclave famille de charges iterees  */
	if (n_OpenFileAppl("ESTC1019_I9", "rt", &(pbd_Rupt->pf_InputFil)) == ERR)
		return ERR;
	/* nombre de rupture a gerer sur le fichier de travail */
	pbd_Rupt->n_NbRupture = 0;
	/* fonction du test de synchronisation de la ligne du maitre avec l'esclave */
	...
	char **pbd_InRecChild  ) /* adresse de la ligne de l'esclave */
{
	...
	DEBUT_FCT( "n_ConditionSyncFutur" ) ; 
	if ((ret = strcmp( pbd_InRecOwner[PER_CTR_NF], pbd_InRecChild[GT_CTR_NF])) != 0)
		return ret ;
	if ((ret = strcmp( pbd_InRecOwner[PER_END_NT], pbd_InRecChild[GT_END_NT])) != 0)
		return ret ;
		
	//[008] if ((ret = strcmp( pbd_InRecOwner[PER_SEC_NF], pbd_InRecChild[GT_SEC_NF])) != 0) return ret ;
	
	if ( ( ret = atoi( pbd_InRecOwner[PER_SEC_NF] ) - atoi( pbd_InRecChild[GT_SEC_NF] ) ) != 0 ) 
		return ret ; 
...
int n_ActionLigneFutur(char **ptb_InRecOwner, char **pbd_InRecChild)
{
		if (!strcmp(pbd_InRecChild[GT_TRNCOD_CF], "1A100012"))
			Kd_FuFixedPrem += atof(pbd_InRecChild[GT_AMT_M]); //[008]Kd_FuFixedPrem += atol(pbd_InRecChild[GT_AMT_M]);
		if (!strcmp(pbd_InRecChild[GT_TRNCOD_CF], "1A494302"))
			Kd_FuClaim += atof(pbd_InRecChild[GT_AMT_M]);     //[008]Kd_FuClaim += atol(pbd_InRecChild[GT_AMT_M]);
	return 0;
}
