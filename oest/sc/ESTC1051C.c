...
[17] 21/12/2021 M.NAJI  :  SPIRA 101403  remplacment du chargment du CURQUOT en memoire fonction GetTaux
[18] 04/05/2022 Charles Socie : SPIRA 101998 :  EBS - RET pattern KO // Report du code du ESTC1051A.c
[19] 22/09/2022 JYP : SPIRA 106713: INI AE for Parent/Local
[20] 18/01/2023 JYP : SPIRA 107106: rework seg/lob rules
[21] 08/12/2023 JYP : SPIRA 110485 : exclude when PARM1 is empty
==============================================================================================================================================*/
...
/*----------------------------------------*/
static char VERSION_ESTC1051C_C[150] = "__version__: ESTC1051C.c version [021] 08/12/2023 SPIRA 110485 : exclude when PARM1 is empty" ;
...
char ErrorMessagePARM1[3000];

static double GetTaux( char * rate_Origine,  char * rate_Dest );
//static double GetTauxOld( char ssd_cf, short year, char* curency, char* targetcurrency );
...
#define GTPLUS_RATE       63
#define GTPLUS_RETRATE    64
#define GTPLUS_RETCTRCAT_CF 65
#define PER_PCPRATE     205
#define PER_EGPRATE     206
...
int main(int argc  , char *argv[])
{
	...
	strcpy(Ksz_norme_cf,psz_GetCharArgv(4));
	s_Prs = atoi(Ksz_Prs);
	...
	//if ( n_OpenFileAppl ( "ESTC1051C_I3","rt",&Kp_InputCurrency ) == ERR )
	//	ExitPgm( ERR_XX , "" );

	
	/* ouverture du fichier de sortie des resultats par affaire */
	if ( n_OpenFileAppl ( "ESTC1051C_O1","wt",&Kp_OutputFilResSii ) == ERR )
		ExitPgm( ERR_XX , "" );

	/* ouverture du fichier de sortie des erreur */
	if ( n_OpenFileAppl ( "ESTC1051C_O2","wt",&Kp_OutputFilErr ) == ERR )
		ExitPgm( ERR_XX , "" );

	//Chargement des CUR_CF dans un tableau
    //ChargementCurrency();

	//if ( n_CloseFileAppl( "ESTC1051C_I3", &Kp_InputCurrency ) == ERR ) 
	//	ExitPgm( ERR_XX , "" );
	...
	if ( n_CloseFileAppl( "ESTC1051C_I1", &( bd_RuptPerUw.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESTC1051C_I2", &( bd_RuptStatGta.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESTC1051C_O1", &Kp_OutputFilResSii ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESTC1051C_O2", &Kp_OutputFilErr ) == ERR )
		ExitPgm( ERR_XX , "" );
...
int n_InitPerUw(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTC1051C_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		return ERR;
...
int n_InitStatGta( T_RUPTURE_SYNC_VAR  *pbd_Rupt )
{
	...
	if ( n_OpenFileAppl( "ESTC1051C_I2", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		return ERR;
...
static double GetTaux( char * rate_Origine,  char * rate_Dest )
{
	...
	if ( b_IsBlankOrEmpty( rate_Origine) ||  b_IsBlankOrEmpty( rate_Dest) || atof(rate_Dest)  <= 0.  )
		d_Ratio = -1 ;
	else
		d_Ratio = atof(rate_Origine)/atof(rate_Dest) ;
...
//	char   sz_AcmTrs3[6];
	...
	char   sz_accret_I17PL[3]="AI"; // for internal retro I17PL 
  ...
  if (strcmp(ptb_InRecChild[GTPLUS_PARM1],"") == 0 
      && (
	     ...
	     || (*Ksz_Accret=='A' && (*ptb_InRecChild[GTSII_TRNCOD_CF]=='1'|| *ptb_InRecChild[GTSII_TRNCOD_CF]=='3')  )
	     )
     )
	{
		sprintf(buf, "%s", ptb_InRecChild[GT_SSD_CF]);
		for (j=GT_ESB_CF;j<=32;j++){
			  sprintf(buf, "%s~%s", buf, ptb_InRecChild[j]);
		}
		sprintf(ErrorMessagePARM1, "The pattern cannot be identifier as a ‘CL’aim or ‘PR’emium as PARM1 in the cash flow pattern mapping table(TPRSMAP)");
		fprintf(Kp_OutputFilErr, "%c:%s:%s~%s\n",*Ksz_Accret,ptb_InRecChild[GTPLUS_MAP_ACMTRS_NT],buf,ErrorMessagePARM1);
		RETURN_VAL(OK);		
	}	
	...
	|| strncmp(ptb_InRecChild[GTSII_ACMTRS3_NT2],"1010",4) == 0 || strncmp(ptb_InRecChild[GTSII_ACMTRS3_NT2],"2010",4) == 0 || strncmp(ptb_InRecChild[GTSII_ACMTRS3_NT2],"2013",4) == 0 || strncmp(ptb_InRecChild[GTSII_ACMTRS3_NT2],"2019",4) == 0){
		if (*ptb_InRecChild[GTSII_TRNCOD_CF] == '2' || *ptb_InRecChild[GTSII_TRNCOD_CF] == '4')
		{
			...
			if ( strcmp(Ksz_Accret, "R") == 0 && strcmp( ptb_InRecChild[GTSII_RETCUR_CF], sz_Cur ) != 0 )
			{
				...
				d_Ratio=GetTaux(ptb_InRecChild[GTPLUS_RETRATE],ptb_InRecOwner[PER_PCPRATE] );
			}		
		}
		else
		{
			...
			if ( strcmp( ptb_InRecChild[GTSII_CUR_CF], sz_Cur ) != 0 )
			{
				...
				d_Ratio=GetTaux(ptb_InRecChild[GTPLUS_RATE], ptb_InRecOwner[PER_EGPRATE]  );
			}
		}
	...
	if (*ptb_InRecChild[GTSII_TRNCOD_CF] == '2' || *ptb_InRecChild[GTSII_TRNCOD_CF] == '4' ) 
	{
		if (strncmp(ptb_InRecChild[GTPLUS_RETCTRCAT_CF] , "02", 2) == 0) 
		{
		//==========   retro - Lob  ==============================
			if ( strlen(trim(ptb_InRecOwner[PER_LOB_CF]))== 0 )
		...
		{ 
			...
			if (*Ksz_Accret=='R')
			{		
				if ( strlen(trim(ptb_InRecChild[GTSII_SEGLOB_CF]))== 0 )
					sprintf(sz_Seglob, "*%s",   ptb_InRecChild[GTSII_RTY_NF] );
				else 
...
//[18]	if ((*Ksz_Accret=='R') && (*sz_Nat=='P') && (*ptb_InRecChild[GTSII_TRNCOD_CF] == '2' || *ptb_InRecChild[GTSII_TRNCOD_CF] == '4' ))
	...
	if ( *ptb_InRecOwner[PER_CTRRET_B] == '1'  && strcmp(Ksz_Accret, "A") == 0 && strcmp(Ksz_norme_cf,"I17P") != 0 && strcmp(Ksz_norme_cf,"I17L") != 0    )
	{
		ptb_InRecChild[GTSII_ACMTRS_NT] = "1";
	}	 

    // rule retro interne, I17P I17L
	if ( *ptb_InRecOwner[PER_CTRRET_B] == '1'  && strcmp(Ksz_Accret, "A") == 0 && ( strcmp(Ksz_norme_cf,"I17P") == 0 || strcmp(Ksz_norme_cf,"I17L") == 0 ) && strcmp(ptb_InRecChild[GTSII_TYP_CT],"A") == 0 )
	{
	 ptb_InRecChild[GTSII_TYP_CT] = sz_accret_I17PL; 
	}	
