...
historique des modifications :
   <jj/mm/aaaa>   <auteur>    <description de la modification>
	...
	17/10/2008   JFVDV       SPOT 16230 Augmentation de la taille du tableau T_ACMTRS dans struct.h (recompilation)
==============================================================================*/
...
/*-------------------------------------------*/
/* SUPPRIME (modif le 20 01 98) : ces titres fixes sont pris */
/* en charge par Starjet */
...
int main(int argc ,char *argv[])
{
	...
	if (n_OpenFileAppl("ESTR203A_I2","rb",&Kp_SubsidFil) == ERR )
		ExitPgm ( ERR_XX , "" );

	if (n_OpenFileAppl("ESTR203A_I3","rb",&Kp_AcmtrshFil) == ERR )
		ExitPgm ( ERR_XX , "" );

	if (n_OpenFileAppl("ESTR203A_I4","rb",&Kp_BanteclFil) == ERR )
		ExitPgm ( ERR_XX , "" );

	if (n_OpenFileAppl("ESTR203A_I5","rb",&Kp_GrpFil) == ERR )
		ExitPgm ( ERR_XX , "" );
...
int n_ActionLigneAno(char **ptb_InRec_Cur)
{
	...
	fprintf(Kp_AnoFil,"%-9.9s%-3.3s%-3.3s%-4.4s%-4.4s%-4.4s%-20.20s\n",
...
int n_ActionFirstRuptPage (char **ptb_InRec_Cur)
{
	...
	if( n_RechSSD (&pbd_LibSsd,ptb_InRec_Cur[ANO_SSD_CF]) == OK )
          {
	       /* Libelles d'anomalie */
	 	n_RechBANTECL (&sz_lib, pbd_LibSsd->sz_LAG_CF,ptb_InRec_Cur[ANO_ANOCOD_CF]);
		...
		fprintf(Kp_AnoFil,"%-10.10s%-10.10s%-20.20s%04d%02d%c%-20.20s%10.10s%d\n",
			...
			Ksz_arr[b_Francais],Kn_page);
                 ...
                 fprintf(Kp_AnoFil,"%-3.3s%-32.32s\n",
         		ptb_InRec_Cur[ANO_ANOCOD_CF],sz_lib);
