...
int main(int argc, char **argv)
{
  ...
  if (n_OpenFileAppl("ESTC1057_O1", "wt", &Kp_OutputBatch)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du 1er fichier (ESCOMPTE)." );
  if (n_OpenFileAppl("ESTC1057_O2", "wt", &Kp_OutputRemainToPay)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du 2e fichier(REMAINTOPAY). " );
  if (n_OpenFileAppl("ESTC1057_O3", "wt", &Kp_OutputStats)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du 3e fichier(Statistiques). " );
  if (n_OpenFileAppl("ESTC1057_O4", "wt", &Kp_OutputErr)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du 4e fichier(NOSYNC). " );

  // Ouverture du fichier de cash flow ( pattern incrementales )
  if (n_OpenFileAppl("ESTC1057_I2", "rt", &Kp_InputPattern) == ERR )   ExitPgm(ERR_XX, "Problème lors de l'ouverture du fichier de cashflow." );
  if (n_OpenFileAppl("ESTC1057_I3", "rt", &Kp_InputCurrency) == ERR )   ExitPgm(ERR_XX, "Problème lors de l'ouverture du fichier de CUR_CF." );


  //Chargement des CUR_CF dans un tableau
  ChargementCurrency();
  // on peut fermer le fichier , les donnees sont dans pDevise
  if (n_CloseFileAppl("ESTC1057_I3", &Kp_InputCurrency))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier de Devise (CUR_CF).");
  //
  ...
  if (n_CloseFileAppl("ESTC1057_I1", &(Kbd_ruptRfrBatchIN.pf_InputFil)))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier d'input.");
  if (n_CloseFileAppl("ESTC1057_I2", &Kp_InputPattern))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier de Pattern Discount.");
  if (n_CloseFileAppl("ESTC1057_O1", &Kp_OutputBatch))          ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier ESCOMPTE.");
  if (n_CloseFileAppl("ESTC1057_O2", &Kp_OutputRemainToPay))    ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier REMAINTOPAY.");
  if (n_CloseFileAppl("ESTC1057_O3", &Kp_OutputStats))    ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier Statistiques.");
  if (n_CloseFileAppl("ESTC1057_O4", &Kp_OutputErr))    ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier NOSYNC.");
...
int n_InitRfrBatchIN(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTC1057_I1", "rt", &(pbd_Rupt->pf_InputFil)))
    return ERR;
...
int n_ActionLigneRfrBatchIN(char **ptb_InRec_Cur)
{
        ...
        for (col = 1; col <= CML_TYP_CT; col++) /* Updated for Phase1b Migration */
        {
          if (col == CML_ACMTRS_NT)
          {
            if (strlen(ptb_InRec_Cur[CML_ACMTRS_NT]) == 3 &&
                ...
                (ptb_InRec_Cur[CML_ACMTRS_NT][1] == '0' || ptb_InRec_Cur[CML_ACMTRS_NT][1] == '1'))
            {
  ...
  if (isOK == 0)
  {
    ...
    fprintf(Kp_OutputBatch, "%s~~~Pattern non trouvee~\n", buf) ;
  }
