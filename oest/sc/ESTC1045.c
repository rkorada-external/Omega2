...
------------------------------------------------------------------------------
description :
	Mise en forme du fichier SHERPA
...
int main(int argc ,char *argv[])
{
  ...
  strcpy(Ksz_inv,"0000");
  strcpy(Ksz_clodat, psz_GetCharArgv(3));
  ...
  if (n_CloseFileAppl("ESTC1045_I1",&(Kbd_Rupt.pf_InputFil)) == ERR)
    ExitPgm (ERR_XX ,"");
...
int n_InitPeop(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if (n_OpenFileAppl ("ESTC1045_I1","rt",&(pbd_Rupt->pf_InputFil)))
		RETURN_VAL (ERR); 
...
int n_Write_out(char **ptb_InRec_Cur) 
{
	...
	fprintf(Kp_GoelPeopOFil,"%-15s%-15s%-10s%-15s%-14s%15s\n",
				Ksz_inv,
