...
------------------------------------------------------------------------------
description :
   TRAITEMENT DES RECONDUCTIONS DU BILAN 1997 SUR L'ANCIEN SYSTEME - GTAA -
...
FILE 		*Kp_OutputFilPeri ; 	/* pointeur sur le fichier de sortie perimetre */
...
int main(int argc  , char *argv[])
{
	...
	if ( n_OpenFileAppl ( "ESTM2902_I3","rb",&Kp_InputFilAccTrsf ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	/* ouverture du fichier en entree FDETTRS */
	if ( n_OpenFileAppl ( "ESTM2902_I4","rb",&Kp_InputFilDetTrs ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	/* ouverture du fichier de sortie des reconductions */
	if ( n_OpenFileAppl ( "ESTM2902_O1","wt",&Kp_OutputFilRecond ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	/* ouverture du fichier de sortie perimetre */
	if ( n_OpenFileAppl ( "ESTM2902_O2","wt",&Kp_OutputFilPeri ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
	...
	if ( n_CloseFileAppl( "ESTM2902_I1", &( bd_RuptPer.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2902_I2", &( bd_RuptRecond.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2902_I3", &Kp_InputFilAccTrsf ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2902_I4", &Kp_InputFilDetTrs ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2902_O1", &Kp_OutputFilRecond ) == ERR )
		ExitPgm( ERR_XX , "" ) ;

	if ( n_CloseFileAppl( "ESTM2902_O2", &Kp_OutputFilPeri ) == ERR )
		ExitPgm( ERR_XX , "" ) ;
...
int n_InitPer(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl( "ESTM2902_I1", "rt", &( pbd_Rupt->pf_InputFil ) ) )
		RETURN_VAL(  ERR ) ;
...
int n_ActionLignePer( char **ptb_InRec_Cur )
{
	...
	/* mise a jour de la nature de l'affaire pour les traites */
	/************************************************************************/
	/* Explication :							*/
	/* lors de la descente du perimetre traite, la nature a ete forcee	*/
	/* a 'N'; celle-ci est mise a jour a 'P' si NAT_CF < 30			*/
	/************************************************************************/
	if ( *ptb_InRec_Cur[PER_CTRNAT_CT] == 'N' && atoi( ptb_InRec_Cur[PER_NAT_CF] ) < 30 )
		ptb_InRec_Cur[PER_CTRNAT_CT] = "P" ;

	/* reconduction du perimetre en sortie */
	n_WriteCols( Kp_OutputFilPeri, ptb_InRec_Cur, SEPARATEUR, 0 ) ;
...
int n_InitRecond( T_RUPTURE_SYNC_VAR  *pbd_Rupt )
{
	...
	if ( n_OpenFileAppl( "ESTM2902_I2", "rt", &( pbd_Rupt->pf_InputFil ) ) == ERR )
		RETURN_VAL (ERR )  ;
	...
	char **ptb_InRecChild ) /* adresse de la ligne de l'esclave */
{
	...
	char	sz_montant[30] ;	/* zone de travail */
	...
	/* si on ne trouve pas de postes dans le tableau */
	if ( i == -1 )
		RETURN_VAL( OK ) ;
	...
	{
		/* si IBNR filiale (issu de la retrocession interne) */
		if ( *ptb_InRecOwner[PER_CTRRET_B] == '1' )
			strcpy( sz_trncod, Ktbd_AccTrsf[i].IBNRSSDTRS_CF ) ;
		else	strcpy( sz_trncod, Ktbd_AccTrsf[i].PROPCRBTRS_CF ) ;

		/* annulation de l'ecriture en entree */
		sprintf( sz_montant, "%-.3f", ( -1 * atof( ptb_InRecChild[GT_AMT_M] ) ) ) ;
		...
		sprintf( sz_montant, "%-.3f", ( -1 * atof( sz_montant ) ) ) ;
	...
	if ( Ktbd_AccTrsf[i].RENV_B == 1 )
	{
		sprintf( MsgAno, "Renvoi : pas d'ecriture attendue pour le contrat ( %s ) et le poste ( %s )\n",
			ptb_InRecChild[GT_CTR_NF], ptb_InRecChild[GT_TRNCOD_CF] ) ;
		n_WriteAno( MsgAno ) ;
	...
	{
		/* FACS */
		if ( *ptb_InRecOwner[PER_CTRNAT_CT] == 'F' )
		{
