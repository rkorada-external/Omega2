...
historique des modifications :
   <jj/mm/aaaa>   <auteur>    <description de la modification>
	4/11/1999   Anb      Correction conversion aliment pour ventilation
	05/06/2014  M.MECHRI [001] Correction de traités non criblé sans traité de rattachement
==============================================================================*/
...
FILE	*Kp_GT_C_Fil,	/* pointeur sur le GT cimulé en sortie */
		...
		*Kp_GT_Log_Fil; /* fichier de log sur info contrat non criblés */
...
int n_PereSansFils_R    (char **ptb_InRecOwner);
...
int main(int argc ,char *argv[])
{
	...
	if ( n_OpenFileAppl ("ESTC2037_O3","wt",&Kp_GT_Log_Fil) == ERR )
		ExitPgm ( ERR_XX , "" );
	...
	if (n_CloseFileAppl ("ESTC2037_O3",&Kp_GT_Log_Fil))
		ExitPgm ( ERR_XX , "" );
...
int n_InitGT_R(T_RUPTURE_SYNC_VAR *pbd_Rupt)
{
	...
	/* fonction père sans fils pour initialiser correctement la devise */
	pbd_Rupt->n_PereSansFils	= n_PereSansFils_R ;
	...
	)
{
    ...
    if ((ret=strcmp(pbd_InRec[GT_ACY_NF],  // Ajout SBE
			pbd_InRec_Cur[GT_ACY_NF]))!=0) // Ajout SBE
        RETURN_VAL (ret);                  // Ajout SBE
...
int n_ActionLastCSACP( char **ptb_InRec_Cur)
{
    ...
    //[001]
    if( (strcmp(ptb_InRec_Cur[GT_ESTCTR_NF], blan)==0) || (strcmp(ptb_InRec_Cur[GT_ESTCTR_NF],"")==0))
    {
        ptb_InRec_Cur[GT_ESTCTR_NF] = "ESTCTRERR";
    } 
...
// A.B.J
    ...
    /***************************************************************/
    /* Ajout 7/01/98 : forcer le code crible a 'R' puisqu'on ecrit */
    /* dans le fichier des traites de rattachement */

	ptb_InRec_Cur[GT_ESTCRB_CT]   ="R";
    /***************************************************************/
...
{
	double f_montant,f_aliment,f_taux1, f_taux2;
	static char   sz_montant[25],sz_aliment[25];

	DEBUT_FCT("n_ActionLigneGT_N");

    /* Ecriture dans le fichier de log des infos contrats non criblés avant cumul et conversion */
    n_WriteCols(Kp_GT_Log_Fil,ptb_InRec_Cur,SEPARATEUR,0);
    
    // 30258
    if ( strcmp(ptb_InRec_Cur[GT_ESTCTR_NF],"         ") == 0 )
    {
        // Dans le cas où le contrat de rattachement est vide, on garde la devise du contrat ESTCUR
        strcpy(Ksz_CUR_CF,ptb_InRec_Cur[GT_ESTCUR_CF]);
    }
    
	/* Controle de l'AS */
	if (ptb_InRec_Cur[GT_COMACC_B][0]=='0') Kb_AS=0;

	/* Correction conversion aliment pour ventilation prévision */
	/* Anb le 4/11/1999 */

	/* Détermination du taux de conversion montant comptabilité / bilan du mouvement */
	f_taux1=d_GetTaux(Kp_CoursFil,
	...
	f_montant=atof(ptb_InRec_Cur[GT_ESTAMT_M])*f_taux1;


	/* Détermination du taux de conversion aliment / exercice de souscription */
	f_taux2=d_GetTaux(Kp_CoursFil,
						(char) atoi(ptb_InRec_Cur[GT_SSD_CF]),
						(short)atoi(ptb_InRec_Cur[GT_UWY_NF]),
						ptb_InRec_Cur[GT_ESTCUR_CF],
						Ksz_CUR_CF);

	/* Conversion de l'aliment */
	f_aliment=atof(ptb_InRec_Cur[GT_SCOEGP_M])*f_taux2;

	/* Mise à niveau de la monnaie */
	...
	/* Ecriture du traite non-crible en sortie */
	sprintf(sz_montant,"%.3lf",f_montant);
	ptb_InRec_Cur[GT_ESTAMT_M]   =sz_montant;

	sprintf(sz_aliment,"%.3lf",f_aliment);
...
)
{
	...
	// n_WriteCols(Kp_GT_C_Fil,ptb_InRecChild,SEPARATEUR,0); mis en commentaire JR le 05 11 03
...
/*==============================================================================
objet :
	fonction lancee si pas de GT de rattachement correspondant.
	Cela permet d'initialiser la variable devise correctement
retour :
	0K ----> traitement correctement effectue
	ERR --> probleme rencontre
==============================================================================*/
int n_PereSansFils_R(
	char **ptb_InRecOwner  /* adresse de la ligne du pere */
)
{
	DEBUT_FCT("n_PereSansFils_R");

    /* si accept GT_TRNCOD_CF[0] = 1 ou 3 : On prend la devise GT_ESTCUR_CF */
    /* si retro  GT_TRNCOD_CF[0] = 2 ou 4 : On prend la devise GT_RETCUR_CF */

    if ( ptb_InRecOwner[GT_TRNCOD_CF][0] == '1' || ptb_InRecOwner[GT_TRNCOD_CF][0] == '3' )
    {
	    strcpy(Ksz_CUR_CF,ptb_InRecOwner[GT_ESTCUR_CF]);
	}
	else 
    {
        strcpy(Ksz_CUR_CF,ptb_InRecOwner[GT_RETCUR_CF]);
    }
	RETURN_VAL (OK);
}
