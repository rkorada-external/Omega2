...
)
{
   ...
   if (n_OpenFileAppl("ESTC0604_I3", "rt", &Kp_GetTaux) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_OpenFileApplGetTaux");
   }

   /* Ouverture du fichier de sortie */
   if (n_OpenFileAppl("ESTC0604_O1", "wt", &Kp_OutputFile) == ERR) {
   ...
   if (n_CloseFileAppl("ESTC0604_I1", &(pbd_Rupture->pf_InputFil)) == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTC0604_I2", &(pbd_Sync->pf_InputFil)) == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTC0604_I3", &Kp_GetTaux) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTC0604_O1", &Kp_OutputFile) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_CloseFileAppl");
   }
...
)
{
   ...
   if (n_OpenFileAppl("ESTC0604_I1", "rt", &(pbd_Rupture->pf_InputFil)) == ERR) {
      RETURN_VAL(ERR);
   }
...
)
{
   ...
   if (n_OpenFileAppl("ESTC0604_I2", "rt", &(pbd_Sync->pf_InputFil)) == ERR) {
      RETURN_VAL(ERR);
   }
...
int n_ActionDerniereRupture1(char *ptsz_LigneCour[])
{
   ...
   if ( (Ks_Analyse == 1) && (Ks_SEGNUL == 0) ) {
      ...
      Ktsz_Sortie[0] = Kptsz_LigneEsclave[SEGEST1_SSD_CF];
      Ktsz_Sortie[1] = Kptsz_LigneEsclave[SEGEST1_SEG_NF];
      Ktsz_Sortie[2] = Kptsz_LigneEsclave[SEGEST1_UWY_NF];
      Ktsz_Sortie[3] = Kptsz_LigneEsclave[SEGEST1_CUR_CF];
      Ktsz_Sortie[4] = Kptsz_LigneEsclave[SEGEST1_SEGNAT_CT];
      Ktsz_Sortie[5] = sz_Ss_M;
      Ktsz_Sortie[6] = sz_SP_R;
      Ktsz_Sortie[7] = sz_SP_CT;
      Ktsz_Sortie[8] = NULL;
...
int n_ActionPereSansFils(char *ptsz_LigneMaitre[])
{
   ...
   if (Kc_SEGTYP_CT == 'E') {
      sprintf (Ksz_MessageErr, "SEG %s, UWY %s has no estimates in database",
               ptsz_LigneMaitre[CASEEST_SEG_NF],
               ptsz_LigneMaitre[CASEEST_UWY_NF]
      );
   }
   else {
      sprintf (Ksz_MessageErr, "SEG %s, UWY %s has no estimates in database",
