...
description :
	...
	de la table TRFCROSSREF, la 2eme celui de TFACCROSSREF
...
//  n_parm_SITE = atoi(psz_GetCharArgv(1));       /* indique si on est sur site emetteur ou recepteur pour TRFCROSSREF  */

	if (n_LocalConnect (&bd_utctlib) != CS_SUCCEED)
		ExitPgm (ERR_XX, "");


	if (n_OpenFileAppl ("ESTX7016_O1","wb",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7016_O2","wb",&Kp_crostrfa) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_Processing (&bd_utctlib) != CS_SUCCEED)
		ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7016_O1",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7016_O2",&Kp_crostrfa) == ERR )
           ExitPgm (ERR_XX, "");
