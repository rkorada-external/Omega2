...
nom du source                 : ESTX7001.c
revision                      : $Revision: 1.2 $
date de creation              : 15/10/1999
auteur                        : J.Ribot
references des specifications : transfert portefeuille - partie GT
...
/*----------------------*/
FILE *Kp_crostrf, *Kp_crostclm, *Kp_dettrs;
   ...
   )
{
	...
	if (n_OpenFileAppl ("ESTX7001_O1","wb",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7001_O2","wb",&Kp_crostclm) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7001_O3","wb",&Kp_dettrs) == ERR )
		ExitPgm (ERR_XX, "");

	if (n_Processing (&bd_utctlib) != CS_SUCCEED)
		ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7001_O1",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7001_O2",&Kp_crostclm) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7001_O3",&Kp_dettrs) == ERR )
		ExitPgm (ERR_XX, "");
