...
------------------------------------------------------------------------------
 Historique des modifications :
   <jj/mm/aaaa>   <auteur>    <description de la modification>
    ...
    19/03/2009   J. Ribot    SPOT 17106   ajout test filiale 2 et suppresion test etab 3
---------------
MODIFICATION   : [003]
Auteur         : D.GATIBELZA
Date           : 29/03/2010
...
int main(int argc, char **argv)
{
  ...
  if (n_OpenFileAppl("ESTM2562_O1", "wt", &Kp_OutputFileMGTAR) == ERR) ExitPgm(ERR_XX ,"");

  /* Lancement du traitement du fichier Maitre */
  if (n_ProcessingRuptureVar(&Kbd_ruptIADVPERICASE) == ERR) ExitPgm(ERR_XX, "");

  /* Fermeture des fichiers ouverts */
  if (n_CloseFileAppl("ESTM2562_I1", &(Kbd_ruptIADVPERICASE.pf_InputFil)) == ERR) ExitPgm(ERR_XX, "");
  if (n_CloseFileAppl("ESTM2562_I2", &(Kbd_ruptMGTAR_SORT.pf_InputFil)) == ERR) ExitPgm(ERR_XX, "");

  if (n_CloseFileAppl("ESTM2562_O1", &Kp_OutputFileMGTAR)) ExitPgm(ERR_XX, "");
...
int n_InitIADVPERICASE(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTM2562_I1","rt", &(pbd_Rupt->pf_InputFil)))
    return ERR;
...
int n_InitMGTAR_SORT(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTM2562_I2","rt", &(pbd_Rupt->pf_InputFil)))
    return ERR;
...
int n_ActionLigneSyncMGTAR_SORT(char **ptb_InRecOwner, char **ptb_InRecChild)
{
   ...
   n_PerEsb = atoi(ptb_InRecOwner[PER_ACCESB_CF]);
   strcpy( sz_GtDbltrn, ptb_InRecChild[GT_DBLTRNCOD_CF] );



   /* tester etablist acceptation == (2 ou 3) */
//   if ( (n_PerEsb==2) || (n_PerEsb==3) )
// SPOT 17106   ajout test filiale 2 et suppresion test etab 3
// [19210] - Filtre sur les postes financiers et de depots
                ...
                && (ptb_InRecChild[GT_TRNCOD_CF][2] != '8') )      //
   {
    ...
    /*[003] tester filiale 20, 22 ou filiale 4 et etab ==2 */
    if ( strcmp (ptb_InRecChild[GT_SSD_CF]  ,"20")== 0                      ||
         ( strcmp (ptb_InRecChild[GT_SSD_CF], "4")== 0 && n_PerEsb==2 )     ||
