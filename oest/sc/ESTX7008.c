   ...
   )
{
	...
	if (n_OpenFileAppl ("ESTX7008_O1","wb",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7008_O2","wb",&Kp_crostclm) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7008_O3","wb",&Kp_dettrs) == ERR )
		ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7008_O4","wb",&Kp_crostrfa) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_Processing (&bd_utctlib) != CS_SUCCEED)
		ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7008_O1",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7008_O2",&Kp_crostclm) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7008_O3",&Kp_dettrs) == ERR )
		ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7008_O4",&Kp_crostrfa) == ERR )
           ExitPgm (ERR_XX, "");
...
CS_RETCODE n_Processing (T_UTCTLIB  *pbd_utctlib)
{
	...
	retcode = n_ExecCmdWithFetch(pbd_utctlib,"Select distinct T1.CTR_NF,      "\
                                    ...
                                    "      where T1.TRFSTS_CT = 3    "\
                                    "       and T1.TRFACCSTS_CT = 34   ",
				   n_retcFetchRowTRF) ;
