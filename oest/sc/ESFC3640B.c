...
int main( int argc,  char *argv[] )
{
	...
	if ( n_OpenFileAppl ( "ESFC3640B_O1","wt",&Kp_OutputFilRatio ) == ERR )
		ExitPgm( ERR_XX , "" );
	...
	/***************************************************************************/
	if ( n_CloseFileAppl( "ESFC3640B_I1", &( Kbd_Rup_RATIO.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3640B_O1", &Kp_OutputFilRatio ) == ERR ) 
		ExitPgm( ERR_XX , "" );
...
int n_Init_RATIO(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl ("ESFC3640B_I1", "rt", &(pbd_Rupt->pf_InputFil)))
		RETURN_VAL (ERR);
...
int n_IsRupt_RATIO(char **ptb_InRec, char **ptb_InRec_Cur)
{
	...
	if ( ( ret = strcmp( ptb_InRec[A_CTR_NF], ptb_InRec_Cur[A_CTR_NF] ) ) != 0 ) 		return ret;
	if ( ( ret = atoi( ptb_InRec[A_SEC_NF] ) - atoi( ptb_InRec_Cur[A_SEC_NF] ) ) != 0 ) return ret;
	if ( ( ret = strcmp( ptb_InRec[A_UWY_NF], ptb_InRec_Cur[A_UWY_NF] ) ) != 0 ) 		return ret;
	if ( ( ret = atoi( ptb_InRec[A_UW_NT] ) - atoi( ptb_InRec_Cur[A_UW_NT] ) ) != 0 ) 	return ret;
...
int n_ActionLastRupt_RATIO( char **pbd_InRecOwner )
{
	...
	//only when CSUO ( Contract / section / UWY / Order ) more than one
	if ( n_RuptCount > 1 ) {
	...
	fprintf(Kp_OutputFilRatio,
			...
			pbd_InRecOwner[A_CTR_NF],
			"", //pbd_InRecOwner[A_END_NT], 
			pbd_InRecOwner[A_SEC_NF], 
			pbd_InRecOwner[A_UWY_NF], 
			pbd_InRecOwner[A_UW_NT],
			EGPI_R1,
			EGPI_R2,
			EARP_R1,
			d_FUTURE_PREM_PREVQ,
...
int n_ActionLigne_RATIO( char **pbd_InRecOwner )
{
	DEBUT_FCT("n_ActionLigne_RATIO");
		...
		d_FUTURE_PREM_PREVQ += atof(pbd_InRecOwner[A_FUTURE_PREM_PREVQ]);
		d_FUTURE_PREM_Q += atof(pbd_InRecOwner[A_FUTURE_PREM_Q]);
		d_PREM_ESTM_PREVQ += atof(pbd_InRecOwner[A_PREM_ESTM_PREVQ]);
		d_PREM_ESTM += atof(pbd_InRecOwner[A_PREM_ESTM]);
		d_REMAIN_ESTM_PREVQ += atof(pbd_InRecOwner[A_REMAIN_ESTM_PREVQ]);
		d_REMAIN_ESTM += atof(pbd_InRecOwner[A_REMAIN_ESTM]);
		d_FIXED_CHARGE_ACT += atof(pbd_InRecOwner[A_FIXED_CHARGE_ACT]);
		d_ITD_PREM_ACT += atof(pbd_InRecOwner[A_ITD_PREM_ACT]);
		d_ITD_PREM += atof(pbd_InRecOwner[A_ITD_PREM]);
		d_UPR_PREVQ += atof(pbd_InRecOwner[A_UPR_PREVQ]);
		d_UPR_Q += atof(pbd_InRecOwner[A_UPR_Q]);
