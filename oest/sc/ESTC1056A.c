...
char    context_ct[4];        // [010] pour récupérer le Context passé en troisième paramètre
char    closing_date[9];        // [010] pour récupérer la closing date passé en quatrième paramètre
...
int main(int argc, char **argv)
{
  ...
  // [010] Pour récupérer le contect et la closing date passé en 3eme et 4eme paramètre
   if (Ksz_Argc == 4)
  { 
...
// A DECOMMENTER POUR RECUPERATION DU TRIMESTRE PASSE EN PARAMETRE
  ...
  if (n_OpenFileAppl("ESTC1056A_O1", "wt", &Kp_OutputFileRfrBatchOut)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du 1er fichier d'output." );
  if (n_OpenFileAppl("ESTC1056A_O2", "wt", &Kp_OutputPatternBatchOut)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du 2e fichier d'output." );
  if (n_OpenFileAppl("ESTC1056A_O3", "wt", &Kp_OutputFileRMNTP)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du 3e fichier d'output." );
  if (n_OpenFileAppl("ESTC1056A_O4", "wt", &Kp_OutputFileRTPFWH)) ExitPgm(ERR_XX, "Problème lors de l'ouverture du 4e fichier d'output." );
 
  // Ouverture du fichier de cash flow ( pattern incrementales )
  if (n_OpenFileAppl("ESTC1056A_I2", "rt", &Kp_InputCashFlow) == ERR) ExitPgm(ERR_XX, "Problème lors de l'ouverture du fichier de cashflow." );
  
   // Ouverture du fichier de pericase 
  if (n_OpenFileAppl("ESTC1056A_I3", "rt", &Kp_InputPericase) == ERR) ExitPgm(ERR_XX, "Problème lors de l'ouverture du fichier de pericase." );

   // Ouverture du fichier de pericase retro
  if (n_OpenFileAppl("ESTC1056A_I4", "rt", &Kp_InputPericaseRetro) == ERR) ExitPgm(ERR_XX, "Problème lors de l'ouverture du fichier de pericase retro." );
...
//  ExtractLineCSWithNoSync();
  ...
  if (n_CloseFileAppl("ESTC1056A_I1", &(Kbd_ruptRfrBatchIN.pf_InputFil)))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier d'input.");

  if (n_CloseFileAppl("ESTC1056A_I2", &Kp_InputCashFlow))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier de cash flow.");
  
  if (n_CloseFileAppl("ESTC1056A_I3", &Kp_InputPericase))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier de pericase.");

  if (n_CloseFileAppl("ESTC1056A_I4", &Kp_InputPericaseRetro))  ExitPgm(ERR_XX, "Problème lors de la fermeture du fichier de pericase retro.");

  if (n_CloseFileAppl("ESTC1056A_O1", &Kp_OutputFileRfrBatchOut))          ExitPgm(ERR_XX, "Problème lors de la fermeture du 1er fichier d'output.");

  if (n_CloseFileAppl("ESTC1056A_O2", &Kp_OutputPatternBatchOut))          ExitPgm(ERR_XX, "Problème lors de la fermeture du 2e fichier d'output.");

  if (n_CloseFileAppl("ESTC1056A_O3", &Kp_OutputFileRMNTP))              ExitPgm(ERR_XX, "Problème lors de la fermeture du 3e fichier d'output.");

  if (n_CloseFileAppl("ESTC1056A_O4", &Kp_OutputFileRTPFWH))              ExitPgm(ERR_XX, "Problème lors de la fermeture du 4e fichier d'output.");
...
int n_InitRfrBatchIN(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTC1056A_I1", "rt", &(pbd_Rupt->pf_InputFil)))
    return ERR;
