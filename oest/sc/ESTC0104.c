...
Historique des modifications :
   <jj/mm/aaaa>   <auteur>    <description de la modification>
    ...
    11/10/2005    J. Ribot    SPOT11506 - maj CTBCOM_B sur chacune des lignes et pas uniquement
                                          sur la premiere
...
///   MIS en commentaire JR 11/10/2005
///
///
///
///
...
///   RETURN_VAL(OK);
///}
///  FIN MIS en commentaire JR 11/10/2005
      ...
      if (Ks_PremiereSection == 1) {
         if (Kn_NbreLignesSection == MAX_LIGNES) {
            sprintf(Ksz_MessageErr, "CTR %s, END %s, SEC %s, UWY %s, UW %s : maximum number of lines in FFAMCHG2 file is reached ; increase MAX_LIGNES value",
              ...
              /* Modif (28/01/98) : le message est ecrit dans le .ano */
              /* du programme et plus dans le .log */
              /* n_WriteLog('E', Ksz_MessageErr);*/

              n_WriteAno(Ksz_MessageErr);

              /* et 'plantage' volontaire du job appelant le programme */
              Kb_ReturnStatus=1;
         }
         else {
