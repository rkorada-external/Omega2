/*==============================================================================
 ...
 Nom du source                 : ESTM2567.c
 Revision                      : $Revision: 1.2 $
 Date de creation              : 11/07/2007
 Auteur                        : gensource v2.0 (auto)
 References des specifications : ######################
 Squelette de base             : batch
------------------------------------------------------------------------------
  Description :
   Generation de MGTAR   SPOT13142
...
int main(int argc, char **argv)
{
  ...
  if (n_OpenFileAppl("ESTM2567_O1", "wt", &Kp_OutputFileMGTAR) == ERR) ExitPgm(ERR_XX ,"");

  /* Lancement du traitement du fichier Maitre */
  if (n_ProcessingRuptureVar(&Kbd_ruptIRDVPERICASE) == ERR) ExitPgm(ERR_XX, "");

  /* Fermeture des fichiers ouverts */
  if (n_CloseFileAppl("ESTM2567_I1", &(Kbd_ruptIRDVPERICASE.pf_InputFil)) == ERR) ExitPgm(ERR_XX, "");
  if (n_CloseFileAppl("ESTM2567_I2", &(Kbd_ruptMGTAR_SORT.pf_InputFil)) == ERR) ExitPgm(ERR_XX, "");

  if (n_CloseFileAppl("ESTM2567_O1", &Kp_OutputFileMGTAR)) ExitPgm(ERR_XX, "");
...
int n_InitIRDVPERICASE(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTM2567_I1","rt", &(pbd_Rupt->pf_InputFil)))
    return ERR;
...
int n_InitMGTAR_SORT(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl("ESTM2567_I2","rt", &(pbd_Rupt->pf_InputFil)))
    return ERR;
