...
int main( int argc,  char *argv[] )
{
	...
	if ( n_OpenFileAppl ( "ESFC3633_O1","wt",&Kp_OutputBatch ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	/** Ouverture du fichier en entree des cours de change FCURQUOT */
	if ( n_OpenFileAppl ( "ESFC3633_I4","rb",&Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );
	...
	/***************************************************************************/
	if ( n_CloseFileAppl( "ESFC3633_I1", &( pbd_Rupture.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3633_I2", &( Kbd_Rupt_CASHFLOW.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3633_I3", &( Kbd_RuptDLGTAAPNAE.pf_InputFil ) ) == ERR ) 
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3633_I4", &Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3633_O1", &Kp_OutputBatch ) == ERR ) 
		ExitPgm( ERR_XX , "" );
...
int n_InitRupture(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl ("ESFC3633_I1", "rt", &(pbd_Rupt->pf_InputFil)))
		RETURN_VAL (ERR);
...
int n_ActionLigneMaitre( char **ptb_InRec_Cur )
{
	...
	{
		ptb_InRec_Cur[PER_FLAG] = "YES";
		n_WriteCols(Kp_OutputBatch,  ptb_InRec_Cur, '~', 0);
	}
	else
	{
		ptb_InRec_Cur[PER_FLAG] = "NO";
		n_WriteCols(Kp_OutputBatch,  ptb_InRec_Cur, '~', 0);
	}
...
int n_InitSync(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3633_I3", "rt", &(pbd_Sync->pf_InputFil));
...
int n_ActionLigneSync( char **pbd_InRecOwner , char **pbd_InRecChild )
{
	...
	if ( strcmp( pbd_InRecOwner[PER_EGPCUR_CF], pbd_InRecChild[GTSII_ACMCUR_CF] ) != 0 )
	{	
...
int n_Init_CASHFLOW(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3633_I2", "rt", &(pbd_Sync->pf_InputFil));
...
int n_ActionLigne_CSF( char **pbd_InRecOwner , char **pbd_InRecChild )
{
	...
	d_FUTURE_PREM += atof(pbd_InRecChild[CML_ACMAMT_MC]);
