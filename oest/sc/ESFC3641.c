...
HISTORIQUE DES MODIFICATIONS :
	...
	13/08/2021    	LEL    		97373    	DEVELOPPEMENT DE LA VERSION INITIALE
...
int main( int argc,  char *argv[] )
{
	...
	if ( n_OpenFileAppl ( "ESFC3641_O1","wt",&Kp_OutputFilRatio ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	/* Ouverture du fichier en entree des cours de change FCURQUOT */
	if ( n_OpenFileAppl ( "ESFC3641_I7","rb",&Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );
	...
	/***************************************************************************/
	if ( n_CloseFileAppl( "ESFC3641_I1", &( Kbd_Rup_TRERETFACCTR.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3641_I2", &( Kbd_Rupt_CASHFLOW_Q.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3641_I3", &( Kbd_Rupt_CASHFLOW_PREVQ.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3641_I4", &( Kbd_Rupt_ITD_PREM.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESFC3641_I5", &( Kbd_Rupt_UPR_Q.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3641_I6", &( Kbd_Rupt_UPR_PREVQ.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3641_I7", &Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3641_I8", &( Kbd_Rupt_CASHFLOW_INI.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3641_O1", &Kp_OutputFilRatio ) == ERR ) 
		ExitPgm( ERR_XX , "" );
...
int n_Init_TRERETFACCTR(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl ("ESFC3641_I1", "rt", &(pbd_Rupt->pf_InputFil)))
		RETURN_VAL (ERR);
...
int n_IsRupt_TRERETFACCTR(char **ptb_InRec, char **ptb_InRec_Cur)
{
	...
	if ( ( ret = strcmp( ptb_InRec[RETCTR_NF], ptb_InRec_Cur[RETCTR_NF] ) ) != 0 ) 			return ret;
	if ( ( ret = atoi( ptb_InRec[RETEND_NT] ) - atoi( ptb_InRec_Cur[RETEND_NT] ) ) != 0 ) 	return ret;
	if ( ( ret = atoi( ptb_InRec[RETSEC_NF] ) - atoi( ptb_InRec_Cur[RETSEC_NF] ) ) != 0 ) 	return ret;
	if ( ( ret = strcmp( ptb_InRec[RTY_NF], ptb_InRec_Cur[RTY_NF] ) ) != 0 ) 				return ret;
	if ( ( ret = atoi( ptb_InRec[RETUW_NT] ) - atoi( ptb_InRec_Cur[RETUW_NT] ) ) != 0 ) 	return ret;
	if ( ( ret = atoi( ptb_InRec[PLC_NT] ) - atoi( ptb_InRec_Cur[PLC_NT] ) ) != 0 ) 		return ret;
...
int n_ActionLigne_TRERETFACCTR( char **pbd_InRecOwner )
{
	...
	fprintf(Kp_OutputFilRatio,
			"%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%.5f~%.5f~%.5f~%.3f~%.3f~%.3f~%.3f~%.3f~%.3f~%.3f~%.3f~%.3f~%.3f~%.3f~%s \n", 
			pbd_InRecOwner[CTR_NF],
			pbd_InRecOwner[END_NT], 
			pbd_InRecOwner[SEC_NF], 
			pbd_InRecOwner[UWY_NF], 
			pbd_InRecOwner[UW_NT],
			pbd_InRecOwner[RETCTR_NF],
			pbd_InRecOwner[RETEND_NT], 
			pbd_InRecOwner[RETSEC_NF], 
			pbd_InRecOwner[RTY_NF], 
			pbd_InRecOwner[RETUW_NT],
			pbd_InRecOwner[PLC_NT],
			EGPI_R1,
			EGPI_R2,
			EARP_R1,
			d_FUTURE_PREM_PREVQ,
...
int n_Init_CASHFLOW_Q(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3641_I2", "rt", &(pbd_Sync->pf_InputFil));
...
int n_CondSync_CASHFLOW_Q( char **pbd_InRecOwner , char **pbd_InRecChild )
{
	...
	if ( ( ret = strcmp( pbd_InRecOwner[RETCTR_NF], pbd_InRecChild[CML_RETCTR_NF] ) ) != 0 ) 		return ret;
	if ( ( ret = atoi( pbd_InRecOwner[RETEND_NT] ) - atoi( pbd_InRecChild[CML_RETEND_NT] ) ) != 0 ) return ret;
	if ( ( ret = atoi( pbd_InRecOwner[RETSEC_NF] ) - atoi( pbd_InRecChild[CML_RETSEC_NF] ) ) != 0 ) return ret;
	if ( ( ret = strcmp( pbd_InRecOwner[RTY_NF], pbd_InRecChild[CML_RTY_NF] ) ) != 0 ) 				return ret;
	if ( ( ret = atoi( pbd_InRecOwner[RETUW_NT] ) - atoi( pbd_InRecChild[CML_RETUW_NT] ) ) != 0 ) 	return ret;
	if ( ( ret = atoi( pbd_InRecOwner[PLC_NT]) - atoi( pbd_InRecChild[CML_PLC_NT] )) != 0 ) 		return ret;
...
int n_ActionLigne_CASHFLOW_Q( char **pbd_InRecOwner ,char **pbd_InRecChild )
{
	...
	{	
		d_taux = d_GetTaux( Kp_InputFilExc, 
							...
							atoi(Blcshyear), //atoi( pbd_InRecChild[CML_BALSHEY_NF] ), 
...
int n_Init_CASHFLOW_PREVQ(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3641_I3", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_CASHFLOW_INI(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3641_I8", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_ITD_PREM(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3641_I4", "rt", &(pbd_Sync->pf_InputFil));
...
int n_CondSync_ITD_PREM( char **pbd_InRecOwner , char **pbd_InRecChild )
{
	...
	if ( ( ret = strcmp( pbd_InRecOwner[RETCTR_NF], pbd_InRecChild[GT_RETCTR_NF] ) ) != 0 ) 		return ret;
	if ( ( ret = atoi( pbd_InRecOwner[RETEND_NT] ) - atoi( pbd_InRecChild[GT_RETEND_NT] ) ) != 0 ) 	return ret;
	if ( ( ret = atoi( pbd_InRecOwner[RETSEC_NF] ) - atoi( pbd_InRecChild[GT_RETSEC_NF] ) ) != 0 ) 	return ret;
	if ( ( ret = strcmp( pbd_InRecOwner[RTY_NF], pbd_InRecChild[GT_RTY_NF] ) ) != 0 ) 				return ret;
	if ( ( ret = atoi( pbd_InRecOwner[RETUW_NT] ) - atoi( pbd_InRecChild[GT_RETUW_NT] ) ) != 0 ) 	return ret;
	if ( ( ret = atoi( pbd_InRecOwner[PLC_NT]) - atoi( pbd_InRecChild[GT_PLC_NT] )) != 0 ) 			return ret;
...
int n_Init_UPR_Q(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3641_I5", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_UPR_PREVQ(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3641_I6", "rt", &(pbd_Sync->pf_InputFil));
