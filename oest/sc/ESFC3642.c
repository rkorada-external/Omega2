...
int main( int argc,  char *argv[] )
{
	...
	if ( n_OpenFileAppl ( "ESFC3642_O1","wt",&Kp_OutputFilRatio ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	/* Ouverture du fichier en entree des cours de change FCURQUOT */
	if ( n_OpenFileAppl ( "ESFC3642_I7","rb",&Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );
	...
	/***************************************************************************/
	if ( n_CloseFileAppl( "ESFC3642_I1", &( Kbd_Rup_TRERETFACCTR.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3642_I2", &( Kbd_Rupt_CASHFLOW_Q.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3642_I3", &( Kbd_Rupt_CASHFLOW_PREVQ.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3642_I4", &( Kbd_Rupt_ITD_PREM.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );

	if ( n_CloseFileAppl( "ESFC3642_I5", &( Kbd_Rupt_UPR_Q.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3642_I6", &( Kbd_Rupt_UPR_PREVQ.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3642_I7", &Kp_InputFilExc ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3642_I8", &( Kbd_Rupt_CASHFLOW_INI.pf_InputFil ) ) == ERR )
		ExitPgm( ERR_XX , "" );
	
	if ( n_CloseFileAppl( "ESFC3642_O1", &Kp_OutputFilRatio ) == ERR ) 
		ExitPgm( ERR_XX , "" );
...
int n_Init_TRERETFACCTR(T_RUPTURE_VAR  *pbd_Rupt)
{
	...
	if ( n_OpenFileAppl ("ESFC3642_I1", "rt", &(pbd_Rupt->pf_InputFil)))
		RETURN_VAL (ERR);
...
int n_Init_CASHFLOW_Q(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3642_I2", "rt", &(pbd_Sync->pf_InputFil));
...
int n_CondSync_CASHFLOW_Q( char **pbd_InRecOwner , char **pbd_InRecChild )
{
	...
	if ( ( ret = strcmp( pbd_InRecOwner[CTR_NF], pbd_InRecChild[CML_RETCTR_NF] ) ) != 0 ) 		return ret;
	if ( ( ret = atoi( pbd_InRecOwner[END_NT]) - atoi(pbd_InRecChild[CML_RETEND_NT])) != 0 ) 	return ret;
	if ( ( ret = atoi( pbd_InRecOwner[SEC_NF]) - atoi( pbd_InRecChild[CML_RETSEC_NF])) != 0)	return ret;
	if ( ( ret = strcmp( pbd_InRecOwner[UWY_NF], pbd_InRecChild[CML_RTY_NF] ) ) != 0 ) 			return ret;
...
int n_Init_CASHFLOW_PREVQ(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3642_I3", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_CASHFLOW_INI(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	n_OpenFileAppl ("ESFC3642_I8", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_ITD_PREM(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3642_I4", "rt", &(pbd_Sync->pf_InputFil));
...
int n_CondSync_ITD_PREM( char **pbd_InRecOwner , char **pbd_InRecChild )
{
	...
	if ( ( ret = strcmp( pbd_InRecOwner[CTR_NF], pbd_InRecChild[GT_RETCTR_NF] ) ) != 0 ) 		return ret;
	if ( ( ret = atoi( pbd_InRecOwner[END_NT] ) - atoi( pbd_InRecChild[GT_RETEND_NT] ) ) != 0 ) return ret;
	if ( ( ret = atoi( pbd_InRecOwner[SEC_NF] ) - atoi( pbd_InRecChild[GT_RETSEC_NF] ) ) != 0 ) return ret;
	if ( ( ret = strcmp( pbd_InRecOwner[UWY_NF], pbd_InRecChild[GT_RTY_NF] ) ) != 0 ) 			return ret;
...
int n_Init_UPR_Q(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3642_I5", "rt", &(pbd_Sync->pf_InputFil));
...
int n_Init_UPR_PREVQ(T_RUPTURE_SYNC_VAR  *pbd_Sync)
{
	...
	memset( pbd_Sync, 0, sizeof(T_RUPTURE_SYNC_VAR) ) ;
	n_OpenFileAppl ("ESFC3642_I6", "rt", &(pbd_Sync->pf_InputFil));
