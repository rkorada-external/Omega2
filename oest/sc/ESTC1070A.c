...
------------------------------------------------------------------------------
  Description : Ratios ULAE appliqu�s aux montants des aggr�gats 
------------------------------------------------------------------------------
...
[002] 03/09/2018 Charles Socie : EXT-IFRS17-903121  REQ 10.02 Cash flow: more detailed granularity ( split between variable and fixed premiums)
[003] 26/08/2020 HR : SPIRA 82685 : struct.h
[003] 20/03/2024 DAD : spira 110913 : add CTRNAT_CT, UWY_NF, LOBN2_NF on TestRupture and ConditionSync(
...
int main(int argc, char **argv)
{
    ...
    if (n_OpenFileAppl("ESTC1070A_O1", "wt", &Kp_OutputBatch)== ERR ) 
					ExitPgm(ERR_XX, "Probl�me lors de l'ouverture 1er fichier (cumul)." );
    if (n_OpenFileAppl("ESTC1070A_O2", "wt", &Kp_OutputLog) ==ERR ) 
					ExitPgm(ERR_XX, "Probl�me lors de l'ouverture du 2e fichier(logs). " );

    // Initialisation des variables de gestion de ruptures
    if (n_InitRupture(pbd_Rupture)== ERR ) 
							ExitPgm(ERR_XX, "Probl�me lors de l'ex�cution de la m�thode pdb_Rupture");
   ...
   if (n_ProcessingRuptureVar(pbd_Rupture) == ERR) 
	...
	if (n_CloseFileAppl("ESTC1070A_I2", &(pbd_Rupture->pf_InputFil)) == ERR)  
								ExitPgm(ERR_XX, "Probl�me lors de la fermeture du fichier d'ULAE.");
	if (n_CloseFileAppl("ESTC1070A_I1", &(pbd_Sync->pf_InputFil)) == ERR)  
								ExitPgm(ERR_XX, "Probl�me lors de la fermeture du fichier de cumul.");
    if (n_CloseFileAppl("ESTC1070A_O1", &Kp_OutputBatch) == ERR)          
								ExitPgm(ERR_XX, "Probl�me lors de la fermeture du fichier de cumul.");
    if (n_CloseFileAppl("ESTC1070A_O2", &Kp_OutputLog) ==ERR)     
								ExitPgm(ERR_XX, "Probl�me lors de la fermeture du fichier log Ledgers.");
...
/* Ouverture du fichier maitre */
   if (n_OpenFileAppl("ESTC1070A_I2", "rt", &(pbd_Rupture->pf_InputFil)) == ERR) {
      RETURN_VAL(ERR);
   }
...
/* Ouverture du fichier esclave */
   if (n_OpenFileAppl("ESTC1070A_I1", "rt", &(pbd_Sync->pf_InputFil)) == ERR) {
      RETURN_VAL(ERR);
   }
...
)
{
   ...
   DEBUT_FCT("n_TestRupture");
   	...
   	if ( ( s_ret = atoi(ptsz_LigneSuiv[RTO_SSD_CF]) - atoi(ptsz_LigneCour[RTO_SSD_CF]) ) != 0 ) return s_ret;
	if ( ( s_ret = atoi(ptsz_LigneSuiv[RTO_ESB_CF]) - atoi(ptsz_LigneCour[RTO_ESB_CF]) ) != 0 ) return s_ret;
	if ( ( s_ret = strcmp( ptsz_LigneSuiv[RTO_CTRNAT_CT], ptsz_LigneCour[RTO_CTRNAT_CT] ) ) != 0 ) return s_ret;
	if ( ( s_ret = atoi(ptsz_LigneSuiv[RTO_UWY_NF]) - atoi(ptsz_LigneCour[RTO_UWY_NF]) ) != 0 ) return s_ret;
	if ( ( s_ret = atoi(ptsz_LigneSuiv[RTO_LOBN2_NF]) - atoi(ptsz_LigneCour[RTO_LOBN2_NF]) ) != 0 ) return s_ret;
...
int n_ActionPremiereRupture(char *ptsz_LigneCour[])
{
   DEBUT_FCT("n_ActionPremiereRupture");
	...
	/* Synchronisation avec le fichier esclave � chaque rupture */
...
)
{
	...
	DEBUT_FCT( "n_ConditionSync" );
		...
		if ( ( ret = atoi(ptsz_LigneMaitre[RTO_UWY_NF]) - atoi(ptsz_LigneEsclave[CML_UWY_NF_2]) ) != 0 ) return ret;
		if ( ( ret = atoi(ptsz_LigneMaitre[RTO_LOBN2_NF]) - atoi(ptsz_LigneEsclave[CML_LOBN2_NF]) ) != 0 ) return ret;
...
)
{
	...
	char acmtrs[][4] = {"301","303","307","309","314","316","320"};
	...
	//define ACMTRS3 depending of ACMRTRS
