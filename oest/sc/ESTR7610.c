...
nom de l'application          : Edition de l'etat synthetique de controle
			        d'inventaire acceptation
nom du source                 : ESTR7610.c
...
------------------------------------------------------------------------------
description :
	Le fichier controle inventaire acceptation en entree est sous
...
historique des modifications :
   <jj/mm/aaaa>   <auteur>    <description de la modification>
    11/05/1998    F.BOULAROT  Modification de la préparation de l'état d'édition
                              On rajoute les lignes pour la nature NON AFFECTE
...
#define NB_LIGNE_MAX_PAGE 31    /*Nbre max de lignes dans une page*/
...
int main(int argc ,char *argv[])
{
  ...
  if (n_OpenFileAppl("ESTR7610_O1","wt",&Kp_OutFil) == ERR )
  ExitPgm ( ERR_XX , "" );
  ...
  if (n_CloseFileAppl("ESTR7610_I1",&(Kbd_Rupt.pf_InputFil)) == ERR)
    ExitPgm (ERR_XX,"");

  if (n_CloseFileAppl("ESTR7610_O1",&Kp_OutFil) == ERR)
    ExitPgm (ERR_XX ,"");
...
static int n_ChargLib()
{
  ...
  if (n_OpenFileAppl ("ESTR7610_I2","rt",&p_Libelle))
    RETURN_VAL (ERR);
  ...
  if (n_CloseFileAppl("ESTR7610_I2",&p_Libelle) == ERR)
    RETURN_VAL(ERR);
...
int n_InitEdit(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl ("ESTR7610_I1","rt",&(pbd_Rupt->pf_InputFil)))
  RETURN_VAL (ERR);
...
int n_ActionLigneEdit(char **ptb_InRec_Cur)
{
  ...
  switch (*ptb_InRec_Cur[SYNA_CTRNAT_CT])
	{
	  ...
	  case 'N' : 					/*Non Prop*/
							/*Working*/
		     if (*ptb_InRec_Cur[SYNA_WRKCAT_CT] == '1')
		       n_Num_Ligne = 9;
							/*Cat*/
		     if (*ptb_InRec_Cur[SYNA_WRKCAT_CT] == '2')
		       n_Num_Ligne = 13;
		     break;

	  case 'F' : n_Num_Ligne = 17;			/*Fac*/
		     break;

	  case 'X' : n_Num_Ligne = 21;			/*Ann*/
		     break;
	}
  ...
  if (n_Num_Ligne != 0)
    ...
    {
      d_AMT10000_M = atof(ptb_InRec_Cur[SYNA_AMT10000_M]) / 1000 ;
      d_AMT10030_M = atof(ptb_InRec_Cur[SYNA_AMT10030_M]) / 1000 ;
      d_AMT10031_M = atof(ptb_InRec_Cur[SYNA_AMT10031_M]) / 1000 ;
      d_AMT10100_M = atof(ptb_InRec_Cur[SYNA_AMT10100_M]) / 1000 ;
      d_AMT10130_M = atof(ptb_InRec_Cur[SYNA_AMT10130_M]) / 1000 ;
      d_AMT10400_M = atof(ptb_InRec_Cur[SYNA_AMT10400_M]) / 1000 ;
      d_AMT10430_M = atof(ptb_InRec_Cur[SYNA_AMT10430_M]) / 1000 ;

      d_AMT20000_M = atof(ptb_InRec_Cur[SYNA_AMT20000_M]) / 1000 ;
      d_AMT20030_M = atof(ptb_InRec_Cur[SYNA_AMT20030_M]) / 1000 ;
      d_AMT20031_M = atof(ptb_InRec_Cur[SYNA_AMT20031_M]) / 1000 ;
      d_AMT22000_M = atof(ptb_InRec_Cur[SYNA_AMT22000_M]) / 1000 ;
      d_AMT23000_M = atof(ptb_InRec_Cur[SYNA_AMT23000_M]) / 1000 ;
      d_AMT24030_M = atof(ptb_InRec_Cur[SYNA_AMT24030_M]) / 1000 ;
      d_AMT24031_M = atof(ptb_InRec_Cur[SYNA_AMT24031_M]) / 1000 ;
   ...
   n_Print_Cellule(sz_Page,n_Num_Ligne,110,&d_AMT10400_M,MASQ_MT,Kc_Lang);
   ...
   if (c_PbRatio245 != 1)	   /* Affichage normal */
    ...
    n_Print_Cellule(sz_Page,n_Num_Ligne+2,170,&d_RatioComb,MASQ_RATIO_PB,Kc_Lang);
      ...
      Kd_Cum_AMT10400_M += d_AMT10400_M;
...
int n_IsR1Page(char **ptb_InRec,char **ptb_InRec_Cur)
{
  ...
  if (strcmp(ptb_InRec[SYNA_SSD_CF],ptb_InRec_Cur[SYNA_SSD_CF])!=0)
    {
      Kc_NewSsd = 1;
      RETURN_VAL(1);
    }

  if (strcmp(ptb_InRec[SYNA_ESB_CF],ptb_InRec_Cur[SYNA_ESB_CF])!=0)
    RETURN_VAL(1);

  if (strcmp(ptb_InRec[SYNA_LOB_CF],ptb_InRec_Cur[SYNA_LOB_CF])!=0)
    RETURN_VAL(1);
...
int n_ActionFirstRuptPage (char **ptb_InRec_Cur)
{
  ...
  n_Lob_A_Afficher = strcmp(ptb_InRec_Cur[SYNA_LOB_CF],LOB_MAX);
  n_Esb_A_Afficher = strcmp(ptb_InRec_Cur[SYNA_ESB_CF],ESB_MAX);
  ...
  /* Recherche filiale */
  n_SSD_CF = atoi(ptb_InRec_Cur[SYNA_SSD_CF]);
  ...
  if (n_indSsd>=0)
    {
     ...
     if (n_Lob_A_Afficher != 0)
       {
         strcpy(sz_LOB_CF,ptb_InRec_Cur[SYNA_LOB_CF]);
  ...
  if (n_Esb_A_Afficher != 0)
    n_Print_Cellule(sz_Page,3,26,ptb_InRec_Cur[SYNA_ESB_CF],MASQ_STRING,Kc_Lang);
...
n_Print_Cellule(sz_Page,n_Num_Ligne,110,&Kd_Cum_AMT10400_M,MASQ_MT,Kc_Lang);
