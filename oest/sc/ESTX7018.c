...
nom du source                 : ESTX7018.c
revision                      : $Revision: 1.1.1.1 $
date de creation              : 29/11/2006
auteur                        : J.Ribot
references des specifications : transfert portefeuille etab - SPOT EST 13427
   ...
   )
{
	...
	if (n_OpenFileAppl ("ESTX7018_O1","wb",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7018_O2","wb",&Kp_crostclm) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7018_O3","wb",&Kp_dettrs) == ERR )
		ExitPgm (ERR_XX, "");

	if (n_OpenFileAppl ("ESTX7018_O4","wb",&Kp_crostrfa) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_Processing (&bd_utctlib) != CS_SUCCEED)
		ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7018_O1",&Kp_crostrf) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7018_O2",&Kp_crostclm) == ERR )
           ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7018_O3",&Kp_dettrs) == ERR )
		ExitPgm (ERR_XX, "");

	if (n_CloseFileAppl ("ESTX7018_O4",&Kp_crostrfa) == ERR )
           ExitPgm (ERR_XX, "");
...
CS_RETCODE n_Processing (T_UTCTLIB  *pbd_utctlib)
{
	...
	retcode = n_ExecCmdWithFetch(pbd_utctlib,"Select distinct T1.CTR_NF,      "\
                                    ...
                                    "      where T1.TRFSTS_CT = 14   "\
                                    "       and T1.TRFACCSTS_CT = 46   ",
				   n_retcFetchRowTRF) ;
