...
[006] 02/11/2020 LEL : Modification taille tableau gsz_seg_nf de 11 au 17 pour supporter SEG_NF TRANSITION (SEG_NF + REF Q)
...
int n_ActionPremiereRupture(char *ptsz_LigneCour[])
{
   DEBUT_FCT("n_ActionPremiereRupture");
	...
	if (strcmp(CTRGRO_CTR_SYNC,ptsz_LigneCour[PER_CTR_NF]) == 0 &&
		...
		(   *CTRGRO_UWY_SYNC == 0 || 
			*CTRGRO_UWY_SYNC == '0'  ||
			strcmp(CTRGRO_UWY_SYNC,ptsz_LigneCour[PER_UWY_NF])==0) ) // on garde le même segment car l'exercice n'a pas changé ou  il est vide
		RETURN_VAL(OK);
	else
		gsz_seg_nf[0] = 0; 
		n_ProcessingRuptureSyncVar(pbd_Sync, ptsz_LigneCour); /* Synchronisation avec le fichier esclave à chaque rupture */
