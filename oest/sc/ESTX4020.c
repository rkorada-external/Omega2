   ...
   )
{
	...
	if (n_OpenFileAppl ("ESTX4020_O1","wb",&Kp_OutputFilSsdActr) == ERR )
           ExitPgm (ERR_XX, "");
        
                  
        if (n_OpenFileAppl("ESTX4020_O2","wb",&Kp_SubtrsAssoFilout) == ERR ) 
  	       ExitPgm ( ERR_XX , "" );
	...
	if (n_CloseFileAppl ("ESTX4020_O1",&Kp_OutputFilSsdActr) == ERR )
           ExitPgm (ERR_XX, "");
           
         
        if (n_CloseFileAppl ("ESTX4020_O2",&Kp_SubtrsAssoFilout) == ERR ) 
   	       ExitPgm ( ERR_XX , "" );
...
CS_RETCODE n_Processing ()
{
	...
	DEBUT_FCT("n_Processing");
        ...
        bd_utctlib.n_RowFetchData = n_retcFetchRowSUBTRSASSO;
	      retcode = n_ProcessingProc (&bd_utctlib, 0,"BEST..PsSUBTRSASSO_01");
