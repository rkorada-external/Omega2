...
------------------------------------------------------------------------------
description :
   synchronisation du fichier PLAC avec le PERICASE RETRO
...
int main(int argc ,char *argv[])
{
  ...
  InitSig ();
    ...
    if ( n_OpenFileAppl ("ESTC2097_O1","wt",&Kp_OutputFil) == ERR )
        ExitPgm ( ERR_XX , "" );
    ...
    if (n_CloseFileAppl ("ESTC2097_O1",&Kp_OutputFil)== ERR)
        ExitPgm ( ERR_XX , "" );

    if (n_CloseFileAppl("ESTC2097_I1",&(bd_RuptPlac.pf_InputFil))== ERR )
        ExitPgm ( ERR_XX , "" );

    if (n_CloseFileAppl ("ESTC2097_I2",&(bd_RuptPerim.pf_InputFil))== ERR)
        ExitPgm ( ERR_XX , "" );
...
int n_InitPlac(T_RUPTURE_VAR  *pbd_Rupt)
{
    ...
    if ( n_OpenFileAppl ("ESTC2097_I1","rt",&(pbd_Rupt->pf_InputFil)))
        ExitPgm ( ERR_XX , "" );
...
int n_InitPerim(T_RUPTURE_SYNC_VAR  *pbd_Rupt)
{
    ...
    n_OpenFileAppl ("ESTC2097_I2","rt",&(pbd_Rupt->pf_InputFil));
                    ...
                    char **ptb_InRecChild)                      // adresse de la ligne de l'esclave
{
    ...
    ptb_InRecOwner[PLA1_ACCADMTYP_CT] = ptb_InRecChild[PER_ACCADMTYP_CT];

    ptb_InRecOwner[PLA1_ACCADMTYP_CT + 1]=0;
...
int n_ActionPlacPereSansFils(char **ptb_InRec)
{
    ...
    ptb_InRec[PLA1_ACCADMTYP_CT] = Ksav_ACCADMTYP_CT;
    ptb_InRec[PLA1_ACCADMTYP_CT+1]=0;
