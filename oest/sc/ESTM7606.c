/*==============================================================================
Nom de l'application          : Separation du GT retrocession en vie et non vie
Nom du source                 : ESTM7606.c
...
------------------------------------------------------------------------------
Description :
  Separation du GT retrocession en vie et non vie.
  En entree : le GT retrocession,
              le perimetre IRDVPERICASE.
...
[02] 17/12/2012 :spot:24041 - Solvency 2 - réactivation de l'écriturte sur le fichier Ano.
[03}  20/11/2014 Florent :spot:27747 corrections des warnings
==============================================================================*/
...
/* Lancement du traitement du fichier maitre */
   ...
   if (n_CloseFileAppl("ESTM7606_I1", &(pbd_Rupture->pf_InputFil)) == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTM7606_I2", &(pbd_Sync->pf_InputFil)) == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTM7606_O1", &Kp_OutputFileDom) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_CloseFileApplDom");
   }

   if (n_CloseFileAppl("ESTM7606_O2", &Kp_OutputFileVie) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_CloseFileApplVie");
   }

   if (n_CloseFileAppl("ESTM7606_O3", &Kp_OutputFileAno) == ERR) {
...
/* Ouverture du fichier maitre */
   if (n_OpenFileAppl("ESTM7606_I1", "rt", &(pbd_Rupture->pf_InputFil)) == ERR) {
      RETURN_VAL(ERR);
   }
...
/* Ouverture du fichier esclave */
   if (n_OpenFileAppl("ESTM7606_I2", "rt", &(pbd_Sync->pf_InputFil)) == ERR) {
      RETURN_VAL(ERR);
   }
...
)
{
   ...
   RETURN_VAL(strcmp(ptsz_LigneSuiv[GT_RETUW_NT], ptsz_LigneCour[GT_RETUW_NT]));
}
...
)
{
   ...
   if ( (s_ret = strcmp(ptsz_LigneMaitre[GT_RETCTR_NF], ptsz_LigneEsclave[PER_CTR_NF])) ) {
      return s_ret;
   }
   if ( (s_ret = strcmp(ptsz_LigneMaitre[GT_RETEND_NT], ptsz_LigneEsclave[PER_END_NT])) ) {
      return s_ret;
   }
   if ( (s_ret = strcmp(ptsz_LigneMaitre[GT_RETSEC_NF], ptsz_LigneEsclave[PER_SEC_NF])) ) {
      return s_ret;
   }
   if ( (s_ret = strcmp(ptsz_LigneMaitre[GT_RTY_NF], ptsz_LigneEsclave[PER_UWY_NF])) ) {
      return s_ret;
   }
   RETURN_VAL(strcmp(ptsz_LigneMaitre[GT_RETUW_NT], ptsz_LigneEsclave[PER_UW_NT]));
}
...
/*
   fprintf (Kp_OutputFileAno, "CTR %s, END %s, SEC %s, UWY %s,  UW %s : not in Retro Perimeter\n",
            ptsz_LigneMaitre[GT_RETCTR_NF],
            ptsz_LigneMaitre[GT_RETEND_NT],
            ptsz_LigneMaitre[GT_RETSEC_NF],
            ptsz_LigneMaitre[GT_RTY_NF],
            ptsz_LigneMaitre[GT_RETUW_NT]
   );
*/
   RETURN_VAL(OK);
}
