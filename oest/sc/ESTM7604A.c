...
[10] 03/09/2018 Charles Socie : EXT-IFRS17-903121  REQ 10.02 Cash flow: more detailed granularity ( split between variable and fixed premiums)
[11] 26/08/2020 HR SPIRA 82685 : struct.h
[12] 03/10/2024 JYP:spira 111541 : bugfix when CSUOE is missing into PERICASE
==============================================================================*/
...
static char VERSION_ESTM7604A_C[151] = "__version__: ESTM7604A.c version [012] 03/10/2024 bugfix CSUOE not in PERICASE " ;
...
/* Ouverture du fichier de sortie GT */
   if (n_OpenFileAppl("ESTM7604A_O1", "wt", &Kp_OutputFileGT) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_OpenFileAppl");
   }

  /* Ouverture du fichiers ANOS en sortie */
  if (n_OpenFileAppl("ESTM7604A_O2", "wt", &Kp_OutputFileOUTANOS) == ERR)  {
      ExitPgm(ERR_XX , "Erreur appel fonction n_OpenFileAppl");
   }

   if (n_OpenFileAppl("ESTM7604A_O3", "wt", &Kp_OutputFileGT3) == ERR) {
...
/* Lancement du traitement du fichier maitre */
   ...
   if (n_CloseFileAppl("ESTM7604A_I2", &(pbd_RupturePer->pf_InputFil)) == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTM7604A_I1", &(pbd_SyncGT->pf_InputFil)) == ERR) {
      ExitPgm(ERR_XX, "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTM7604A_O1", &Kp_OutputFileGT) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTM7604A_O2", &Kp_OutputFileOUTANOS) == ERR) {
      ExitPgm(ERR_XX , "Erreur appel fonction n_CloseFileAppl");
   }

   if (n_CloseFileAppl("ESTM7604A_O3", &Kp_OutputFileGT3) == ERR) {
...
/* Ouverture du fichier maitre */
   if (n_OpenFileAppl("ESTM7604A_I2", "rt", &(pbd_Rupture->pf_InputFil)) == ERR) 
   {
      RETURN_VAL(ERR);
   }
...
/* Ouverture du fichier esclave */
   if (n_OpenFileAppl("ESTM7604A_I1", "rt", &(pbd_Sync->pf_InputFil)) == ERR) {
      RETURN_VAL(ERR);
   }
...
int n_ActionLigneSyncGT( char *ptsz_LigneMaitre[], char *ptsz_LigneEsclave[] )
{
   ...
   //ptsz_LigneEsclave[128]  = ptsz_LigneMaitre[124]; 
...
int n_ActionFilsSansPerePer(char *ptsz_LigneEsclave[])
{
    ...
    {
	//[10] add ACMTRS3 at the end of te file 
	ptsz_LigneEsclave[GT_ACMTRS3_NT2+4] = ptsz_LigneEsclave[GT_ACMTRS3_NT2];
	ptsz_LigneEsclave[GT_ACMTRS3_NT2] = "";
	ptsz_LigneEsclave[GT_ACMTRS3_NT2+1] = "";
	ptsz_LigneEsclave[GT_ACMTRS3_NT2+2] = "";
	ptsz_LigneEsclave[GT_ACMTRS3_NT2+3] = "";
