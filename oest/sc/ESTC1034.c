...

{
 ...
 /* if (n_OpenFileAppl("ESTC1034_I1","rt",&Kp_GolIFil) == ERR )
	 ExitPgm ( ERR_XX , "" ); */

 /* ouverture du fichier peoplesoft */
  if (n_OpenFileAppl("ESTC1034_O1","wt",&Kp_GoelPeopOFil) == ERR )
	ExitPgm ( ERR_XX , "" );
  ...
  if (n_CloseFileAppl("ESTC1034_I1",&(Kbd_Rupt.pf_InputFil)) == ERR)
    ExitPgm (ERR_XX ,"");
  
  if (n_CloseFileAppl("ESTC1034_O1",&Kp_GoelPeopOFil) == ERR)
    ExitPgm (ERR_XX ,"");
...
int n_InitPeop(T_RUPTURE_VAR  *pbd_Rupt)
{
  ...
  if (n_OpenFileAppl ("ESTC1034_I1","rt",&(pbd_Rupt->pf_InputFil)))
  	RETURN_VAL (ERR); 
